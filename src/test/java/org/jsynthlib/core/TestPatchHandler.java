...
import org.jsynthlib.patchtest.LoadedParam;
import org.jsynthlib.patchtest.PatchFile;
import org.jsynthlib.patchtest.PatchFilesDocument;
import org.jsynthlib.patchtest.PatchFilesDocument.PatchFiles;
...
public class TestPatchHandler extends AbstractPatchHandler {
    ...
    public boolean isEmpty() {
        ...
        return patchFileArray != null && patchFileArray.length > 0
                && patchFileArray[0].getFileName() != null
                && !patchFileArray[0].getFileName().isEmpty();
    }
    ...
    @Override
    protected LoadedBank handleLoadedBank(PatchFile patchFile, int index,
            String value) {
        LoadedBank[] loadedBanks = patchFile.getLoadedBanksArray();
        assertTrue(loadedBanks.length > index);
        assertEquals("Check patch position", loadedBanks[index].getIndex(),
                index);
        assertEquals("Check patch name in bank", loadedBanks[index].getName(),
                value);
    ...
    protected void handleParamValue(XmlObject parent, int index, String value) {
        ...
        assertEquals("Check param position", loadedParams[index].getIndex(),
                index);
