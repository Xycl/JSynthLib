...
import org.jsynthlib.view.desktop.JSLWindowMenu;

import core.AppConfig;
import core.Constants;
import core.ErrorMsg;
import core.ExtensionFilter;
import core.IPatch;
import core.MacUtils;
import core.MidiMonitor;
...
final public class Actions {
    ...
    private static JMenu createPatchMenu(HashMap mnemonics, int mask) {
        ...
        mi.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P,
                KeyEvent.SHIFT_MASK + mask));// ctrl+shift+p
    ...
    public static class MenuFrame extends JSLFrame {
        ...
        public MenuFrame(String title, boolean addToolBar, boolean resizable,
        ...
        public MenuFrame(String title) {
            this(title, AppConfig.getToolBar(), true, true, true, true);
        }

        public void setVisible(boolean b) {
            if (isMac && !JSLDesktop.Factory.useMDI()) {
                JFrame frame = getJFrame();
                if (b) {
                    if (frame.getJMenuBar() == null)
                        setJMenuBar(frames.get(this) == null ? createMenuBar()
                                : (JMenuBar) frames.get(this));
                } else {
    ...
    private static PatchBasket getSelectedFrame() {
        return (PatchBasket) JSLDesktop.Factory.getDesktop().getSelectedFrame();
    }

    // //////////////////////////////////////////////////////////////////////
    /*
     * Now we start with the various action classes. Each of these preforms one
     * of the menu commands and are called either from the menubar, popup menu
     * or toolbar.
     */
    ...
    private static class PasteAction extends AbstractAction {
        ...
        public void setEnabled(boolean b) {
            try {
                ...
                b =
                        b
                                && f.canImport(cb.getContents(this)
            ...
            public void run() {
                try {
                    ...
                    if (frm != null) {
                        ...
                        /*
                         * if (frm instanceof PatchEditorFrame) for (int i = 0;
                         * i < ((PatchEditorFrame) frm).sliderList.size(); i++)
                         * { JSlider slider = (JSlider) ((PatchEditorFrame)
                         * frm).sliderList.get(i); Dimension dim =
                         * slider.getSize(); if (dim.width > 0) { dim.width++;
                         * slider.setSize(dim); } }
                         */
                        try {
    ...
    private static class OpenAction extends AbstractAction {
        ...
        static {
            ...
            filter =
                    new ExtensionFilter("JSynthLib Library/Scene Files (*"
                            + lext + ", *" + sext + ")", new String[] {
                            lext, sext });
        }
