...
import core.PatchesAndScenes;
import core.Scene;
import core.SwingWorker;
...
class SceneFrame extends AbstractLibraryFrame {
    ...
    void sendScene() {
        ...
        for (int i = 0; i < myModel.getRowCount(); i++) {
            ...
            scene.getPatch()
                    .send(scene.getBankNumber(), scene.getPatchNumber());
        }
    }

    public void storeSelectedPatch() {
        Scene scene =
                ((SceneListModel) myModel).getSceneAt(table.getSelectedRow());
        new SysexStoreDialog(scene.getPatch(), scene.getBankNumber(),
    ...
    void UpdatePatch(int row) { // wirski@op.pl
        ...
        if ((variableSize) || (sysexSize == patchSize)) {
            ...
            if (patarray.length == 1) {
                myModel.setPatchAt(patarray[0], row, scene.getBankNumber(),
                        scene.getPatchNumber());
            } else {
    ...
    public class UpdateSceneTask { // wirski@op.pl
        ...
        public UpdateSceneTask(PatchTableModel model, int[] indx) {
            ...
            for (int i = 0; i < noOfPatches; i++) {
                syxArray[i] =
                        ((SceneListModel) myModel).getSceneAt(i).getPatch()
    ...
    private class SceneListModel extends PatchTableModel {
        ...
        public boolean isCellEditable(int row, int col) {
            return (col > PATCH_NAME && !((col == BANK_NUM || col == PATCH_NUM) && ((Scene) list
                    .get(row)).getPatch().hasNullDriver()));
        }
