...
/**
 * Class for recording/playing back SAX events. Call recordX at the beginning of
 * X in you DefaultHandler. If an element has an id attribute, call
 * startRecording, and save the result. Recording will automatically stop when
 * that element is ended. Call replay on the recording to replay the events when
 * the id is referenced. Replayed events will not have the id attribute to
 * prevent recording looops.
 * @author ribrdb
 */
        ...
        public void replay(DefaultHandler h) throws SAXException {
            if (end == -1)
                throw new SAXException(
                        "Cannot replay recorded events until recording is finished.");
