...
class MidiConfigPanel extends ConfigPanel {
    ...
    private void setEnable(boolean midiEn) {
        cbxEnMC.setEnabled(midiEn && MidiUtil.isOutputAvailable()
                && MidiUtil.isInputAvailable());
        testButton.setEnabled(midiEn && MidiUtil.isOutputAvailable()
                && MidiUtil.isInputAvailable());
    ...
    void commitSettings() {
        if (cbxEnMidi.isSelected()) {
            ...
            AppConfig.setMidiOutBufSize(((Integer) spBufSize.getValue())
                    .intValue());
            AppConfig
                    .setMidiOutDelay(((Integer) spDelay.getValue()).intValue());
