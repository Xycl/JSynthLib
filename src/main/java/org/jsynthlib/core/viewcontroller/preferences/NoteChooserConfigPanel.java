...
/**
 * ConfigPanel for play note. Gives the user a way to change the note played...
 * when the user changes something in a synth editor.
 * @author Unknown - Hacked up by emenaker 2003.03.17
 * @author Hiroo Hayashi
 */
public class NoteChooserConfigPanel extends ConfigPanel implements
        ActionListener {
    ...
    private static final boolean USE_SEQUENCER = false;
    ...
    private static final String SEQ_LABEL = "MIDI Sequencer";
    private static final String TONE_LABEL = "Single Tone";
    ...
    NoteChooserConfigPanel(PrefsDialog parent) {
        ...
        if (USE_SEQUENCER) {
            ...
            b0.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    fc.showDialog(PatchEdit.getInstance(), "Choose MIDI file");
        ...
        nc.gridy = 0;
        nc.gridx = 0;
        notePanel.add(new JLabel("MIDI Note : "), nc);
        nc.gridx = 1;
        notePanel.add(s1, nc);
        nc.gridx = 2;
        notePanel.add(t1, nc);
        nc.gridy = 1;
        nc.gridx = 0;
        notePanel.add(new JLabel("Velocity  :  "), nc);
        nc.gridx = 1;
        notePanel.add(s2, nc);
        nc.gridx = 2;
        notePanel.add(t2, nc);
        nc.gridy = 2;
        nc.gridx = 0;
        notePanel.add(new JLabel("Duration  :"), nc);
        nc.gridx = 1;
        notePanel.add(s3, nc);
        nc.gridx = 2;
        notePanel.add(t3, nc);
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
        setContainerEnabled(sequencePanel,
                e.getActionCommand().equals(SEQ_LABEL));
    ...
    @Override
    protected void init() {
        note = getAppConfig().getNote();
        velocity = getAppConfig().getVelocity();
        delay = getAppConfig().getDelay();
        if (note <= 0 || note > 127) {
            note = DEFAULT_NOTE;
        }
        ...
        if (USE_SEQUENCER) {
            ...
            boolean isSeq = getAppConfig().getSequencerEnable();
            seqButton.setSelected(isSeq);
            toneButton.setSelected(!isSeq);
            t0.setText(getAppConfig().getSequencePath());
    ...
    protected void commitSettings() {
        if (USE_SEQUENCER) {
            /* Sequencer related parts */
            getAppConfig().setSequencerEnable(seqButton.isSelected());
            getAppConfig().setSequencePath(t0.getText());
        }

        getAppConfig().setNote(note);
        getAppConfig().setVelocity(velocity);
        getAppConfig().setDelay(delay);
        setModified(false);
    }
