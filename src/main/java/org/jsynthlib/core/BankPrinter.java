...
/**
 * Created by IntelliJ IDEA. User: Joe Emenaker Date: Oct 22, 2005 Time: 7:30:33
 * PM
 */
public class BankPrinter implements Printable {
    ...
    boolean includePageNumbers = true; // Include page numbers on the bottom of
                                       // the page?
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex)
        ...
        double pageHeight =
                pageFormat.getImageableHeight()
                        - (includePageNumbers ? fontHeight : 0);
        ...
        double tableHeaderHeightOnPage =
        ...
        int numRowsOnAPage =
                (int) ((pageHeight - tableHeaderHeightOnPage) / oneRowHeight);
        // How many pages is the table going to use?
        int totalNumPages =
                (int) Math
                        .ceil(((double) table.getRowCount()) / numRowsOnAPage);
        // If we're being asked to print a page past that, report that there are
        // no more pages to print.
        if (pageIndex >= totalNumPages) {
            return NO_SUCH_PAGE;
        }
        // How many rows are on *this* page
        int numRowsOnThisPage =
                table.getRowCount() - numRowsOnAPage * (totalNumPages - 1);

        // How much vertical space is used on the page, after scaling?
        double tableBodyHeightOnPage = oneRowHeight * numRowsOnThisPage;
        double tableTotalHeightOnPage =
                tableBodyHeightOnPage + tableHeaderHeightOnPage;

        g2.translate(pageLeft, pageTop); // Move to the top-left of the
                                         // printable area
        g2.translate(1, 1); // Make room for the top and left table border

        // If we want a page number, move to the bottom of the page and print it
        // now
        if (includePageNumbers) {
            ...
            g2.drawString(pageNumString, ((int) pageWidth - widthOfString) / 2,
                    (int) (pageHeight + fontHeight - fontDesent));// bottom
                                                                  // center
        }
        g2.translate((pageWidth - tableWidthOnPage) / 2, 0); // Center the table
                                                             // on the page
        ...
        /*
         * if (pageIndex + 1 == totalNumPages) { int lastRowPrinted =
         * numRowsOnAPage * pageIndex; int numRowsLeft = table.getRowCount() -
         * lastRowPrinted; g2.setClip(0, (int) (tableBodyHeightOnPage *
         * pageIndex), (int) Math.ceil(tableWidthOnPage), (int)
         * Math.ceil(oneRowHeight * numRowsLeft)); } else { //else clip to the
         * entire area available. g2.setClip(0, (int) (tableBodyHeightOnPage *
         * pageIndex), (int) Math.ceil(tableWidthOnPage), (int)
         * Math.ceil(tableBodyHeightOnPage)); }
         */
        ...
        table.paint(g2);
        g2.setTransform(unscaledTransform);
        g2.drawRect(0, 0, (int) tableWidthOnPage - 1,
                (int) tableTotalHeightOnPage);
        /*
         * For debugging. This draws lines on the page where the table should
         * go... // Draw lines over the row separators.... for(int i=0;
         * i<=numRowsOnThisPage; i++) { g2.drawLine(0,(int) ( oneRowHeight *
         * (double) i), (int) tableWidthOnPage, (int) ( oneRowHeight * (double)
         * i)); }
         */
        g2.setTransform(unscaledTransform);
        // If this isn't the first page, move the (0,0) point up off the top of
        // the page
