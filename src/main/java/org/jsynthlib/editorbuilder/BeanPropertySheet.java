...
public class BeanPropertySheet extends JPanel {
    ...
    public class TextWrapper extends PropertyEditorWrapper implements
         ...
         * (non-Javadoc)
         * @see
         * org.jsynthlib.editorbuilder.BeanPropertySheet.PropertyEditorWrapper
         * #getComponent()
         */
    ...
    public class TagWrapper extends PropertyEditorWrapper implements
            ActionListener {
    ...
    public class CustomWrapper extends PropertyEditorWrapper implements
            ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (ed instanceof JComponent)
                ((JComponent) ed).setBorder(BorderFactory.createEmptyBorder(6,
                        6, 6, 6));
            c.add(ed);
            d.pack();
            d.setVisible(true);
    ...
    protected abstract class PropertyEditorWrapper implements
        ...
        public PropertyEditorWrapper(Object bean, PropertyDescriptor desc,
                PropertyEditor editor) {
            this.bean = bean;
            this.desc = desc;
            this.editor = editor;
    ...
    private void positionEditors() {
        Spring width =
                Spring.sum(Spring.sum(nameWidthSpring, valueWidthSpring),
                        spacer(6));
        Spring valueSpring = Spring.sum(nameWidthSpring, spacer(4));

        Constraints myc = layout.getConstraints(this);
        myc.setConstraint(SpringLayout.SOUTH,
                Spring.sum(height, Spring.constant(6, 6, Integer.MAX_VALUE)));
    ...
    protected void addProperty(PropertyDescriptor descriptor, Object bean) {
        ...
        height =
                Spring.sum(Spring.max(
                        layout.getConstraint(SpringLayout.SOUTH, name),
                        layout.getConstraint(SpringLayout.SOUTH,
                                wrapper.getComponent())), spacer());
        editors.add(wrapper);
        labels.add(name);
    }

    protected PropertyEditorWrapper addWrapper(Object bean,
            PropertyDescriptor descriptor, PropertyEditor editor) {
        ...
        c.setY(height);
        valueWidthSpring =
                Spring.max(valueWidthSpring,
                        layout.getConstraints(w.getComponent()).getWidth());
        return w;
    }
    ...
    protected JLabel addLabel(PropertyDescriptor descriptor) {
        ...
        c.setY(height);
        nameWidthSpring =
                Spring.max(nameWidthSpring, layout.getConstraints(name)
                        .getWidth());
        return name;
    }
