...
class DeviceSearchHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        if (lasttag.equals("xdevice")) {
            tags.addFirst(el);
        } else if (lasttag.equals("name") || lasttag.equals("inquiryid")
                || lasttag.equals("manufacturer")) {
            throw new SAXParseException(lasttag + "can only contain text",
                    locator);
        } else {
            // these should come first, but don't want to get the name for
            // something other than a device
            tags.addFirst("x" + el);
        }
    }

    public String getManufacturer() {
        return manuf.toString();
    }
