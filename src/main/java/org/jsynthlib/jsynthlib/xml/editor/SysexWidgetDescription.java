...
public class SysexWidgetDescription extends WidgetDescription {
    ...
    private SysexWidget createWidget(XMLPatch patch) throws Exception {
        return (SysexWidget) cons.newInstance(new Object[] {
                patch, getParameter(patch) });
    }

    private Constructor findConstructor(Class c) {
        try {
            return c.getConstructor(new Class[] {
                    IPatch.class, SysexWidget.IParameter.class });
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private XMLParameter getParameter(XMLPatch patch) {
        return patch.getParameter(param.getGroup(), param.getSysex(),
                param.getName());
    }
