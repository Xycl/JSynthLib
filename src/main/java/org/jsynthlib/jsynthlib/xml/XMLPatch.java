...
public class XMLPatch implements ISinglePatch {
    ...
    protected XMLPatch(HashMap parameters, SysexGroup[] groups,
    ...
    protected XMLPatch(HashMap parameters, XMLDriver driver, XMLDevice device,
            SysexGroup[] groups, SysexDesc[] descs, int size) {
        this.parameters = parameters;
        this.driver = driver;
        this.device = device;
        this.groups = groups;
     ...
     * driver.checkAndPutPatch(this, singlePatch, patchNum); } public void
     * delete(int patchNum) { driver.deletePatch(this, patchNum); } public
     * IPatch get(int patchNum) { return ((IBankDriver) driver).getPatch(this,
     * patchNum); } public String getName(int patchNum) { return ((IBankDriver)
     * driver).getPatchName(this, patchNum); } public void setName(int patchNum,
     * String name) { ((IBankDriver) driver).setPatchName(this, patchNum, name);
     * }
     */
    ...
    protected void setMessages(SysexMessage[] messages) {
        if (messages.length != sysex.length) {
            throw new IllegalArgumentException(
                    "Wrong number of sysex messages.");
        }
        ...
        for (int i = 0; i < messages.length; i++) {
            ...
            while (it.hasNext()) {
                ...
                if (d.matches(header)
                        && d.getSize() == messages[i].getLength() - 1) { // don't
                                                                         // count
                                                                         // the
                                                                         // first
                                                                         // byte
            ...
            if (!found)
                throw new IllegalArgumentException(
                        "Unrecognized message with header " + header);
        }
        String s;
        try {
            XMLParameter p = getParameter("name");
