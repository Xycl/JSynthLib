...
public class XMLDriverImplementation {
    ...
    public static final MidiMessage midiMessage(String str)
            throws InvalidMidiDataException {
        str = whitespace.matcher(str).replaceAll("");
        if (str.length() % 2 == 1)
            throw new IllegalArgumentException(
                    "Message must have even number of characters");
        if (str.length() < 2)
            throw new IllegalArgumentException(
        ...
        for (int index = 0, start = 0; index < data.length; start += 2) {
            data[index++] =
                    (byte) Integer
                            .parseInt(str.substring(start, start + 2), 16);
        }
    ...
    public void playPatch(XMLPatch p) {
        try {
            ...
            msg.setMessage(ShortMessage.NOTE_ON, channel, AppConfig.getNote(),
