...
public class PatchLoadHandler extends AdvDefaultHandler {
        ...
        public Object generate(String element, String type) throws SAXException {
            if (curdec == null) {
                throw new SAXParseException(
                        "Parameters may only be contained by sysex messages with decoders.",
                        locator);
            }
    ...
    public class SysexBuilder {
        ...
        public SysexDesc getSysex() throws SAXException {
            XMLParameter[] pa =
                    (XMLParameter[]) params.values().toArray(
                            new XMLParameter[0]);
            if (size != -1) {
                if (decoder != null) {
                ...
                if (decoder == null)
                    throw new SAXParseException(
                            "Size must be specified for a sysex message if it has no decoder.",
                            locator);
                setSize(decoder.getSize());
            }
            SysexDesc d =
                    new SysexDesc(pa, header, size, decoder, checksum, name, id);
    ...
    public class GroupBuilder {
        ...
        public SysexGroup getGroup() {
            return new SysexGroup(name,
                    (SysexDesc[]) descs.toArray(new SysexDesc[0]));
        }
    }
    ...
    public static class PatchBuilder {
        ...
        public XMLPatch getPatch() {
            ...
            SysexGroup[] _groups =
                    (SysexGroup[]) groups.toArray(new SysexGroup[0]);
    ...
    private class PatchGenerator extends Generator {
        public Object generate(String element, String type) throws SAXException {
            if (curpatch != null) {
                throw new SAXParseException("Patches cannot contain patches",
                        locator);
            }
