 ...
 * concept is to store a sysex as a byte array, using an index to the special
 * bytes (eg <code>@@</code>, <code>*patchNum*</code>, ..) to insert values.
 * <OL>
 ...
 * <LI>On converstion to byte array (prior to being sent as a sysex message):
 * <DIR>
 * <LI><code>**</code> - Replace by the value of argument <code>value</code>.
 * <LI><code>@@</code> - Replaced by the value of argument <code>deviceID</code>
 * argument.
 * <LI><code>##</code> - Replaced by the value of argument <code>deviceID</code>
 * argument + 16.
 * <LI><code>*name*</code> - Replaced by the appropriate value by using a
 * <code>NameValue</code> instance. This is useful if you have to replace
 * multiple values. </DIR>
 * </OL>
 * Example:
 *
 ...
 * </pre>
 * @see NameValue
 * @see AbstractPatchDriver#sysexRequestDump
...
public class SysexHandler {
    ...
    private final Vector<NameValue> vNameValueIndex = new Vector<NameValue>();
    /** Index for "<code>@@</code>". */
    ...
    private void setSysex(String src) {
        ...
        for (int iByte = 0; st.hasMoreTokens(); iByte++) {
            ...
            case '*':
                vNameValueIndex.addElement(new NameValue(sToken.substring(1,
                        sToken.length() - 1), iByte));
                sysex[iByte] = (byte) 0;
                break;
            ...
            default:
                try {
                    ...
                    ErrorMsg.reportError("SysexHandler", "Invalid number, "
                            + sToken + " in space separated hex string: " + src);
                    return;
                }
                break;
            }
    ...
    private void setSysexFromDenseHexStr(String src) {
        ...
        for (int iByte = 0; iByte < nBytes; iByte++, iSrc += 2) {
            ...
            case '*':
                vNameValueIndex.addElement(new NameValue(src.substring(iSrc,
                        iSrc + 2), iByte));
                break;
            ...
            default:
                if (src.charAt(iSrc + 1) == '*') { // eg 0*
                    vNameValueIndex.addElement(new NameValue(src.substring(
                            iSrc, iSrc + 2), iByte));
                } else {
                    try {
                        sysex[iByte] =
                                (byte) Integer.parseInt(
                                        src.substring(iSrc, iSrc + 2), 16);
                    } catch (Exception ex) {
                        ErrorMsg.reportError("SysexHandler", "Invalid number, "
                                + src.substring(iSrc, iSrc + 2)
     ...
     * arguments.
     * <p>
     * <code>toSysexMessage(int deviceID, int value)</code> is a simple form of
     * this method.
     * @param deviceID
     *            Device ID
     * @param nameValue
    ...
    public byte[] toByteArray(int deviceID, NameValue[] nameValues) {
        ...
        // Replace values
        for (Enumeration<NameValue> en = vNameValueIndex.elements(); en
                .hasMoreElements();) {
            NameValue nameValueIndex = en.nextElement();

            boolean bValueFound = false;
            for (int i = 0; i < nameValues.length; i++) {
                if (nameValueIndex.getName().equalsIgnoreCase(
                        nameValues[i].getName())) {
                    sysex[nameValueIndex.getValue()] =
                            (byte) nameValues[i].getValue();
                    bValueFound = true;
                    break;
                }
            }
            if (!bValueFound) {
                ErrorMsg.reportError("SysexHandler", "No value specified for: "
                        + nameValueIndex.getName());
                return sysex;
            }
        }
        return sysex;
    ...
    public byte[] toByteArray(int deviceID, int value) {
        ...
        // Replace values
        Enumeration<NameValue> en = vNameValueIndex.elements();
        while (en.hasMoreElements()) {
            sysex[en.nextElement().getValue()] = (byte) value;
        }
    ...
    /**
     * Return a byte array where <code>@@</code>,<code>**</code> etc.. have been
     * replaced by 0. This is equivalent with <code>toByteArray(0, 0)</code>.
