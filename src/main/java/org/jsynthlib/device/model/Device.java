...
import org.jsynthlib.device.viewcontroller.DeviceDetailsDialog;
 ...
 * manufacturer, the model name, author(s) of drivers, etc. It also manages a
 * list of Driver classes which provides actual functions.
 * <p>
 * Subclass must define two constructor, with no argument and with one argument,
 * <code>Preferences</code>. Here is an example;
 *
 * <pre>
 *    import java.util.prefs.Preferences;
 *    ...
 ...
 * </pre>
 *
 * Compatibility Note: The following fields are now <code>private</code>. Use
...
public abstract class Device implements Iterable<IDriver> {
    ...
    /**
     * The fixed name of the model supported by this driver, as stated on the
     * type plate of the engine. eg TG33/SY22
     */
    ...
    private final List<IDriver> driverList = new ArrayList<IDriver>();
     ...
     *            stated on the type plate of the engine. eg TG33/SY22
     * @param inquiryID
     ...
     *            Authors of the device driver.
     */
    public Device(String manufacturerName, String modelName, String inquiryID,
            String infoText, String authors) {
        this.manufacturerName = manufacturerName;
        this.modelName = modelName;
        this.inquiryID = (inquiryID == null) ? "NONE" : inquiryID;
        this.infoText =
                (infoText == null) ? "There is no information about this Device."
                        : infoText;
    ...
    /**
     * Setter for property synthName. The synthName is your personal naming of
     * the device. A user can change it in the first column of the
     * Synth-Configuration dialog. modelName is used as default value. A synth
    ...
    /**
     * Setter for property channel which is used for playPatch, etc. The value
     * must be 1 or greater than 1, and 16 or less than 16. A synth driver may
     * use this method to set default value.
     * <p>
     * Some old drivers use this for device ID. Use setDeviceID method to set
     * device ID.
     * @param channel
    ...
    /**
     * Setter for property deviceID. The value must be 1 or greater than 1, and
     * 256 or less than 256. A synth driver may use this to set default device
     * ID.
     * <p>
     * For backward compatibility if this has the initial value (-1), The value
     * of <code>channel</code> is used as device ID.
     * @param deviceID
     ...
     */
    public final String getOutPortName() {
        String outPortName = prefs.get("port", "");
        if (!outPortName.isEmpty()) {
            return outPortName;
        } else {
            return midiSettings.getOutputName(midiSettings.getInitPortOut());
        }
    }

    /**
     * Setter for property port, the MIDI output port number, where the cable
     * <B>to</B> the device is connected. A synth driver should not use this.
     * @param outPortName
    ...
    public final void send(MidiMessage message) {
        ...
        try {
             ...
             * midiService.send(rcvr, message); else
             */
            midiService.send(outPortName, message,
                    Math.min(midiOutBufSize, midiSettings.getMidiOutBufSize()),
                    Math.max(midiOutDelay, midiSettings.getMidiOutDelay()));
    ...
    public final int getInPort() {
        ...
        String[] inputNames = midiSettings.getInputNames();
        for (int i = 0; i < inputNames.length; ++i) {
            if (inputNames[i].equals(uniqueName)) {
                return i;
            }
        }
        return midiSettings.getInitPortIn();
    }
    ...
    public final void setInPort(int inPort) {
        ...
        if (!initInPort || getInPort() != inPort) {
            midiService.setSysexInputQueue(inPort);
        }
        prefs.put("inPort", midiSettings.getInputNames()[inPort]);
        initInPort = true;
    }

    // Getters/Setters, etc for Drivers
    /**
     * Add Driver. Usually a constructor of a subclass of <code>Device</code>
     * calls this. Bulk converters must be added before simple drivers!
     * @param driver
     *            IDriver to be added.
     * @see IConverter
     */
    ...
    public final int getDeviceNum() {
        return deviceManager.getDeviceIndex(this);
    }
    ...
    public String getDeviceName() {
        ...
        sb.append("  -  MIDI Channel: ").append(getChannel());
        return sb.toString();
    }
    ...
    protected List<IDriver> getDriverList() {
        return driverList;
    }
}
