...
public class SysexMatcher extends Object {
    ...
    // ----- Instance variables
    protected String sSysexHeader; // String representation of header of sysex
                                   // data
    protected int numSysexMsgs; // Number of sysex messges in the sysex data
    protected byte[] sysex; // The actual data (stored by reference, not
                            // copied!)
    ...
    public void setSysex(byte[] sysex) {
        int nBytes =
                sysex.length < MAX_MATCH_LENGTH ? sysex.length
                        : MAX_MATCH_LENGTH;
        ...
        // ErrorMsg.reportStatus("SysexMatcher->setSysex | numSysexMsgs: " +
        // numSysexMsgs + " | size: " + this.sysex.length + " | sSysexHeader: "
        // + sSysexHeader);
    }
    ...
    public String detectCorruptSysex(IDriver driver, Patch patch,
        ...
        if (numSysexMsgs != 0 && numSysexMsgs != this.numSysexMsgs) {
            return ("Matches sysexID for driver: " + driver.toString() + "\n"
                    + "   But number of sysex messages in patch: "
                    + this.numSysexMsgs + " does not equal required number: "
                    + numSysexMsgs + "\n");
        }
