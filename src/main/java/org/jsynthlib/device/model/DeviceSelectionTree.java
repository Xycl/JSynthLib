...
public class DeviceSelectionTree extends JTree {
    ...
    public static final int GROUP_NONE = 0; // No ordering. Just a flat list of
                                            // devices
    public static final int GROUP_MANUF = 1; // Devices are grouped by
                                             // manufacturer
    public static final int GROUP_TYPE = 2; // Devices are grouped by type
    public static final int GROUP_TYPE_MANUF = 3; // Devices are grouped by
                                                  // type, then manufacturer
    ...
    public DeviceSelectionTree() {
        ...
        setSelectionModel(new DeviceSelectionModel()); // Prevent user from
                                                       // selecting anything but
                                                       // a device
    ...
    /**
     * This method gets a list of all known DeviceDescriptors and then passes
     * them to either buildTreeNoGrouping(), buildTreeByManufacturer(), or
     * buildTreeByType(), depending upon the groupStyle.
     * @param node
    ...
    /**
     * This method merely adds all items of the collection to the provided tree
     * node. No grouping is done.
     * @param col
     *            The collection of DeviceDescriptors to add
     * @param node
     *            The node of the tree to add the items to
     */
    ...
    /**
     * This method takes a collection of DeviceDescriptors and makes sub-nodes
     * for all manufacturers of the devices in the collection. If groupStyle is
     * GROUP_MANUF_TYPE, then it has buildTreeByType sub-group the devices even
     * further by type.
     * @param col
     *            The collection of DeviceDescriptors to add
     * @param node
     *            The node of the tree to add the items to
     */
    private void buildTreeByManufacturer(Collection col,
            DefaultMutableTreeNode node) {
        // Cycle through all of the manufacturers found in this colleciton
        for (Iterator i = getManufacturersInCollection(col).iterator(); i
                .hasNext();) {
            ...
            Collection subCol =
                    filterDescriptorsByManufacturer(col, manufacturer);
            // Now, if we're grouping by Manufacturer or by Type,Manufacturer,
            // then this is the last
            // step in the grouping. Just add the whole sub-collection to this
            // node. Otherwise, if we're
            // grouping by Manufacturer,Type, then we need to group this
            // sub-collection by type. Emenaker 2005-03-04
            switch (groupStyle) {
            case GROUP_MANUF_TYPE:
    ...
    /**
     * This method takes a collection of DeviceDescriptors and makes sub-nodes
     * for all types of the devices in the collection. If groupStyle is
     * GROUP_TYPE_MANUF, then it has buildTreeByManufacturer sub-group the
    ...
    private void buildTreeByType(Collection col, DefaultMutableTreeNode node) {
        ...
        for (Iterator i = getTypesInCollection(col).iterator(); i.hasNext();) {
            ...
            // Now, if we're grouping by Type or by Manufacturer,Type, then this
            // is the last
            // step in the grouping. Just add the whole sub-collection to this
            // node. Otherwise, if we're
            // grouping by Type,Manufacturer, then we need to group this
            // sub-collection by manufacturer. Emenaker 2005-03-04
            switch (groupStyle) {
            case GROUP_TYPE_MANUF:
    ...
    /**
     * Returns a sorted collection of manufacturers of the devices in a
     * collection
     * @param col
     *            The collection of DeviceDescriptors
    ...
    /**
     * Returns a sorted collection of DeviceDescriptors with the manufacturer
     * <code>manufacturer</code>
     * @param col
     *            The Collection of DeviceDescriptors to filter
     * @param manufacturer
     *            The manufacturer name
     * @return The sorted, filtered Collection of DeviceDescriptors
     */
    private Collection filterDescriptorsByManufacturer(Collection col,
            String manufacturer) {
    ...
    /**
     * Returns a sorted collection of DeviceDescriptors of the type
     * <code>type</code>
     * @param col
     *            The Collection of DeviceDescriptors to filter
     * @param type
    ...
    /**
     * This class overrides a normal DefaultTreeSelectionModel used by JTrees.
     * This is to prevent the user from being able to select any node that isn't
     * a *leaf* (ie, an actual devicename). This way, the user can't select
     * manufacturer and type "folders". Emenaker 2005-03-03
     */
    class DeviceSelectionModel extends DefaultTreeSelectionModel {
        ...
        public void addSelectionPath(TreePath path) {
            ...
            // Since we don't allow multiple selections, we'll just do
            // setSelectionPath
        ...
        /**
         * This overrides the superclass' setSelectionPath. We need to figure
         * out if the user clicked on a leaf (ie, an actual drivernname) or a
