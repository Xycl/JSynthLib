...
public class XmlDriverEditorController extends AbstractDriverEditor {
    ...
    @Inject
    public XmlDriverEditorController(@Assisted IDriver d,
            @Assisted XmlSingleDriverDefinition xmlDriverDef,
            @Assisted Patch p, ParamModelFactory pmFactory,
            MidiSenderFactory sFactory) {
        super(d, p, pmFactory, sFactory);
    ...
    final void initFX(JFXPanel fxPanel) {
        ...
        try {
            ...
            initNodeRecursive(xmlDriverDef.getPatchParams());
    ...
    void handleCombinedGroup(CombinedGroup combinedGroup) {
        ...
        final ISender sender =
                getSenderFactory().newSender(combinedGroup.getMidiSender(),
                        patch);
        final CombinedGroupModel paramModel =
                (CombinedGroupModel) getParamModelFactory().newParamModel(
                        combinedGroup.getParamModel(), patch);
    ...
    void handleTextField(final TextField tf, StringParamSpec paramSpec) {
        ...
        if (senderSpec != null) {
            sender = getSenderFactory().newStringSender(senderSpec, patch);
        }
        StringModelReference stringModelSpec = paramSpec.getStringModel();
        PatchStringModel model = null;
        if (stringModelSpec != null) {
            model =
                    getParamModelFactory().newPatchStringModel(stringModelSpec,
                            patch);
        }
        ...
        for (EnvelopeNodeSpec envelopeNode : envelopeNodes) {
            ...
            if (xParamSpec != null) {
                ...
                if (xParam.isVariable()) {
                    final ISender sender =
                            getSenderFactory().newSender(
                                    xParamSpec.getMidiSender(), patch);
                    final IParamModel paramModel =
                            getParamModelFactory().newParamModel(
            ...
            if (yParamSpec != null) {
                ...
                if (yParam.isVariable()) {
                    final ISender sender =
                            getSenderFactory().newSender(
                                    yParamSpec.getMidiSender(), patch);
                    final IParamModel paramModel =
                            getParamModelFactory().newParamModel(
    ...
    void handleSlider(final Slider knob, final IntParamSpec paramSpec) {
        ...
        final IParamModel paramModel =
                getParamModelFactory().newParamModel(paramSpec.getParamModel(),
                        patch);
    ...
    public XmlSingleDriverDefinition getXmlDriverSpec() {
        return xmlDriverDef;
    }
