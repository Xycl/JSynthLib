 ...
 * @version $Id: EnvelopeWidget.java 1090 2008-01-23 00:43:39Z billzwicky $
 * @see Node
 * @see SysexWidget
 */
public class EnvelopeWidget extends SysexWidget {
    ...
    protected void createWidgets() {
        ...
        for (int i = 0; i < nodes.length; i++) {
            if (nodes[i].variableX) {
                params[j] =
                        new Param(new JLabel(nodes[i].nameX), new JTextField(
                                String.valueOf(nodes[i].pmodelX.get()), 4), i,
                                true);
            ...
            if (nodes[i].variableY) {
                params[j] =
                        new Param(new JLabel(nodes[i].nameY), new JTextField(
                                String.valueOf(nodes[i].pmodelY.get()), 4), i,
                                false);
                params[j].textField.setEditable(false);
                nodes[i].faderNumY = j;
                j++;
    ...
    public void setSliderNum(int num) {
        _setSliderNum(num);
        setToolTipText("Bank " + ((num - 1) / 16 + 1) + "  Slider "
                + ((num - 1) % 16 + 1) + "  to  Bank "
    ...
    public void setFaderValue(int fader, int value) {
        ...
        if (params[fader].isX) {
            value =
                    (int) (node.minX + ((float) (value) / 127 * (node.maxX - node.minX)));
            ...
            value =
                    (int) (node.minY + ((float) (value) / 127 * (node.maxY - node.minY)));
            // set paramModel and show the value.
        ...
        private class MyListener extends MouseInputAdapter {
            ...
            private boolean isClose(Node node, int x, int y) {
                ...
                return ((node.variableX || node.variableY)
                        && (Math.abs(x - node.posX) < DELTA) && (Math.abs(y
                        - node.posY) < DELTA));
            }
            ...
            public void mouseDragged(MouseEvent e) {
                ...
                if (node.variableX && x != oldx) {
                    ...
                    params[node.faderNumX].textField.setText(String
                            .valueOf(xVal));
                ...
                if (node.variableY && y != oldy) {
                    ...
                    params[node.faderNumY].textField.setText(String
                            .valueOf(yVal));
    ...
    protected static class Param {
        ...
        protected Param(JLabel label, JTextField textField, int node,
                boolean isX) {
            this.label = label;
    ...
    public static class Node {
        ...
        private final IParamModel pmodelX;
        private final IParamModel pmodelY;
        private final ISender senderX;
        private final ISender senderY;
        ...
        private final boolean variableY;
         ...
         * @param miny
         *            The minimum value permitted by the synth parameter which
         *            rides the Y axis of the node. Using <code>Node.SAME</code>
         *            for <code>miny</code> means that the height remains at
         *            whatever the previous node was at.
         * @param maxy
         *            The maximum value permitted by the synth parameter which
         *            rides the Y axis of the node. When <code>Node.SAME</code>
         *            is used for <code>miny</code>, this parameter is ignored.
         * @param pmodely
         *            The Parameter Model which provides reading/writing
         *            abilities to the sysex data representing the parameter.
         * @param basey
         ...
         *            synths when the Node is moved on the X axis direction.
         * @param sendery
         *            The ISender which send system exclusive messages to the
         *            synths when the Node is moved on the Y axis direction.
         * @param namex
         ...
         * @see EnvelopeWidget
         */
        public Node(int minx, int maxx, IParamModel pmodelx, int miny,
                int maxy, IParamModel pmodely, int basey, boolean invertx,
                ISender senderx, ISender sendery, String namex, String namey) {
            baseY = basey;
            minX = minx;
            maxX = maxx;
            minY = miny;
            ...
            variableX = minX != maxX;
            variableY = minY != maxY;
            if (variableX && (pmodelX == null || senderX == null) || variableY
                    && (pmodelY == null || senderY == null)) {
        ...
        void setFaderNumX(int faderNumX) {
            this.faderNumX = faderNumX;
        }
        ...
        void setFaderNumY(int faderNumY) {
            this.faderNumY = faderNumY;
        }
        ...
        IParamModel getPmodelX() {
            return pmodelX;
        }

        IParamModel getPmodelY() {
            return pmodelY;
        }
