...
public class Yamaha01vDynamicsDriver extends AbstractPatchDriver {
    ...
    public Yamaha01vDynamicsDriver() {
        ...
        System.arraycopy(generateNumbers(1, 80, "Lib 00"), 0,
                patchNumbers, 0, 80);
        System.arraycopy(generateNumbers(1, 12, "Ch 0"), 0,
                patchNumbers, 80, 12);
        patchNumbers[91] = new String("Ch 13/14");
        patchNumbers[92] = new String("Ch 15/16");
        System.arraycopy(generateNumbers(1, 4, "AUX 0"), 0,
                patchNumbers, 93, 4);
        patchNumbers[96] = new String("ST MAS");
    }

    /**
     * Correct the patchNumber value due to the fact, that there is a hole in
     * the patch numbers in Equalizer Library dump request.
     * <p>
     */
    ...
    public Patch createNewPatch() {
        ...
        try {
            java.io.InputStream fileIn =
                    getClass().getResourceAsStream("01v_Dynamics.syx");
            fileIn.read(sysex);
            fileIn.close();

        } catch (Exception e) {
            log.warn("Unable to find 01v_Dynamics.syx.", e);
        }
        ;
