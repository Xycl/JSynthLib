...
public class Yamaha01vRemoteIntDriver extends AbstractPatchDriver {

    private static final SysexHandler SYS_REQ = new SysexHandler(
            "F0 43 *ID* 7E 4C 4D 20 20 38 42 33 34 49 *patchNum* F7");
    ...
    public Patch createNewPatch() {
        ...
        try {
            java.io.InputStream fileIn =
                    getClass().getResourceAsStream("01v_RemoteInt.syx");
            fileIn.read(sysex);
            fileIn.close();

        } catch (Exception e) {
            log.warn("Unable to find 01v_RemoteInt.syx.", e);
        }
        ;
