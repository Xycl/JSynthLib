 ...
 * @version $Id: WaldorfMW2BankDriver.java 939 2005-03-03 04:05:40Z hayashi $
 */
public class WaldorfMW2BankDriver extends AbstractBankDriver {
    ...
    public WaldorfMW2BankDriver() {
        ...
        this.patchNumbers =
                generateNumbers(1, MW2Constants.PATCH_NUMBERS, "000");
    ...
    protected int getPatchStart(int patchNum) {
        return (MW2Constants.PATCH_SIZE * patchNum)
    ...
    public String getPatchName(Patch p, int patchNum) {
        int nameStart =
                getPatchStart(patchNum) + MW2Constants.PATCH_NAME_START
                        - MW2Constants.SYSEX_HEADER_OFFSET;

        try {
            StringBuffer s =
                    new StringBuffer(new String(p.sysex, nameStart,
                            MW2Constants.PATCH_NAME_SIZE, "US-ASCII"));
            return s.toString();
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(p.sysex, MW2Constants.SYSEX_HEADER_OFFSET, bank.sysex,
                ...
                // The SYSEX_FOOTER_SIZE is for that the checksum is copied,
                // too!
    ...
    public void storePatch(Patch bank, int bankNum, int patchNum) {
        ...
        for (int patchNo = 0; patchNo < this.patchNumbers.length; patchNo++) {
            ...
            try {
                // Wait a little bit so that the Microwave 2 / XT doesn't hang
                // up
                Thread.sleep(50);
    ...
    protected Patch getPatch(Patch bank, int bankNum, int patchNum) {
        try {
            ...
            WaldorfMW2SingleDriver.calculateMW2Checksum(p.sysex,
                    MW2Constants.SYSEX_HEADER_OFFSET,
                    MW2Constants.SYSEX_HEADER_OFFSET
                            + MW2Constants.PURE_PATCH_SIZE - 1,
                    MW2Constants.SYSEX_HEADER_OFFSET
                            + MW2Constants.PURE_PATCH_SIZE);
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        // Request dumps for all 128 single programs of a bank (that are 128
        // requests!!!)
    ...
    protected BankPatch createNewPatch() {
        ...
        for (int patchNo = 0; patchNo < patchNumbers.length; patchNo++) {
            tempPatch =
                    getPatchFactory().createNewPatch(patchSysex, getDevice());
            WaldorfMW2SingleDriver.createPatchHeader(tempPatch, 0, patchNo);
            tempPatch.sysex[264] = MW2Constants.SYSEX_END_BYTE;
            System.arraycopy(patchSysex, 0, bankSysex, getPatchStart(patchNo)
                    - MW2Constants.SYSEX_HEADER_OFFSET, MW2Constants.PATCH_SIZE);
            setPatchName(bankSysex, patchNo, "New sound " + patchNo);
        }

        p = getPatchFactory().newBankPatch(bankSysex);

        for (int patchNo = 0; patchNo < patchNumbers.length; patchNo++) {
            WaldorfMW2SingleDriver.calculateMW2Checksum(p.sysex, offset
                    + this.checksumStart, offset + this.checksumEnd, offset
                    + this.checksumOffset);
