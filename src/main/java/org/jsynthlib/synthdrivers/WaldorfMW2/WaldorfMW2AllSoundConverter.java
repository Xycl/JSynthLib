 ...
 * @version $Id: WaldorfMW2AllSoundConverter.java 939 2005-03-03 04:05:40Z
 *          hayashi $
 */
public class WaldorfMW2AllSoundConverter extends Converter {
    ...
    private int getPatchStart(int patchNo) {
        return (MW2Constants.PURE_PATCH_SIZE * patchNo)
    ...
    public Patch[] extractPatch(Patch basePatch) {
        ...
        Patch tempPatch;
        byte[] bankSysex =
                new byte[MW2Constants.PATCH_SIZE * MW2Constants.PATCH_NUMBERS];

        // Convert the sounds of bank A
        for (int patchNo = 0; patchNo < MW2Constants.PATCH_NUMBERS; patchNo++) {
            System.arraycopy(baseSysex, getPatchStart(patchNo), temporarySysex,
                    MW2Constants.SYSEX_HEADER_OFFSET,
                    MW2Constants.PURE_PATCH_SIZE
                            + MW2Constants.SYSEX_FOOTER_SIZE);
            tempPatch = getPatchFactory().createNewPatch(temporarySysex, getDevice());

            WaldorfMW2SingleDriver.createPatchHeader(tempPatch, 0, patchNo);
            WaldorfMW2SingleDriver.calculateMW2Checksum(tempPatch.sysex,
                    MW2Constants.SYSEX_HEADER_OFFSET,
                    MW2Constants.SYSEX_HEADER_OFFSET
            ...
            System.arraycopy(tempPatch.sysex, 0, bankSysex, patchNo
                    * MW2Constants.PATCH_SIZE, MW2Constants.PATCH_SIZE);
        }

        tempPatch = getPatchFactory().createNewPatch(bankSysex, getDevice());
        ...
        int halfSize =
                MW2Constants.PURE_PATCH_SIZE * MW2Constants.PATCH_NUMBERS;
        int index = 0;
        bankSysex =
                new byte[MW2Constants.PATCH_SIZE * MW2Constants.PATCH_NUMBERS];
        ...
        for (int patchNo = 0; patchNo < MW2Constants.PATCH_NUMBERS; patchNo++) {
            ...
            System.arraycopy(baseSysex, index, temporarySysex,
                    MW2Constants.SYSEX_HEADER_OFFSET,
                    MW2Constants.PURE_PATCH_SIZE
                            + MW2Constants.SYSEX_FOOTER_SIZE);
            tempPatch = getPatchFactory().createNewPatch(temporarySysex, getDevice());
