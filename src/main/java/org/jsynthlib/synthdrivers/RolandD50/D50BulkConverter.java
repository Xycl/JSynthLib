...
public class D50BulkConverter extends Converter {
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        while (pos < sysex.length) {
            if ((sysex[pos] & 0xFF) == 0xF0) {
                ...
                for (int i = 0; i < D50Constants.PARTIALS_PER_MESSAGE; i++) {
                    if (partialCounter == 0) {
                        ...
                        byte[] buf =
                                new byte[D50Constants.SYSEX_HEADER.length
                                        + D50Constants.PARTIALS_SIZE
                                        + D50Constants.SYSEX_FOOTER.length];
                        System.arraycopy(D50Constants.SYSEX_HEADER, 0, buf, 0,
                                D50Constants.SYSEX_HEADER.length);
                        System.arraycopy(D50Constants.SYSEX_FOOTER, 0, buf,
                                buf.length - D50Constants.SYSEX_FOOTER.length,
                                D50Constants.SYSEX_FOOTER.length);
                        patchList.add(buf);
                    }
                    ...
                    System.arraycopy(sysex, pos, bs,
                            D50Constants.SYSEX_HEADER.length + partialCounter
                ...
                throw new IllegalArgumentException("Bad patch bank data: "
                        + Arrays.toString(sysex));
            }
        }
        ...
        for (int i = 0; i < patchList.size(); i++) {
            ...
            patches[i] = getPatchFactory().createNewPatch(bs);
        }
        return patches;
    }
}
