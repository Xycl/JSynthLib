 ...
 * Transfer) system exclusive message. Created: Wed Jun 25 22:57:37 2003
...
public class RolandDT1Sender implements ISender {
     ...
     *            3 byte ID : "00 00 01" -> 0x010000
     * @param modelId
     *            Model ID for System Exclusive Message. ("00 3F" -> 0x3f00)
     * @param addrSize
     ...
     *                if an error occurs
     */
    public RolandDT1Sender(int manufacturesId, int modelId, int addrSize,
            int addr, int dataSize) throws IllegalArgumentException {
        this.addrSize = addrSize; // save for setValue()
        this.dataSize = dataSize; // save for setValue()

        // Setup all data except channel, data and check-sum here for
        // efficiency.
        b =
                new byte[5 + (manufacturesId > 255 ? 3 : 1)
                        + (modelId > 255 ? 2 : 1) + addrSize + dataSize];
        int i = 0;
     ...
     */
    public RolandDT1Sender(int manufacturesId, int modelId, int addrSize,
            int dataSize) throws IllegalArgumentException {
    ...
    /**
     * Generate SysEX byte sequence. Address offset must be specified by
     * constructor.
     * @param value
     *            Data
     * @exception IllegalArgumentException
    ...
    public String toString() {
        return MidiMessageFormatter.hexDump(b, 0, b.length, 0);
    }
} // RolandDT1Sender
