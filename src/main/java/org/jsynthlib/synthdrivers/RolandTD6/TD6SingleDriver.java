 ...
 * @version $Id: TD6SingleDriver.java 907 2005-02-09 06:32:08Z hayashi $
 */
public final class TD6SingleDriver extends AbstractPatchDriver {
    ...
    /**
     * Patch must be sent in 13 packets. data packet for the 1st packet is 37
     * byte, and one for others is 55 byte.
     */
    private static final int[] PKT_SIZE = {
            37, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55 };
    ...
    public TD6SingleDriver() {
        ...
        /*
         * for (int i = 1; i <= NUM_PATCH; i++) patchNumbers[i - 1] = (i < 10 ?
         * "0" : "") + String.valueOf(i);
         */
    ...
    /**
     * Send a patch (bulk dump system exclusive message) to MIDI device. TD-6
     * has 99 drum kits. Drum kit can be selected only by a button on the
     * device. Either Bank select or Program Change message cannot select a drum
     * kit. A drum kit number is used as address for System Exclusive message.
     * Thus this driver ignores bank number and uses patch number as Drum Kit
     * number. "sendPatch()" is a proper name.!!!FIXIT!!!
     * @param p
     ...
     *            SysEX byte array.
     * @param offset
     *            offset index in <code>sysex</code>.
     * @param patchNum
     *            the patch number.
     */
    void storePatch(byte[] sysex, int offset, int patchNum) {
        ...
        for (int i = 0; i < NUM_PKT; i++, offset += size) {
            ...
            Patch p = getPatchFactory().createNewPatch(tmpSysex, (AbstractPatchDriver) null);
     ...
     * MIDI device. Use Drum Kit 99 as an edit buffer.
     * @param p
     *            a <code>Patch</code> value
     */
     ...
     * ErrorMsg.reportStatus("  start = " + start + ", end = " + end); int sum =
     * 0; for (int i = start; i <= end; i++) sum += b[i]; return (byte) (-sum &
     * 0x7f); }
     */
    /**
    ...
    /**
     * Create getPatchFactory().createNewPatch using a patch file <code>patchFileName</code>.
     * @return a <code>Patch</code> value
     */
    public Patch createNewPatch() {
        return (Patch) getPatchFactory().createNewPatch(this, patchFileName,
                SINGLE_SIZE);
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        send(SYS_REQ.toSysexMessage(getDeviceID(), new SysexHandler.NameValue(
                "patchNum", patchNum), new SysexHandler.NameValue("checkSum",
                checkSum)));
    }
