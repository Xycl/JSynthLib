...
class EffectsParamModel extends ParamModel {
    ...
    /**
     * Array of byte arrays representing default values for the effect
     * parameters. Every time one of the sixteen pre-defined effects
     * configurations is selected, the V-Amp sets up default effects parameters
     * in it's internal memory. The V-Amp editor has to emulate this to keep
     * it's internal buffer in sync with the device. These are the values the
     * editor uses to set up the defaults.
     */
     ...
     * @param offset
     *            The offset into the patch (including the size of the sysex
     *            header) representing the value to be edited.
     */
    ...
    public void set(int value) {
        ...
        log.info("  " + MidiMessageFormatter.hexDump(patch.sysex, 0, -1, 16));
    }

    /**
     * Gets the value of the byte in the sysex record defined by this
     * EffectsParamModel.
     * @return The value of this parameter.
     */
    public int get() {
        return (int) patch.sysex[offset];
    }
}