 ...
 * patch. When responding to a sysex data dump request, the device sometimes
 * sends a single sysex message and sometimes it sends a block of multiple
 * messages along with garbage data (the behavior is unpredictable). The last
 * valid sysex message sent is always the one we are interested in but it can be
 * preceded or succeeded with other garbage data. This converter will attempt to
 * parse out the last FCB patch in the block and discard the rest of the data.
 * @author Jeff Weber
 */
public class VAmp2Converter extends Converter {
     ...
     * out the last patch of a block of data and compare the header to the V-Amp
     * 2 header bytes. The job of actually converting the patch is handled by
     * extractPatch. If supportsPatch returns true, JSynthLib Core will call
     * extractPatch.
     * @param patchString
     *            String representing the patch header & identifying information
     *            of a driver.
     * @param sysex
     *            Byte array containing the sysex data.
     * @return Boolean representing if the patch is supported by this driver. A
     *         value of true indicates the patch is supported.
     */
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        log.info("  " + MidiMessageFormatter.hexDump(newPatchArray[0].sysex, 0, -1, 16));
        log.info(">>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<");
     ...
     * @return Sysex data for the last valid patch in a block of data, or if it
     *         cannot extract a valid patch, the original block of data is
     *         returned unchanged.
     */
            ...
            if (sysex[i] == Constants.VAMP2_DUMP_HDR_BYTES[0]) {
                if (isVAmpPatch(sysex, i)) {
                    ...
                    if ((i + Constants.HDR_SIZE + Constants.BANK_PATCH_SIZE) < sysex.length
                        ...
                        patchLength =
                                Constants.HDR_SIZE + Constants.BANK_PATCH_SIZE
                                        + 1;
                    }
     ...
     * opcode byte.
     * @return A boolean representing whether a patch is a valid V-Amp patch or
     *         not. A value of true indicates the patch is a valid V-Amp patch.
     */
