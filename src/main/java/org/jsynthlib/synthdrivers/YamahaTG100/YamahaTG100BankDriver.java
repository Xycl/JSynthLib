 ...
 * @version $Id: YamahaTG100BankDriver.java 871 2005-02-01 06:32:11Z jbackhaus $
 */
public class YamahaTG100BankDriver extends AbstractBankDriver {

    private final YamahaTG100SingleDriver singleDriver;
    ...
    public YamahaTG100BankDriver(YamahaTG100SingleDriver singleDriver) {
        ...
        this.patchSize = this.singleSize * patchNumbers.length; // Should be
                                                                // 6720 Bytes
    }

    public void calculateChecksum(Patch p, int patchNum) {
        calculateChecksum(p, getPatchStart(patchNum) + this.checksumStart,
                getPatchStart(patchNum) + this.checksumEnd,
                getPatchStart(patchNum) + this.checksumOffset);
    }

    /**
     * Replacement for sendPatchWorker which doesn't work here as each sysex sub
     * part has to be send separately
     */
    private final void doSendPatch(Patch p) {
        ...
        for (int patchNum = 0; patchNum < patchNumbers.length; patchNum++) {
            ...
            System.arraycopy(p.sysex, getPatchStart(patchNum), tempSysex, 0,
                    tempSysex.length);
        ...
        try {
            ...
            ErrorMsg.reportError("Error",
                    "Error requesting the internal bank.", ex);
        }
    }
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(p.sysex, 0, bank.sysex, getPatchStart(patchNum)
                + TG100Constants.SYSEX_HEADER_OFFSET, this.singleSize
                - TG100Constants.SYSEX_HEADER_OFFSET);
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        try {
            ...
            ErrorMsg.reportError("Error", "Error in Yamaha TG-100 Bank Driver",
                    e);
            return null;
        }
    }
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        nameStart += TG100Constants.PATCH_NAME_START; // offset of name in patch
                                                      // data
        try {
            StringBuffer s =
                    new StringBuffer(new String(p.sysex, nameStart,
                            TG100Constants.PATCH_NAME_SIZE, "US-ASCII"));
            return s.toString();
    ...
    public void setPatchName(Patch p, int patchNum, String name) {
        int tempPatchNameStart =
                this.patchNameStart + this.getPatchStart(patchNum);
    ...
    public BankPatch createNewPatch() {
        ...
        for (int patchNum = 0; patchNum < patchNumbers.length; patchNum++) {
            tempPatch = singleDriver.createNewPatch(patchNum);

            calculateChecksum(tempPatch);

            System.arraycopy(tempPatch.sysex, 0, sysex,
                    getPatchStart(patchNum), this.singleSize);
        }
