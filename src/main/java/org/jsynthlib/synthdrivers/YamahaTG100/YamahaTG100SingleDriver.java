 ...
 * @version $Id: YamahaTG100SingleDriver.java 717 2004-09-15 04:34:19Z hayashi $
 */
public class YamahaTG100SingleDriver extends AbstractPatchDriver {
    ...
    public YamahaTG100SingleDriver() {
        ...
        this.bankNumbers = new String[] {
            "Internal voice bank" }; // Bank is ignored since there is only one
                                     // bank for internal voices

        this.patchNumbers =
                generateNumbers(1, TG100Constants.PATCH_NUMBER_LENGTH, "#");
    ...
    private void storePatch(Patch p, int patchNum) {
        int iTemp =
                TG100Constants.SYSEX_VOICE_START_ADDRESS3
                        + (patchNum * TG100Constants.SYSEX_SINGLE_VOICE_SIZE);

        ((Patch) p).sysex[5] =
                (byte) ((iTemp / 128) + TG100Constants.SYSEX_VOICE_START_ADDRESS2);
     ...
     *            Ignored
     * @param patchNum
     *            The number of the internal voice memory
     */
     ...
     * TG100Constants.SYSEX_VOICE_START_ADDRESS3 + (patchNum *
     * TG100Constants.SYSEX_SINGLE_VOICE_SIZE); int startAddress2 = (int)
     * ((iTemp / 128) + TG100Constants.SYSEX_VOICE_START_ADDRESS2); int
     * startAddress3 = (int) (iTemp % 128); int sum = 0; int checksum = 0; //sum
     * = 0x4C + 0x4D + 0x20 + 0x20 + 0x30 + 0x30 + 0x36 + 0x38 + 0x52 + 0x51 //
     * + 0x30 + startAddress2 + startAddress3 +
     * TG100Constants.SYSEX_SINGLE_VOICE_SIZE; //sum = 0x30 + startAddress2 +
     * startAddress3 + TG100Constants.SYSEX_SINGLE_VOICE_SIZE; sum = 0x30 +
     * startAddress2 + startAddress3; checksum = (byte) (-sum & 0x7f);
     * //checksum = (byte) 0x00; SysexHandler.NameValue[] nameValues = { new
     * SysexHandler.NameValue("adressOffset2", startAddress2 ), new
     * SysexHandler.NameValue("adressOffset3", startAddress3 ), new
     * SysexHandler.NameValue("byteCount1", 0), new
     * SysexHandler.NameValue("byteCount2", 0), new
     * SysexHandler.NameValue("byteCount3",
     * TG100Constants.SYSEX_SINGLE_VOICE_SIZE), new
     * SysexHandler.NameValue("checksum", checksum) };
     ...
     * @param patchNum
     *            The number of Voice memory
     */
    public final Patch createNewPatch(int patchNum) {
        ...
        // Create getPatchFactory().createNewPatch on internal voice 1
        sysex[0] = (byte) 0xF0;
        sysex[1] = (byte) 0x43;
