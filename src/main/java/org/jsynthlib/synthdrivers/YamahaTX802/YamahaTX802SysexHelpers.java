 ...
 * ==============================================
 * @version $Id: YamahaTX802SysexHelpers.java 698 2004-09-11 04:39:44Z hayashi $
...
public class YamahaTX802SysexHelpers {
    ...
    protected final static int TG8 = 0x60; // Tongenerator 1 On/Off / Parameter
                                           // Select
    ...
    protected final static SysexHandler Button = new SysexHandler(
            "f0 43 @@ 1B *switch* *OnOff* f7"); // don't care about "OnOff"

    // switch off internal/cartridge memory protection
    protected static void swOffMemProt(IDriver d, byte ch) // port, channel
    {
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                SYSTEM_SETUP), new SysexHandler.NameValue("OnOff", DEPRESS)));
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                TG8), new SysexHandler.NameValue("OnOff", DEPRESS)));
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                OFF), new SysexHandler.NameValue("OnOff", DEPRESS)));
    }

    protected static void swOnMemProt(IDriver d, byte ch) // port, channel
    {
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                SYSTEM_SETUP), new SysexHandler.NameValue("OnOff", DEPRESS)));
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                TG8), new SysexHandler.NameValue("OnOff", DEPRESS)));
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                ON), new SysexHandler.NameValue("OnOff", DEPRESS)));
    }

    // choose the desired MIDI receive/transmit block
    protected static void chBlock(IDriver d, byte ch, byte bn) // port, channel,
    {
    ...
    {
        d.send(Button.toSysexMessage(ch, new SysexHandler.NameValue("switch",
                0x52), new SysexHandler.NameValue("OnOff", 0x00)));
