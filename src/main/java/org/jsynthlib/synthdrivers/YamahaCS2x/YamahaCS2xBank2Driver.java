...
public class YamahaCS2xBank2Driver extends AbstractBankDriver {
    ...
    public YamahaCS2xBank2Driver() {
        ...
        patchNumbers = generateNumbers(1, 128, "u##");
        patchNameStart = PATCHNAME_OFFSET;
        patchNameSize = 8; // name of the bank is based on sysex[6] of the first
                           // performance
        patchSize = BANK_SIZE;
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        for (i = 0; i < CS2x.numPerfInBank; i++) {
            /**
             * trying to get a wait dialog showing when the request is being
             * done. switch(i) { case 1: core.PatchEdit.showWaitDialog(
             * "requesting all performances of the selected bank,\n wait 3 minutes please,\n there's a lot to request and the CS2x is limited in speed."
             * ); break; case 127: core.PatchEdit.hideWaitDialog(); break;
             * //default: break; }
             */
            switch (bankNum) {
            case 0:
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        if (oSize == p.sysex.length) {
            System.arraycopy(p.sysex, 0, bank.sysex,
                    oStart, oSize);
        } else if (csize == p.sysex.length
                && p.sysex[6] == (byte) 0x60) {
            System.arraycopy(p.sysex, 0, bank.sysex,
                    oStart, 95);
            System.arraycopy(p.sysex, 97, bank.sysex, 95,
                    20);
        }
    }
     ...
     *            bank patch
     * @param patchNum
     *            patch, in the bank, to give new name
     * @param name
     *            getPatchFactory().createNewPatchName value
     */
    @Override
    ...
    public void calculateChecksum(Patch p) {
        ...
        for (i = 0; i < 128; i++) { // for each performance, do this:
            oPatchStart = getPatchStart(i); // find the start of the performance
                                            // in the bank
            calculateChecksum(p, oPatchStart + 4, oPatchStart + 60,
                    oPatchStart + 61); // common part1
            calculateChecksum(p, oPatchStart + com1 + 4, oPatchStart + com1
                    + 29, oPatchStart + com1 + 30); // common part2
            calculateChecksum(p, oPatchStart + com2 + 4, oPatchStart + com2
                    + 17, oPatchStart + com2 + 18); // common part3
            calculateChecksum(p, oPatchStart + com3 + 4, oPatchStart + com3
                    + 53, oPatchStart + com3 + 54); // layer 1
            calculateChecksum(p, oPatchStart + com3 + lyr + 4, oPatchStart
                    + com3 + lyr + 53, oPatchStart + com3 + lyr + 54); // layer
                                                                       // 2
            calculateChecksum(p, oPatchStart + com3 + (2 * lyr) + 4,
                    oPatchStart + com3 + (2 * lyr) + 53, oPatchStart + com3
                            + (2 * lyr) + 54); // layer 3
            calculateChecksum(p, oPatchStart + com3 + (3 * lyr) + 4,
                    oPatchStart + com3 + (3 * lyr) + 53, oPatchStart + com3
                            + (3 * lyr) + 54); // layer 4
        }
    }
     ...
     * @param bankNum
     *            the bank to store this bank in.
     */
    @Override
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        byte bl2;
        byte bl3;
        byte bl4; // the bytes about the bank we're working with
        ...
        case 0: // it's User Bank 1
            ...
            bl2 = 0x72;
            bl3 = 0x73;
            bl4 = 0x74; // initialize the bytes for later
            break;
        case 1: // it's User Bank 2
            ...
            bl2 = 0x7A;
            bl3 = 0x7B;
            bl4 = 0x7C; // initialize the bytes for later
            break;
        }
        ...
        int i;
        int oPatchStart;
        if (bc == bs) { // the source bank is the same as the destination bank,
                        // just send all messages
            for (i = 0; i < 128; i++) { // for each performance, do this:
                oPatchStart = getPatchStart(i); // find the start of the
                ...
                CS2x.sleep();
                // layer 3
                System.arraycopy(p, oPatchStart + com3 + (2 * lyr), lyrX, 0,
                        lyr);
                send(lyrX);
                CS2x.sleep();
                // layer 4
                System.arraycopy(p, oPatchStart + com3 + (3 * lyr), lyrX, 0,
                        lyr);
                send(lyrX);
                CS2x.sleep();
            }
            ...
            for (i = 0; i < 128; i++) { // for each performance, do this:
                ...
                System.arraycopy(p, oPatchStart, cmn1, 0, com1);
                cmn1[6] = bc;
                calculateChecksum(cmn1, 4, 60, 61);
                send(cmn1);
                CS2x.sleep();
                // common part2
                System.arraycopy(p, oPatchStart + com1, cmn2, 0, com2 - com1);
                cmn2[6] = bc;
                calculateChecksum(cmn2, 4, 29, 30);
                send(cmn2);
                CS2x.sleep();
                // common part3
                System.arraycopy(p, oPatchStart + com2, cmn3, 0, com3 - com2);
                cmn3[6] = bc;
                calculateChecksum(cmn3, 4, 17, 18);
                send(cmn3);
                CS2x.sleep();
                // layer 1
                System.arraycopy(p, oPatchStart + com3, lyrX, 0, lyr);
                lyrX[6] = bc;
                calculateChecksum(lyrX, 4, 53, 54);
                send(lyrX);
                CS2x.sleep();
                // layer 2
                System.arraycopy(p, oPatchStart + com3 + lyr, lyrX, 0, lyr);
    ...
    public boolean canHoldPatch(Patch p) {
        // if (p.sysex[1]==(byte)0x43 && p.sysex[3]==(byte)0x63 &&
        // p.sysex[4]==(byte)0x00 && p.sysex[5]==(byte)0x34) {// check header
        return (p.sysex.length == 339 || p.sysex.length == 341);// check
                                                                                    // size
        // } else {
        // return false;
        // }
