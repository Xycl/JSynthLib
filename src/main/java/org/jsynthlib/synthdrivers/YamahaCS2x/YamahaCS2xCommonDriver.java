 ...
 * @author peter
 */
public class YamahaCS2xCommonDriver extends AbstractPatchDriver {
    ...
    public YamahaCS2xCommonDriver() {
        ...
        sysexID = "F0430*630034****00"; // : The hex header that sysex files of
                                        // the format this driver supports will
                                        // have. The program will attempt to
                                        // match loaded sysex files with the
                                        // sysexID of a loaded driver. It can be
                                        // up to 16 bytes and have wildcards in
                                        // the form of an asterix.
        deviceIDoffset = 2;// : The location of the getDeviceID in the sysex
                           // byte array (@@ or ## in SYS_REQ)
        bankNumbers = new String[] {
                "Current Performance", "Bank1", "Bank2" }; // : String Array
                                                           // holding
                                                           // names/numbers for
                                                           // all banks. Used
                                                           // for comboBox
                                                           // selection.
        patchNumbers = generateNumbers(1, 128, "u##"); // : String
                                                                  // Array
                                                                  // holding
                                                                  // names/numbers
                                                                  // for all
                                                                  ...
                                                                  // comboBox
                                                                  // selection.
        patchSize = 0; // : The size of the patch this Driver supports (or 0 for
                       // variable).
        patchNameSize = 8; // : Number of characters in the patch name. (0 if no
                           // name)
        patchNameStart = 9;
        // sysexRequestDump ; //: You don't have to use this field when you're
        // overriding the requestPatchDump method.
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        final SysexHandler.NameValue[] nameValues = {
                new SysexHandler.NameValue("patchnum", pN), // = the patchNum
                                                            // from
                                                            // comboselection
                                                            // and
                new SysexHandler.NameValue("banknum", bN), // = the bankNum were
                                                           // chosen by the user
    ...
    public void sendPatch(Patch p) {
        System.arraycopy(p.sysex, 0, p1, 0, 63); // copy sysex message 1 to
                                                 // buffer
        p1[6] = (byte) 0x60; // make it a current performance
        p1[7] = (byte) 0x00;
        calculateChecksum(p1, 4, 60, 61);
        send(p1);
        CS2x.sleep(); // wait at least 120 milliseconds .
        if (p.sysex.length == 117 && p.sysex[6] == (byte) 0x60) { // the source
                                                                  // is a
                                                                  // 'current
                                                                  // performance'
            System.arraycopy(p.sysex, 63, p2c, 0, 34); // so copy part2 to p2c
            System.arraycopy(p.sysex, 97, p3, 0, 20);// and part3 to p3
            calculateChecksum(p2c, 4, 31, 32);
            calculateChecksum(p3, 4, 17, 18);
            send(p2c); // send part 2
            CS2x.sleep(); // wait at least 120 milliseconds .
            send(p3); // send and leave the if-else switch
        } else if (p.sysex.length == 115) { // source patch is of type 'bank1 or
                                            // bank2 common'
            System.arraycopy(p.sysex, 63, p2c, 0, 32); // copy part2
            p2c[6] = (byte) 0x60; // convert from user performance common to
                                  // current performance common
            p2c[7] = (byte) 0x00; // now this byte means 'common' instead of
                                  // 'patchNum'
            ...
            p3[6] = (byte) 0x60;
            p3[7] = (byte) 0x00;
            calculateChecksum(p3, 4, 17, 18); // checksum message 3
            send(p3); // send part 3
        }
    }
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        case 1: // user puts patch in bank 1
            ...
            p1[7] = (byte) patchNum;
            calculateChecksum(p1, 4, 60, 61);
            if (p.sysex.length == 117 && p.sysex[6] == (byte) 0x60) { // the
                                                                      // source
                                                                      // is a
                                                                      // 'current
                                                                      // performance'
                System.arraycopy(p.sysex, 63, p2u, 0, 32); // so copy only the
                                                           // first 32 bytes of
                                                           // part2!!
                System.arraycopy(p.sysex, 97, p3, 0, 20);// and part3 to p3,
                                                         // starting from byte
                ...
                p2u[7] = (byte) patchNum;
                calculateChecksum(p2u, 4, 29, 30);
            } else if (p.sysex.length == 115) { // source patch is of type
                                                // 'bank1 or bank2 common'
                System.arraycopy(p.sysex, 63, p2u, 0, 32); // copy part2
                System.arraycopy(p.sysex, 95, p3, 0, 20); // copy part3
                p2u[6] = (byte) 0x70; // bank 1
                p2u[7] = (byte) patchNum;
                calculateChecksum(p2u, 4, 31, 32);
            }
    ...
    public Patch createNewPatch() {
        ...
        InputStream fileIn =
                getClass().getResourceAsStream("InitPerfCommon.syx");
        try {
            fileIn.read(sysex);
            fileIn.close();
            return getPatchFactory().createNewPatch(sysex, this);
        } catch (Exception e) {
            ErrorMsg.reportError("Error", "Unable to find InitPerfCommon.syx",
                    e);
            return null;
        }
    }
