 ...
 * @version $Id$
 */
public class RocktronIntellifexBankDriver extends AbstractBankDriver {
    ...
    public void calculateChecksum(Patch p) {
        for (int i = 0; i < NUM_PRESETS; i++) {
            presetDriver.calculateChecksum(p.sysex, PRESET_SIZE * i + 6, PRESET_SIZE
                    * i + 6 + PRESET_DATA, PRESET_SIZE * i + 6 + PRESET_DATA);
        }

        int pos = PRESET_SIZE * NUM_PRESETS;
        s81Driver.calculateChecksum(p.sysex, pos + 6, pos + 6 + S81_DATA, pos + 6
                + S81_DATA);

        pos += S81_SIZE;
        s82Driver.calculateChecksum(p.sysex, pos + 6, pos + 6 + S82_DATA, pos + 6
                + S82_DATA);
    }
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        ...
        try {
            Patch p = getPatchFactory().createNewPatch(sysex, driver.getDevice());
            p.calculateChecksum();
            return p;
        } catch (Exception e) {
