...
public class SPD11PadConverter extends Converter {
     ...
     * and 1 "Settings" SinglePatch values. SPD11DumpallConverter: converts a
     * spd11 triggered "dump all" sysexfile into a jsynthlib SPD11 "Dump All"
     * Patch, which is a bankPatch containing 64 "Patch" BankPatches , a
     * "System" SinglePatch and a "Chain" Single Patch.
     */
    public SPD11PadConverter(SPD11PatchDriver theDriver) {
        ...
        settingsDriver = (SPD11SettingsDriver) theDriver.getPatchDriver(1); // was
                                                                            // kitDriver.getSettingsDriver(1);
        patchDriver = null;
    }
    ...
    public boolean supportsPatch(String header, byte[] sysex) {
        if (sysex.length < 16)
            return false;
        return sysex[0] == (byte) 0xF0 && sysex[1] == 0x41 && sysex[3] == 0x60
                && sysex[4] == 0x12;
    }
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        SysexMessage[] msgs = p.getMessages(); // get the sysexMessages from the
                                               // patch
