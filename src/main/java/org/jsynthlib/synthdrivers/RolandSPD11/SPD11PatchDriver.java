...
public class SPD11PatchDriver extends AbstractBankDriver {
     ...
     */
    public SPD11PatchDriver(SPD11PadDriver padDriver,
        ...
        singleSize = (0); // (can be 19 or 28) -> set to 0 for 'variable
                          // singleSize'.
     ...
     */
    // declared in SPD11_Constants.caluclateChecksum(Patch p, int start, int
    // end, int offset)
    /*
     * @Override protected void calculateChecksum(Patch p, int start, int end,
     * int offset) { int sum=0x00; //add all values of address and data for(int
     * i=start;i<offset;i++){ sum += p.sysex[i]; } //calculate checkSum with a
     * simple formula int chkSm = 128-(sum % 128); p.sysex[offset] =
     * (byte)chkSm; }
     */
    /**
    ...
    public BankPatch createNewPatch() {
        ...
        for (int i = 0; i < 32; i++) {
            pad1.sysex[7] = (byte) (i + 1); // +1 because 0 is "settings" and we
                                            // must start at 0 for the arraycopy
                                            // offsetvalue calculation
            // arraycopy(Object src, int srcPos,Object dest, int destPos,int
            // length)
            System.arraycopy(pad1.sysex, 0, patchSysex, 19 + (i * 28), 28);
        }
        BankPatch bp = getPatchFactory().newBankPatch(patchSysex, this);
        return bp;
    }

    /**
     * commented out to test and if I don't override this method, this driver
     * returns a CompletePatch. When I reassign it to a Patch, I can edit the
     * bankPatch in a BankEditor frame that can be printed.
     * @Override public Patch[] createPatches(SysexMessage[] msgs){ Patch[]
     *           padarray = new Patch[33]; // create a patch array for
     *           32pads+1settings //if msgs[0].getMessage() padarray[0] =
     *           settingsDriver.createNewPatch(msgs[0].getMessage()); //equals:
     *           padarray[0] = getPatchFactory().createNewPatch(msgs[0].getData(),settingsDriver); for
     *           (int i = 1; i < 33; i++) { // 32 times do this: (with each
     *           patch) padarray[i] =
     *           padDriver.createNewPatch(msgs[i].getMessage()); //padarray[i]=
     *           getPatchFactory().createNewPatch(msgs[i].getData(),padDriver); // copy the sysex of
     *           each message in a getPatchFactory().createNewPatch with this /* //check if byte nr8 is
     *           zero, set the driver to settingsDriver, else to padDriver.
     *           //padarray[i].setDriver((padarray[i].sysex[7] == 0x00) ?
     *           settingsDriver : padDriver); if ((padarray[i].sysex[7] == 0) &&
     *           padarray[i].sysex.length == 19) {
     *           padarray[i].setDriver(settingsDriver); } else {
     *           padarray[i].setDriver(padDriver); } } return padarray; }
     */
    ...
    public Patch getPatch(Patch patch, int patchNum) {
        if (patchNum == 0 && patch.sysex[7] == (byte) 0) { // it's the settings
            ...
            return getPatchFactory().createNewPatch(settings, settingsDriver);
        } else {
            byte[] sysex = new byte[28]; // it's a pad
            System.arraycopy(patch.sysex, 19 + (28 * (patchNum - 1)), sysex, 0,
                    28);
            return getPatchFactory().createNewPatch(sysex, padDriver);
        }
    }
     ...
     * @param patch
     *            the Patch of the drumkit aka bank
     * @param patchNum
     *            the given pad or the settings if patchNum is 0.
    ...
    public void putPatch(Patch bank, Patch single, int patchNum) {
        if ((single.getDriver() == settingsDriver)/** &&(patchNum == 0) */
            ...
            if (single.getDriver() == padDriver) {
                ...
                // (maybe this should be done only once after also the bankNum
                // has changed)
                SPD11_Constants.calculateChecksum(single, 5, 25, 26);
                // copy new single to new address in the bank
                System.arraycopy(single.sysex, 0, bank.sysex,
                        19 + (28 * patchNum), 28);
            }
        }
    }
     ...
     * patchNum is ignored
     * @param bankNum
     *            is the SPD-11 patch we request (0~63 = patch1~64)
     * @param patchNum
     */
    @Override
     ...
     * checksums with the new value in the addresses of each pad or the
     * settings.
     * <p>
     * @param p
     * @param bankNum
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            SPD11_Constants.calculateChecksum(p, 24 + i * 28, 25 + i * 28,
                    36 + i * 28);
        }
        // send the getPatchFactory().createNewPatch
        sendPatch(p);
    }
     ...
     * @TODO put method in SPD11_Constants because CompletePatchDriver also uses
     *       this
     */
    AbstractPatchDriver getPatchDriver(int i) {
        if (i == 0) {
            return padDriver;
        } else {
