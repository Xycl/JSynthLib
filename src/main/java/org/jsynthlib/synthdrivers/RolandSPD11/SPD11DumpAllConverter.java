...
public class SPD11DumpAllConverter extends Converter {
    ...
    public SPD11DumpAllConverter(SPD11Patch2Driver patch2Driver,
            SPD11ChainDriver chainDriver, SPD11SystemDriver systemDriver) {
        super("Dump All Converter", "Peter Geirnaert");
        sysexID = "F041**601200000000";
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        // create a Patch[] pa to hold the created Patches (2*Single and
        // 64*Bank)
        ...
        for (int i = 0; i < NR_OF_PATCHBANKS; i++) {
            ...
            // create a new CompletePatch with these bytes and put it in pa at
            // the current position
            pa[i] = getPatchFactory().createNewPatch(ptchbnk, patch2Driver);
        ...
        System.arraycopy(msgs, NR_OF_PATCHBANKS * PATCHBANKSIZE, chn, 0,
                CHAINSIZE);
        // create a new chain patch with these bytes and put it in the
        // patcharray
        pa[64] = getPatchFactory().createNewPatch(chn, chainDriver);
        // now create the system byte array sstm
        byte[] sstm = new byte[SYSTEMSIZE];
        // copy the system bytes into sstm
        System.arraycopy(msgs, (NR_OF_PATCHBANKS * PATCHBANKSIZE) + CHAINSIZE,
                sstm, 0, SYSTEMSIZE);
        // create a new system patch with these bytes and put it in the
        // patcharray
        pa[65] = getPatchFactory().createNewPatch(sstm, systemDriver);
