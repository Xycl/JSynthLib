...
public class Line6Pod20Converter extends Converter {
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        if (sysex[6] == (byte) 0x00) { // Is this a program patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new Line6Pod20SingleDriver());
        } else {
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new Line6Pod20BankDriver());
        }
