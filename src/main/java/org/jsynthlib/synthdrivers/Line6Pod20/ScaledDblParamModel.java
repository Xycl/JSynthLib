...
class ScaledDblParamModel extends ParamModel {
    ...
    /**
     * Reverses the range of values sent by the sysexWidget over the range max
     * to min. A value of true causes the range to be reversed.
     */
     ...
     * non-nibblized bytes, not including the header bytes.
     */
    ...
    /**
     * Constructs a ScaledDblParamModel. This is used to scale values when the
     * max sysex value is different than the max CC value and the minimum CC and
     * sysex values are not zero. Patch p is the reference to the patch
     * containing the sysex record. int o is the offset into the sysex record in
     * non-nibblized bytes, not including the header bytes. int minCC and int
     * maxCC are the the minimum and maximum CC values, respectively. int
     * minSysex and int maxSysex are the minimum and maximum sysex values,
     * respectively. boolean reverse reverses the range to min to max when set
     * to true.
     */
    ScaledDblParamModel(Patch p, int o, int minCC, int maxCC, int minSysex,
            int maxSysex, boolean reverse) {
        super(p, o);
     ...
     */
    public int get() {
        int msB = 0xFF & (int) PatchBytes.getSysexByte(patch.sysex, 9, offset);
        int lsB = 0xFF & (int) PatchBytes.getSysexByte(patch.sysex, 9, offset + 1);
