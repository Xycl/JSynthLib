 ...
 * 0-762, where the max value is represented in bytes 27-29 as 0x0002FA. The
 * value stored in offset 27-29 of the sysex record is simply the sum of the
 * course and fine values. So the value with the coarse and fine values both set
 * to maximum is 98298 or 0x017FFA. For the coarse control the control range of
 * 0-127 split over the range 0-97536 gives steps of 768 or 0x300. The two
 * values are separated as follows: <br />
 * <code>
...
class DelayCoarseSpeedModel extends ParamModel {
    /**
     * The minimum CC value that will be used for the control. This is used when
     * calculating a scaling factor. The scaling factor is calculated as
     * (maxSysex - minSysex) / (maxCC - minCC).
     */
    private int minCC = 0;

    /**
     * The maximum CC value that will be used for the control. This is used when
     * calculating a scaling factor. The scaling factor is calculated as
     * (maxSysex - minSysex) / (maxCC - minCC).
     */
    private int maxCC;
    ...
    /**
     * The scaling factor, calculated as (maxSysex - minSysex) / (maxCC -
     * minCC).
     */
     ...
     * containing the sysex record. int o is the offset into the sysex record in
     * non-nibblized bytes, not including the header bytes. int maxCC is the
     * maximum CC value. int maxSysex is the max sysex value
     */
     ...
     * Constructs a DelayCoarseSpeedModel. Patch p is the reference to the patch
     * containing the sysex record. int o is the offset into the sysex record in
     * non-nibblized bytes, not including the header bytes. int min CC and int
     * maxCC are the minimum and maximum CC values, respectively. int minSysex
     * and int maxSysex are the minimum and maximum sysex values, respectively
     */
    private DelayCoarseSpeedModel(Patch p, int o, int minCC, int maxCC,
            int minSysex, int maxSysex) {
        super(p, o);
        this.minCC = minCC;
        this.maxCC = maxCC;
    ...
    public void set(int i) {
        int sysexValue =
                ((i - minCC) * (maxSysex - minSysex) / (maxCC - minCC))
                        + minSysex;
        sysexValue += getFineValue();
        byte msB = (byte) (sysexValue / 256);
        byte lsB = (byte) (sysexValue % 256);
        PatchBytes.setSysexByte(patch, 9, offset, msB);
        PatchBytes.setSysexByte(patch, 9, offset + 1, lsB);
    }
    ...
    public int get() {
        ...
        returnValue =
                ((returnValue - minSysex) * (maxCC - minCC) / (maxSysex - minSysex))
                        + minCC;
        return returnValue;
    }
    ...
    private int getBytesValue() {
        int msB = (int) PatchBytes.getSysexByte(patch.sysex, 9, offset);
        int lsB = (int) PatchBytes.getSysexByte(patch.sysex, 9, offset + 1);
