...
public class NLPatchBankDriver extends AbstractBankDriver {
    ...
    public NLPatchBankDriver() {
        super("Patch Bank", "Kenneth L. Martinez",
                NLPatchSingleDriver.PATCH_LIST.length, 3);
        sysexID = "F033**04**";
        sysexRequestDump =
    ...
    protected void sendPatchWorker(Patch p, int bankNum) {
        ...
        int max;
        if (bankNum == 0) {
            max = 40; // only the first 40 programs are writeable in the user
                      // bank
        } else {
            max = NUM_IN_BANK;
        }
