...
public class AlesisDM5SysInfoEditor extends PatchEditorFrame {
    ...
    private void addTrigPane(Patch patch) {
        ...
        dm5EditPanel.setBorder(new TitledBorder(new EtchedBorder(
                EtchedBorder.RAISED), "System Info", TitledBorder.CENTER,
                TitledBorder.CENTER));
    ...
    private void addWidgets(JPanel panel, Patch patch) {
        addWidget(panel, new CheckBoxWidget("Omni Enable", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 1, true),
                new DM5SysInfoSender(patch, 1)), 0, 0, 1, 1, -1);

        addWidget(panel, new CheckBoxWidget("MIDI Thru Enable", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 2, false),
                new DM5SysInfoSender(patch, 1)), 0, 0, 1, 1, -2);

        addWidget(panel, new CheckBoxWidget("Program Change Enable", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 4, false),
                new DM5SysInfoSender(patch, 1)), 0, 0, 1, 1, -3);

        addWidget(panel, new CheckBoxWidget("Controllers Enable", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 8, false),
                new DM5SysInfoSender(patch, 1)), 0, 0, 1, 1, -4);

        addWidget(panel, new CheckBoxWidget("Drumset Edited", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 16, false),
                new DM5SysInfoSender(patch, 1)), 0, 0, 1, 1, -5);

        addWidget(panel, new ComboBoxWidget("Footswitch Mode", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 32, false),
                new DM5SysInfoSender(patch, 1), new String[] {
                        "Hi Hat", "Drumset Advance" }), 0, 0, 1, 1, 1);

        addWidget(panel, new CheckBoxWidget("Note Chase Enable", patch,
                new DM5SysInfoModel(patch, headerSize + 0, 64, false),
                new DM5SysInfoSender(patch, 1)), 0, 0, 1, 1, -6);

        addWidget(panel, new ComboBoxWidget("Currently Selected Drumset",
                patch, new ParamModel(patch, headerSize + 2),
                new DM5SysInfoSender(patch, 1), new String[] {
    ...
    class DM5SysInfoModel extends ParamModel {
        ...
        /**
         * Creates a new DM5SysInfoModel instance given the patch, the offset of
         * the data within the patch, the selected bit of the data, represented
         * by a power of two, and a boolean representing whether the values are
         * reversed (low to high).
         */
         ...
         */
        public int get() {
            int returnVal = (patch.sysex[offset] & bitmask) / mult;
            if (reverse) {
    ...
    /**
     * A subclass of SysexSender used to send the system info patch. Since the
     * patch is only 11 bytes, this sender just sends the whole patch.
     */
