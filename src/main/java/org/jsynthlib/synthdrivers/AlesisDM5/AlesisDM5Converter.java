...
/**
 * Removes "Garbage Data" from Alesis response to dump request and extracts
 * desired patch. When responding to a sysex data dump request, the DM5
 * sometimes sends a single sysex message and sometimes it sends a block of
 ...
 * converter will attempt to parse out the last DM5 patch in the block and
 * discard the rest of the data. It will work with any of the five patch types -
 * system info, edit buffer, program change table, single set, or trigger setup.
 * @author Jeff Weber
 */
public class AlesisDM5Converter extends Converter {
    ...
    public boolean supportsPatch(String patchString, byte[] sysex) {
        if (this.thisSupportsPatch(parseSysex(sysex))) { // Parse out the last
                                                         // patch in the block
                                                         // and test it.
            if ((sysex.length == Constants.HDR_SIZE + Constants.SYS_INFO_SIZE
                    + 1)
                    || (sysex.length == Constants.HDR_SIZE
                            + Constants.PROG_CHNG_SIZE + 1)
                    || (sysex.length == Constants.HDR_SIZE
                            + Constants.TRIG_SETP_SIZE + 1)
                    || (sysex.length == Constants.HDR_SIZE
                            + Constants.EDIT_BUFF_SIZE + 1)
                    || (sysex.length == Constants.HDR_SIZE
                            + Constants.SINGL_SET_SIZE + 1)) {
                return false; // If sysex has already been converted to program
                              // or bank, Converter no longer supports it.
            } else {
     ...
     * sysex record against the defined headers for each of the DM5 patch types
     * and returns true if there is a match. This is a more thorough check than
     * the isDM5Patch method, in that it checks the whole header, including the
     * opcode byte.
     */
    private boolean thisSupportsPatch(byte[] sysex) {
        if (isDM5Patch(sysex, 0)) {
            if (((sysex[6] == Constants.SYS_INFO_DUMP_HDR_BYTES[6]) && (sysex.length == Constants.HDR_SIZE
                    + Constants.SYS_INFO_SIZE + 1))
                    || ((sysex[6] == Constants.EDIT_BUFF_DUMP_HDR_BYTES[6]) && (sysex.length == Constants.HDR_SIZE
                            + Constants.EDIT_BUFF_SIZE + 1))
                    || ((sysex[6] == Constants.PROG_CHNG_DUMP_HDR_BYTES[6]) && (sysex.length == Constants.HDR_SIZE
                            + Constants.PROG_CHNG_SIZE + 1))
                    || ((sysex[6] == Constants.TRIG_SETP_DUMP_HDR_BYTES[6]) && (sysex.length == Constants.HDR_SIZE
                            + Constants.TRIG_SETP_SIZE + 1))
                    || ((sysex[6] >= (byte) 0x20 && (sysex[6] <= (byte) 0x34)) && (sysex.length == Constants.HDR_SIZE
                            + Constants.SINGL_SET_SIZE + 1))) {
                return true;
            } else {
                return false;
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        if (sysex[6] == (byte) 0x00) { // Is this a system info patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new AlesisDM5SysInfoDriver());
        } else if (sysex[6] == (byte) 0x01) { // Is this an edit buffer patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new AlesisDM5EdBufDriver()); // Convert
                                                                             // to
                                                                             // a
                                                                             // AlesisDM5SgSetDriver
        } else if (sysex[6] == (byte) 0x03) { // Is this a program change table
                                              // patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new AlesisDM5PrChgDriver());
        } else if (sysex[6] == (byte) 0x05) { // Is this a trigger setup patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new AlesisDM5TrSetDriver());
        } else if ((sysex[6] >= (byte) 0x20) && (sysex[6] <= (byte) 0x34)) { // Is
                                                                             // this
                                                                             // a
                                                                             // single
                                                                             // drumset
                                                                             // patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new AlesisDM5SgSetDriver());
        }
        return newPatchArray;
    }

    /**
     * Attempts to parse a Alesis patch (program, edit buffer, or bank) from a
     * Alesis response to a dump request. If parseSysex cannot find a valid
    ...
    private byte[] parseSysex(byte[] sysex) {
                    ...
                    if (isDM5Patch(sysex, i)) {
                        if ((sysex[i + 6] == Constants.SYS_INFO_DUMP_HDR_BYTES[6])
                                || (sysex[i + Constants.HDR_SIZE
                                        + Constants.SYS_INFO_SIZE] == (byte) 0xF7)) {
                            patchLength =
                                    Constants.HDR_SIZE
                                            + Constants.SYS_INFO_SIZE + 1;
                            if (sysex[i + 6] != (byte) 0x00) { // Sometimes Sys
                                                               // Info patch
                                                               // sends the
                                                               // wrong opcode
                        ...
                        if ((i + patchLength - 1) < sysex.length) {
                            if (sysex[i + patchLength - 1] == (byte) 0xF7) {
                                System.arraycopy(sysex, i, l6Sysex, 0,
                                        (patchLength));
                                break;
                            }
                        }
        ...
        if ((l6Sysex[0] == (byte) 0xF0)
                && (l6Sysex[patchLength - 1] == (byte) 0xF7)) {
