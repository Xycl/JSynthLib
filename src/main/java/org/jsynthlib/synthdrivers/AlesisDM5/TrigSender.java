 ...
 * by the TrigSender are specifically related to trigger setups--i.e. all the
 * parameters represented by a trigger setup sysex record.
 * @author Jeff Weber
 */
class TrigSender extends NRPNSender implements ISender {
     ...
     * NRPNs associated with any of the parameters of the trigger setup record.
     * A single invocation of the send method will cause two groups of NRPNs to
     * be send. The first group selects the active trigger and consists of the
     * MSB, the LSB representing the active trigger select command, and the
     * trigger number. The second group sets the value for the active trigger
     * and consts of the LSB to select the parameter, followed by the Data Entry
     * value for that parameter.
     */
    public void send(IDriver driver, int value) {
        if (trigNum != LAST_TRIG_NUM) {
            try {
                driver.send(newControlChange(driver, NRPN_MSB, 0)); // Send NRPN
                                                                    // MSB
                driver.send(newControlChange(driver, NRPN_LSB,
                        SELECT_ACTIVE_TRG)); // Command to select active trigger
                                             // number (NRPN LSB)
