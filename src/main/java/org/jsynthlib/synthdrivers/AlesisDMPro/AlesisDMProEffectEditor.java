...
class AlesisDMProEffectEditor extends PatchEditorFrame {
    ...
    public AlesisDMProEffectEditor(Patch patch) {
        ...
        addWidget(pnRev, new ComboBoxWidget("Reverb type", patch,
                ...
                new AlesisDMProFXSender(0, 1), new String[] {
                        "Plate1", "Plate2", "Room", "Hall", "Large", "Gate",
                        "Reverse" }), 0, 0, 2, 1, 0);
        ...
        // Overdrive
        addWidget(pnOverdrive, new ScrollBarWidget("Overdrive treshold", patch,
                0, 99, 0, new AlesisDMProFXModel(oParser, 7, 6, 7),
                new AlesisDMProFXSender(1, 0)), 0, 0, 2, 1, 10);

        addWidget(pnOverdrive, new ScrollBarWidget("Overdrive brightness",
                patch, 0, 99, 0, new AlesisDMProFXModel(oParser, 8, 5, 7),
        ...
        addWidget(pnDelay, new ScrollBarWidget("Delay input balance", patch,
                -99, 99, 0, new AlesisDMProFXModel(oParser, 10, 3, 8),
                new AlesisDMProFXSender(2, 1)), 2, 0, 2, 1, 13);

        addWidget(pnDelay, new ScrollBarWidget("Delay time 10ms", patch, 0, 79,
                0, new AlesisDMProFXModel(oParser, 11, 3, 7),
                new AlesisDMProFXSender(2, 2)), 0, 1, 2, 1, 15);

        addWidget(pnDelay, new ScrollBarWidget("Delay time 1ms", patch, 0, 9,
                0, new AlesisDMProFXModel(oParser, 12, 2, 4),
                new AlesisDMProFXSender(2, 3)), 2, 1, 2, 1, 16);

        addWidget(pnDelay, new ScrollBarWidget("Delay feedback", patch, 0, 99,
                0, new AlesisDMProFXModel(oParser, 12, 6, 7),
                new AlesisDMProFXSender(2, 4)), 0, 2, 2, 1, 17);

        addWidget(pnDelay, new ScrollBarWidget("Delay out to reverb", patch, 0,
                99, 0, new AlesisDMProFXModel(oParser, 13, 5, 7),
        ...
        addWidget(pnPitch, new ScrollBarWidget("Pitch input balance", patch,
                -99, 99, 0, new AlesisDMProFXModel(oParser, 15, 3, 8),
        ...
        addWidget(pnPitch, new ComboBoxWidget("Pitch Type", patch,
                ...
                new AlesisDMProFXSender(3, 2), new String[] {
                        "Mono Chorus", "Stereo Chorus", "Mono Flanger",
                        "Stereo Flanger", "Resonator" }), 0, 1, 2, 1, 20);

        addWidget(pnPitch, new ComboBoxWidget("Pitch Waveform", patch,
                new AlesisDMProFXModel(oParser, 16, 6, 1),
                new AlesisDMProFXSender(3, 4), new String[] {
                        "Sine", "Triangle" }), 2, 1, 2, 1, 21);
        ...
        addWidget(pnPitch, new ScrollBarWidget("Pitch feedback", patch, 0, 99,
                0, new AlesisDMProFXModel(oParser, 18, 5, 7),
                new AlesisDMProFXSender(3, 6)), 0, 3, 2, 1, 24);

        addWidget(pnPitch, new ScrollBarWidget("Pitch output to reverb", patch,
                0, 99, 0, new AlesisDMProFXModel(oParser, 19, 4, 7),
...
class AlesisDMProFXSender extends SysexSender {
    ...
    public byte[] generate(int value) {
        ...
        sysex[4] = (byte) 0x19;
        sysex[5] = (byte) 0x10;
        sysex[6] = (byte) (48 | (m_nFunc & 15)); // 3 << 4 = Effectedit, low
                                                 // nibble = func
...
class AlesisDMProFXModel implements IParamModel {
    ...
    public AlesisDMProFXModel(AlesisDMProParser oParser, int nByte, int nBit,
            int nBits) {
