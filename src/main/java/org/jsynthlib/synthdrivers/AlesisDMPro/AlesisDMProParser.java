...
public class AlesisDMProParser {
    ...
    /**
     * Gets value from raw bytes. nBits is number of bits No parameter covers
     * more than 2 bytes Arguments are sent according to docs (compressed,
     * "raw") TODO: Variable names could be more clear
     */
    public int getValue(int nByte, int nBit, int nBits) {

        int nStartBit = (nByte * 8) + nBit; // Picture a bitarray...
        int nStartByte = (nStartBit / 7) + 7; // Sysex only holds 7 bits in each
                                              // byte, + 7 for header
        ...
        int n1 =
                (m_p.sysex[nStartByte] >> nBitNumber)
                        & LengthBitMask(nShiftDown);
        int n2 = 0;

        if (nRemain > 0)
            n2 =
                    (m_p.sysex[nStartByte + 1] & LengthBitMask(nRemain)) << (nShiftDown - 1);
    ...
    public void setValue(int nByte, int nBit, int nBits, int nValue) {
        ...
        // We need to adjust mask to empty 8'th bit in sysex if it spans more
        // than one byte
        if (nRemain > 0)
