...
// @version $Id: KawaiK5000CombiDriver.java 939 2005-03-03 04:05:40Z hayashi $
...
public class KawaiK5000CombiDriver extends AbstractPatchDriver {
    final static int PATCH_DATA_START = 8;
    // PATCH_DATA_SIZE = (check sum) + (effect DATA) + (common DATA) + (Section
    // DATA)*4 = 1 + 38 + 15 + 13 * 4 = 106
    // ... however 103 works!!
    ...
    final static byte[] BSYSEX_HEADER = (new SysexHandler(
            "F0 40 00 20 00 0A 20 00")).toByteArray();
    ...
    public Patch createPatchFromData(byte[] data, int dataOffset,
        ...
        System.arraycopy(data, dataOffset, sysex, BSYSEX_HEADER.length,
                dataLength);
