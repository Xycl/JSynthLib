...
// @version $Id: PeaveyPC1600BankDriver.java 667 2004-08-14 22:27:38Z ribrdb $
...
public class PeaveyPC1600BankDriver extends AbstractBankDriver {
    final static int NIBBLE_MULTIPLIER =
            PeaveyPC1600SingleDriver.NIBBLE_MULTIPLIER;

    final static int PATCH_NAME_SIZE = PeaveyPC1600SingleDriver.PATCH_NAME_SIZE;
    final static int PATCH_NAME_CHAR_BYTES =
            PeaveyPC1600SingleDriver.PATCH_NAME_CHAR_BYTES;

    final static int NUM_PATCHES =
            PeaveyPC1600SingleDriver.PATCH_NUMBERS.length;

    final static int FIRST_NAME_START = 9; // (F0 00 00 1B 0B 00 01 32 nn).size

    // <memory pointer> == Preset Pointer
    final static int FIRST_PRESET_POINTER_START = FIRST_NAME_START
            + NUM_PATCHES * PATCH_NAME_SIZE
            * PeaveyPC1600SingleDriver.PATCH_SIZE_FACTOR;
    final static int PRESET_POINTER_BYTES = 4;
    final static int FIRST_PRESET_START = FIRST_PRESET_POINTER_START
            + PRESET_POINTER_BYTES * (NUM_PATCHES + 1);
    final static int PRESET_POINTER_FACTOR = 2;

    public int[] patchIndex = new int[NUM_PATCHES + 1];
    public Patch indexedBank = null;

    final static byte[] BSYSEX_INIT_PRESET =
            (new SysexHandler(
                    "f0 00 00 1b 0b 0d 04 02 0d 02 0d 04 09 06 0e 06 09 07 04 06 09 06 "
                            ...
                            + "01 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 01 00 "
                            + "00 00 01 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 "
                            + "01 00 00 00 01 00 00 00 01 00 00 f7"))
                    .toByteArray();

    final static byte[] BSYSEX_HEADER = (new SysexHandler(
            "F0 00 00 1B 0B 00 04")).toByteArray();
    ...
    public PeaveyPC1600BankDriver() {
        super("Bank", "Phil Shepherd",
                PeaveyPC1600SingleDriver.PATCH_NUMBERS.length, 3);
    ...
    public void generateIndex(Patch bank) {
        ...
        NibbleSysex nibbleSysex =
                new NibbleSysex(bank.sysex, FIRST_PRESET_POINTER_START);
        for (int i = 0; i < getNumPatches() + 1; i++) {
            patchIndex[i] =
                    nibbleSysex.getNibbleInt(PRESET_POINTER_BYTES,
                            NIBBLE_MULTIPLIER) * PRESET_POINTER_FACTOR;
        }
    ...
    public String getPatchName(Patch bank, int patchNum) {
        NibbleSysex nibbleSysex =
                new NibbleSysex(((Patch) bank).sysex, FIRST_NAME_START
                        + patchNum * PATCH_NAME_SIZE * PATCH_NAME_CHAR_BYTES);
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        int newPatchSize =
                ((Patch) p).sysex.length
                        - PeaveyPC1600SingleDriver.NON_DATA_SIZE;

        ((Patch) bank).sysex =
                Utility.byteArrayReplace(((Patch) bank).sysex,
                        FIRST_PRESET_START + patchIndex[patchNum],
                        oldPatchSize, ((Patch) p).sysex,
        ...
        NibbleSysex nibbleSysex =
                new NibbleSysex(((Patch) bank).sysex,
                        FIRST_PRESET_POINTER_START);
        for (int i = 0; i < getNumPatches(); i++) {
            nibbleSysex.putNibbleInt(patchIndex[i] / PRESET_POINTER_FACTOR,
                    PRESET_POINTER_BYTES, NIBBLE_MULTIPLIER);
        }
    }
    ...
    public void deletePatch(Patch bank, int patchNum) {
        // ErrorMsg.reportStatus("PeaveyPC1600BankDriver->deletePatch: " +
        // patchNum);

        Patch p = getPatchFactory().createNewPatch(BSYSEX_INIT_PRESET, getDevice());
        // ErrorMsg.reportStatus("  PeaveyPC1600BankDriver->deletePatch: " +
        // p.getDriver().getPatchName(p));
    ...
    public Patch createPatchFromData(byte[] data, int dataOffset,
            int dataLength, String name) {
        byte[] sysex =
                new byte[dataLength + BSYSEX_HEADER.length + PATCH_NAME_SIZE
                        * PATCH_NAME_CHAR_BYTES
        ...
        nibbleSysex.putNibbleStr(name, PATCH_NAME_SIZE, PATCH_NAME_CHAR_BYTES,
                NIBBLE_MULTIPLIER);
        nibbleSysex.putNibbleInt(dataLength
                / PeaveyPC1600SingleDriver.PATCH_SIZE_FACTOR,
                PeaveyPC1600SingleDriver.PATCH_SIZE_BYTES, NIBBLE_MULTIPLIER);
