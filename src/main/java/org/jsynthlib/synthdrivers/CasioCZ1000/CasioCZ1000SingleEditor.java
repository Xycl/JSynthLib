 ...
 * not the 263-byte version from the synth.
 * <P>
...
public class CasioCZ1000SingleEditor extends PatchEditorFrame {
    ...
    public CasioCZ1000SingleEditor(Patch patch) {
        ...
        hackMode =
                "Extended".equals(patch.getDevice().getPreferences()
                        .get("paramMode", null));
        wideGui =
                "Wide".equals(patch.getDevice().getPreferences()
                        .get("guiMode", null));
    ...
    public JComponent getWideGUI(Patch patch) {
        JComponent paramgui = getParamGUI(patch);
        paramgui.setBorder(new TitledBorder(new EtchedBorder(
                EtchedBorder.RAISED), "Parameters", TitledBorder.LEFT,
                TitledBorder.CENTER));
    ...
    protected JPanel getDca2Panel(Patch patch) {
        ...
        if (hackMode)
            addWidget(dca2Panel, new ScrollBarWidget("Key Follow", patch, 0,
                    0xFFFF, 0, new WordFieldModel(patch, CZModel.SAMD, 0xFFFF),
                    null), 0, 0, 7, 1, 0);
        else
            addWidget(dca2Panel, new ScrollBarWidget("Key Follow", patch, 0, 9,
                    0, new KeyFollowAModel(patch, CZModel.SAMD), null), 0, 0,
                    7, 1, 0);
    ...
    protected JPanel getDcw2Panel(Patch patch) {
        ...
        if (hackMode)
            addWidget(dcw2Panel, new ScrollBarWidget("Key Follow", patch, 0,
                    0xFFFF, 0, new WordFieldModel(patch, CZModel.SWMD, 0xFFFF),
                    null), 0, 0, 7, 1, 0);
        else
            addWidget(dcw2Panel, new ScrollBarWidget("Key Follow", patch, 0, 9,
                    0, new KeyFollowWModel(patch, CZModel.SWMD), null), 0, 0,
                    7, 1, 0);
    ...
    protected JPanel getDca1Panel(Patch patch) {
        ...
        if (hackMode)
            ...
            addWidget(
                    dca1Panel,
                    new ScrollBarWidget("Key Follow", patch, 0, 0xFF, 0,
                            new CZModel(patch, TONE_DATA + CZModel.MAMV), null),
                    0, 0, 7, 1, 0);
        else
            // 10 steps only
            addWidget(dca1Panel, new ScrollBarWidget("Key Follow", patch, 0, 9,
                    0, new KeyFollowAModel(patch, CZModel.MAMD), null), 0, 0,
                    7, 1, 0);
    ...
    protected JPanel getDcw1Panel(Patch patch) {
        ...
        if (hackMode)
            ...
            addWidget(
                    dcw1Panel,
                    new ScrollBarWidget("Key Follow", patch, 0, 0xFF, 0,
                            new CZModel(patch, TONE_DATA + CZModel.MWMV), null),
                    0, 0, 7, 1, 0);
        else
            // 10 steps only
            addWidget(dcw1Panel, new ScrollBarWidget("Key Follow", patch, 0, 9,
                    0, new KeyFollowWModel(patch, CZModel.MWMD), null), 0, 0,
                    7, 1, 0);
    ...
    public JComponent getParamGUI(Patch patch) {
        ...
        int row = -1;
        addWidget(miscPane, new ScrollBarLookupWidget("Line Select", patch, 0,
                3, 80, new LineModel(patch), null, new String[] {
                        "1", "2", "1+1'", "1+2'" }), 0, ++row, 7, 1, 0);

        if (hackMode) {
            addWidget(miscPane, new CheckBoxWidget("Ring", patch, new BitModel(
                    patch, CZModel.MFW + 2, 5), null), 0, ++row, 1, 1, 0);
            addWidget(miscPane, new CheckBoxWidget("Noise1", patch,
                    new BitModel(patch, CZModel.MFW + 2, 4), null), 1, row, 1,
                    1, 0);
            addWidget(miscPane, new CheckBoxWidget("Noise2", patch,
                    new BitModel(patch, CZModel.MFW + 2, 3), null), 2, row, 1,
                    1, 0);
            // undocumented bit, seems to mute first voice
            addWidget(miscPane, new CheckBoxWidget("Mute1", patch,
                    new BitModel(patch, CZModel.MFW + 2, 2), null), 3, row, 1,
                    1, 0);
        } else {
            addWidget(miscPane, new ScrollBarLookupWidget("Modulation", patch,
                    0, 3, 80, new ModModel(patch), null, new String[] {
                            "OFF", "Ring", "Noise" }), 0, ++row, 7, 1, 0);
        }

        addWidget(miscPane, getDetuneStepGUI(patch), 0, ++row, 7, 1, 0);
        addWidget(miscPane, new ScrollBarWidget("Detune Fine", patch, 0, 63, 0,
                80, new DetuneFineModel(patch), null), 0, ++row, 7, 1, 0);

        // Octave val==3 is same as 1 (+1 octave)
        addWidget(miscPane, new ScrollBarWidget("Octave", patch, -1, 1, 0, 80,
                new OctaveModel(patch), null), 0, ++row, 7, 1, 0);
        add(miscPane, getVibratoGUI(patch), 0, ++row, 7, 4);

        return miscPane;
    }

    public void add(JPanel parent, Component widget, int gridx, int gridy,
    ...
    protected SysexWidget getDetuneStepGUI(Patch patch) {
        ...
        return new ScrollBarLookupWidget("Detune Step", patch, -max, max, 80,
                new DetuneStepModel(patch), null, dsopts);
        // boring half-step number only:
        // return new ScrollBarWidget("Detune Step",patch,-max,max,0,80,new
        // DetuneStepModel(patch),null);
    }
    ...
    protected JPanel getVibratoGUI(Patch patch) {
        ...
        panel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED),
                "Vibrato", TitledBorder.LEFT, TitledBorder.CENTER));
        int row = 0, width = 40;
        /*
         * --disabled .. weird combos do nothing interesting.
         * addWidget(panel,new CheckBoxWidget("Triangle",patch,new
         * BitModel(patch,8,3),null),0,row,1,1,0); addWidget(panel,new
         * CheckBoxWidget("Saw Up",patch,new
         * BitModel(patch,8,2),null),1,row,1,1,0); addWidget(panel,new
         * CheckBoxWidget("Saw Down",patch,new
         * BitModel(patch,8,5),null),2,row,1,1,0); addWidget(panel,new
         * CheckBoxWidget("Square",patch,new
         * BitModel(patch,8,1),null),3,row,1,1,0);
         */
        addWidget(panel, new ScrollBarLookupWidget("Wave", patch, 0, 4, width,
                new VibratoWaveModel(patch), null, new String[] {
                        "OFF", "1 Tri", "2 Up", "3 Dn", "4 Pul" }), 0, row, 7,
                1, 0);

        if (hackMode) {
            // Rate is limited by the computed 16-bit value
            addWidget(panel, new ScrollBarWidget("Delay", patch, 0, 255, 0,
                    width, new VibratoDelayModel(patch), null), 0, ++row, 7, 1,
                    0);
            addWidget(panel, new ScrollBarWidget("Rate", patch, 0, 142, 0,
                    width, new VibratoRateModel(patch), null), 0, ++row, 7, 1,
                    0);
            addWidget(panel, new ScrollBarWidget("Depth", patch, 0, 255, 0,
                    width, new VibratoDepthModel(patch), null), 0, ++row, 7, 1,
                    0);
        } else {
            // Officially limited to 0-99.
            addWidget(panel, new ScrollBarWidget("Delay", patch, 0, 99, 0,
                    width, new VibratoDelayModel(patch), null), 0, ++row, 7, 1,
                    0);
            addWidget(panel, new ScrollBarWidget("Rate", patch, 0, 99, 0,
                    width, new VibratoRateModel(patch), null), 0, ++row, 7, 1,
                    0);
            addWidget(panel, new ScrollBarWidget("Depth", patch, 0, 99, 0,
                    width, new VibratoDepthModel(patch), null), 0, ++row, 7, 1,
                    0);
        }
        return panel;
    ...
    protected JPanel getWaveformGUI(Patch patch, int offset) {
        ...
        // TODO non-hack-mode == waveforms 0 1 2 4 5 6, but only one osc can use
        // resonance (6); res 0 1 2 3 only.
        JPanel panel = new JPanel();
        panel.setLayout(new GridBagLayout());
        int row = -1, width = 40;
        ...
        String[] wfLabels =
                new String[] {
                        "1 Saw", "2 Pul", "3 Tri", "Null", "4 TrSi", "5 SiPu",
                        "SinSy", "NPul"
                // "Sine Sync" and "Narrow Pulse"
                };
        String[] resLabels = new String[] {
                // "Window Functions"
                "OFF", "6 Saw", "7 Tri", "8 Trap", "SawPu", "Spike"
        // "Trapezoid", "Saw-Pulse", "Spike"
        // last two are also spike
                };
        // Labeled sliders
        addWidget(panel, new ScrollBarLookupWidget("Waveform 1", patch, 0,
                wfLabels.length - 1, 80, new WordFieldModel(patch, TONE_DATA
                        + offset, 0xE000), null, wfLabels), 0, ++row, 7, 1, 0);
        addWidget(panel, new ScrollBarLookupWidget("Waveform 2", patch, 0,
                wfLabels.length - 1, 80, new WordFieldModel(patch, TONE_DATA
                        + offset, 0x1C00), null, wfLabels), 0, ++row, 7, 1, 0);
        addWidget(panel, new CheckBoxWidget("Enable WF 2", patch, new BitModel(
                patch, offset, 1), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarLookupWidget("Window", patch, 0,
                resLabels.length - 1, 80, new WordFieldModel(patch, TONE_DATA
                        + offset, 0x01C0), null, resLabels), 0, ++row, 7, 1, 0);
        return panel;
    }
    ...
    protected JPanel getEnvelopeGUI1(Patch patch, int firstOffset) {
        ...
        panel.setLayout(new GridBagLayout());
        panel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED),
                "Envelope", TitledBorder.LEFT, TitledBorder.CENTER));
        ...
        if (hackMode)
            ...
            maxval = 99;
        addWidget(panel, new ScrollBarWidget("1 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(0), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(0), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(0),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("2 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(1), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(1), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(1),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("3 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(2), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(2), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(2),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("4 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(3), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(3), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(3),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("5 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(4), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(4), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(4),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("6 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(5), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(5), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(5),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("7 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(6), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(6), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(6),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("8 Rate", patch, 0, maxval, 0,
                width, model.getRateModel(7), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0,
                width, model.getLevelModel(7), null), 1, row, 1, 1, 0);
        addWidget(panel,
                new CheckBoxWidget("Sustain", patch, model.getSustainModel2(7),
                        null), 2, row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Final Stage", patch, 0, 7, 1,
                model.getEndStageModel(), null), 0, ++row, 1, 1, 0);
        return panel;
    }

    /**
     * "Normal mode": envelope control give access to params as seen on LCD.
     * This is an experimental panel that uses the EnvelopeWidget. Doesn't work
     * for some reason.
     * @param patch
     *            work area
     * @param firstOffset
    ...
    protected JPanel getEnvelopeGUI2(Patch patch, int firstOffset) {
        ...
        Node[] nodes =
                new Node[] {
                        ...
                        new Node(0, maxval,
                                model.getRateModel(0), 0, maxval,
                                model.getLevelModel(0), 0, false,
                                new NullSender(), new NullSender(), "1 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(1), 0, maxval,
                                model.getLevelModel(1), 0, false,
                                new NullSender(), new NullSender(), "2 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(2), 0, maxval,
                                model.getLevelModel(2), 0, false,
                                new NullSender(), new NullSender(), "3 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(3), 0, maxval,
                                model.getLevelModel(3), 0, false,
                                new NullSender(), new NullSender(), "4 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(4), 0, maxval,
                                model.getLevelModel(4), 0, false,
                                new NullSender(), new NullSender(), "5 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(5), 0, maxval,
                                model.getLevelModel(5), 0, false,
                                new NullSender(), new NullSender(), "6 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(6), 0, maxval,
                                model.getLevelModel(6), 0, false,
                                new NullSender(), new NullSender(), "7 Rate",
                                "   Level"),
                        new Node(0, maxval,
                                model.getRateModel(7), 0, maxval,
                                model.getLevelModel(7), 0, false,
                                new NullSender(), new NullSender(), "8 Rate",
                                "   Level"), };
        SysexWidget wid = new EnvelopeWidget("Envelope", patch, nodes);
        // wid.setBorder(new TitledBorder(new
        // EtchedBorder(EtchedBorder.RAISED),"Envelope",TitledBorder.LEFT,TitledBorder.CENTER));
        // wid.setMinimumSize(new Dimension(600,300));
        addWidget(panel, wid, 0, row, 1, 1, 0);
        // addWidget(panel, new ScrollBarWidget("Sustain Stage", patch, -1, 7,
        // 1, 70, model.getSustainModel(), null), 0, ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarLookupWidget("Sustain Stage", patch, -1,
                7, 70, model.getSustainModel(), null, new String[] {
                        "OFF", "1", "2", "3", "4", "5", "6", "7", "8" }), 0,
                ++row, 1, 1, 0);
        addWidget(panel, new ScrollBarWidget("Final Stage", patch, 0, 7, 1, 70,
                model.getEndStageModel(), null), 0, ++row, 1, 1, 0);
        return panel;
    }
    ...
    class WordFieldModel extends CZModel {
        ...
        public void set(int value) {
            setWordH(offset, (getWordH(offset) & ~mask) | ((value << shift) & mask));
        }

        public int get() {
            int value = (getWordH(offset) & mask) >> shift;
            return value;
        }
    }
    ...
    class LineModel extends CZModel {
        ...
        public void set(int i) {
            setByte(offset, (getByte(offset) & 0xFC) | (i & 3));
        }

        public int get() {
            return getByte(offset) & 3;
        }
    }
    ...
    class ModModel extends CZModel {
        ...
        public void set(int value) {
            value &= 0x3;
            setByte(offset, (getByte(offset) & ~MASK) | VALS[value]);
        }

        public int get() {
            int value = getByte(offset) & MASK;
            switch (value) {
            case 0x00:
                value = 0;
                break;
    ...
    class OctaveModel extends CZModel {
        ...
        public void set(int i) {
            if (i == -1)
                i = 2;
            setByte(offset, (getByte(offset) & 0xF3) | ((i << 2) & 0xC));
        }

        public int get() {
            int i = (getByte(offset) & 0x0C) >> 2;
            if (i == 2)
                i = -1;
            return i;
        }
    ...
    class DetuneStepModel extends CZModel {
        ...
        public void set(int value) {
            setByte(offset, (value < 0 ? 1 : 0));
            setByte(offset + 4, Math.abs(value) & 0x3F);
        }

        public int get() {
            int value = getByte(offset + 4);
            if ((getByte(offset) & 1) != 0)
                value = -value;
            return value;
        }
    }
    ...
    class DetuneFineModel extends CZModel {
        ...
        public void set(int value) {
            setByte(offset, value << 2);
        }

        public int get() {
            return getByte(offset) >> 2;
        }
    }

    class VibratoWaveModel extends CZModel {
        final int[] VALS = {
                0, 0x08, 0x04, 0x20, 0x02 };
        ...
        public void set(int value) {
            setByte(offset, VALS[value]);
        }

        public int get() {
            int value = getByte(offset);
    ...
    /**
     * Vibrato params have this weird multi-segment curve, and are stored in
     * three bytes: first byte is the raw value. Second and third are the low
     * and high bytes from the appropriate point on the curve.
     */
    ...
    class VibratoRateModel extends VibratoModel {
        ...
        int[] BIASES = {
                0, 0x460, 0x8E0, 0x11E0, 0x23E0, 0x47E0 };
    ...
    class VibratoDepthModel extends VibratoModel {
        int[] STAGES = {
                0, 32, 48, 64, 80, 96, 99, 999 };
        int[] MULTYS = {
                1, 2, 4, 8, 16, 32, 64 };
        int[] BIASES = {
                1, 0x23, 0x47, 0x8F, 0x11F, 0x23F, 0x300 };
    ...
    class KeyFollowAModel extends KeyFollowModel {
        int[] MAP = {
                0x00, 0x08, 0x11, 0x1A, 0x24, 0x2F, 0x3A, 0x45, 0x52, 0x5F };
    ...
    class KeyFollowWModel extends KeyFollowModel {
        int[] MAP = {
                0x00, 0x1F, 0x2C, 0x39, 0x46, 0x53, 0x60, 0x6E, 0x92, 0xFF };
    ...
    class EnvelopeModel extends CZModel {
         ...
         * @param patch
         * @param firstOffset
         *            Offset of 'end step' param within tone data. Envelope data
         *            is assumed to follow. Block is 17 bytes total.
         */
         ...
         *            0..7
         * @param value
         *            angle 0..127, where 127 = 90degrees = no delay.
         */
        public void setRate(int stage, int value) {
            if (stage < 0 || stage > 7)
                throw new IllegalArgumentException("stage=" + stage
                        + "; must be 0..7");
            int o = offset + 2 + 4 * stage;
            value &= 0x7F;
    ...
    class BitModel extends CZModel {
        ...
        public void set(int i) {
            if (i == 0)
                setByte(offset, getByte(offset) & neg);
            else
                setByte(offset, getByte(offset) | pos);
        }

        public int get() {
            return (getByte(offset) & pos) != 0 ? 1 : 0;
        }
    }
