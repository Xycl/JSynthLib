...
/**
 * Intercept the 263-byte messages sent by the synth, and convert them into
 * standard 264-byte patches.
 * @author Bill Zwicky
 * @version $Id: CasioCZ1000RcvConverter.java 1000 2005-03-20 05:04:23Z
 *          billzwicky $
 */
public class CasioCZ1000RcvConverter extends Converter {
    ...
    public Patch createPatch(byte[] sysex) {
        ...
        return getPatchFactory().createNewPatch(sysex2, this);
    }

    public Patch[] extractPatch(Patch p) {
        return new Patch[] {
            (Patch) createPatch(p.sysex) };
    }
}
