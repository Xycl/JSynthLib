/* Made by Yves Lefebvre
   ...
   @version $Id: NovationNova1SingleDriver.java 939 2005-03-03 04:05:40Z hayashi $
 */
...
public class NovationNova1SingleDriver extends AbstractPatchDriver {
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        byte[] newsysex = new byte[297]; // a dump and write format is one byte
                                         // longer than an edit buffer dump
        ...
        Patch patchtowrite = getPatchFactory().createNewPatch(newsysex, this);
    ...
    public void sendPatch(Patch p) {
        if (bShowMenu == true) {
            NovationNova1PatchSender nps =
                    new NovationNova1PatchSender(null, p, this);
            nps.setVisible(true);
        } else {
            sendPatchWorker(p);
        }
    ...
    class NovationNova1PatchSender extends JDialog {
        ...
        public NovationNova1PatchSender(JFrame Parent, Patch p,
            ...
            localPatch = getPatchFactory().createNewPatch(newsysex, driver);
        ...
        void ProgPressed() {
            ...
            if (deviceIDoffset > 0) {
                localPatch.sysex[deviceIDoffset] = (byte) (getChannel() - 1);
            }
            try {
                localPatch.send();
            } catch (Exception e) {
    ...
    class infoPannel extends JDialog {
        public infoPannel(JFrame Parent) {
            ...
            l1 =
                    new JLabel(
                            "This will default to send patch to prog buffer only. The Nova");
            l2 =
                    new JLabel(
                            "Patch sender panel will only appear next time you start JSynthLib");
