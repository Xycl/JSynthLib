/* Made by Yves Lefebvre
   ...
   @version $Id: NovationNova1BankDriver.java 744 2004-10-03 17:44:17Z hayashi $
 */
...
public class NovationNova1BankDriver extends AbstractBankDriver {
    final static SysexHandler bankARequestDump = new SysexHandler(
            "F0 00 20 29 01 21 @@ 12 05 F7 ");
    final static SysexHandler bankBRequestDump = new SysexHandler(
            "F0 00 20 29 01 21 @@ 12 06 F7 ");
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        // This method is called when doing a paste (from another bank or a
        // single)
        // the patch received will be a single dump (meant for the edit buffer)
        // we need to extract the actual patch info and paste it in the bank
        // itself
        ...
        System.arraycopy(p.sysex, 9, bank.sysex,
                getPatchStart(patchNum), 296 - 9);
        calculateChecksum(bank);
    }

    @Override
    public Patch getPatch(Patch bank, int patchNum) {
        // this method is call when you have a bank opened and want to send or
        // play individual patches
        // OR when you do a Cut/Copy
        // The method is call to retreive a single patch to send using the
        // default sendPatch()
        // Must remove the bank and patch number information and convert to a
        // single dump for edit buffer
        try {
    ...
    public BankPatch createNewPatch() {
        // On the Nova, Bank A or Bank B dump are just a collection of 128
        // writable single dump
        // ie : single dump meant to go to a specific memory location (see the
        // storePatch method
        // of the SingleDriver. There is no additionnal header on the Bank dump
        // itself.
        // In fact, we could probably create partial bank dump or partial bank
        // instead.
        // However, for now, creating a new Bank will have 128 entry just like
        // what the Nova
        ...
        for (int i = 0; i < 128; i++) {
            ...
            System.arraycopy(NovationNova1InitPatch.initpatch, 9, p.sysex,
                    (i * 297) + 10, 296 - 9);
    ...
    public void storePatch(Patch bank, int bankNum, int patchNum) {
        ...
        // The bank number (bank A or B) information is written in EACH writable
        // single dump
        // (a bank is just a group of individual single dump)
        // We must overwrite all the individual patch in the bank to set there
        // BankNumber to
        ...
        Patch p = getPatchFactory().createNewPatch(newsysex);
        try {
            for (int i = 0; i < 128; i++) {
                System.arraycopy(bank.sysex, 297 * i, p.sysex, 0, 297);
                sendPatchWorker(p);
                Thread.sleep(150); // Nova have problem receiving too fast,
                                   // NOTE : Do not modify this to send the bank
                                   // in one shot! It will be faster but some
                                   // patch will not be received correctly on
                                   // the Nova!
            }
        } catch (Exception e) {
