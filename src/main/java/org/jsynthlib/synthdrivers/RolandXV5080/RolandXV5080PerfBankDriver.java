...
// @version $Id: RolandXV5080PerfBankDriver.java 668 2004-08-16 03:14:10Z hayashi $
...
public class RolandXV5080PerfBankDriver extends AbstractBankDriver {
    ...
    public RolandXV5080PerfBankDriver() {
        super("PerfBank", "Phil Shepherd",
                RolandXV5080PerfDriver.PATCH_NUMBERS.length, 4);
    ...
    public String getPatchName(Patch p, int patchNum) {
        try {
            return new String(((Patch) p).sysex,
                    RolandXV5080PerfDriver.PATCH_SIZE * patchNum
                            + RolandXV5080PerfDriver.PATCH_NAME_START,
                    RolandXV5080PerfDriver.PATCH_NAME_SIZE, "US-ASCII");
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        try {
            ...
            System.arraycopy(((Patch) bank).sysex,
                    RolandXV5080PerfDriver.PATCH_SIZE * patchNum, sysex, 0,
                    RolandXV5080PerfDriver.PATCH_SIZE);
            return getPatchFactory().createNewPatch(sysex, getDevice());
        } catch (Exception ex) {
            ErrorMsg.reportError("Error", "Error in XV5080 Perf Bank Driver",
                    ex);
            return null;
        }
    }
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        RolandXV5080PerfDriver singleDriver =
                (RolandXV5080PerfDriver) pInsert.getDriver();
        ...
        ((Patch) bank).sysex =
                Utility.byteArrayReplace(((Patch) bank).sysex,
                        RolandXV5080PerfDriver.PATCH_SIZE * patchNum,
                        RolandXV5080PerfDriver.PATCH_SIZE, pInsert.sysex, 0,
                        RolandXV5080PerfDriver.PATCH_SIZE);
    }
