...
// @version $Id: RolandXV5080PatchDriver.java 662 2004-08-13 03:08:21Z hayashi $
...
public class RolandXV5080PatchDriver extends AbstractPatchDriver {
    final static int[] PATCH_SYSEX_START = new int[] {
            0, 91, 248, 312, 407, 460, 609, 758, 907 };
    final static int[] PATCH_SYSEX_SIZE = new int[] {
            91, 157, 64, 95, 53, 149, 149, 149, 149 };

    final static int PATCH_SIZE = 1056;

    final static int PATCH_NUMBER_OFFSET = 7; // (F0 41 10 00 10 12 30 **).size
                                              // - 1
    ...
    final static SysexHandler SYSEX_REQUEST_DUMP = new SysexHandler(
            "F0 41 10 00 10 11 30 *patchNum* 00 00 00 01 00 00 00 F7");
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        // ErrorMsg.reportStatus("RolandXV5080PatchDriver->storePatch: " +
        // bankNum + " | " + patchNum);
    ...
    // Notes:
    // Bn 00 57 Bn 20 00 Cn <patchNum>, where n is the XV-5080 patch receive
    // channel (Rx-Ch)
    // ----------------------------------------------------------------------------------------------------------------------
    ...
    public void calculateChecksum(Patch p) {
        for (int i = 0; i < PATCH_SYSEX_START.length; i++) {
            ...
            calculateXV5080Checksum(((Patch) p).sysex, checksumStart, checksumEnd,
                    checksumOffset);
        }
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        calculateXV5080Checksum(sysex, 6, sysex.length - 3, sysex.length - 2);
        send(sysex);
    }
    ...
    public static void calculateXV5080Checksum(byte[] sysex, int start, int end,
            int ofs) {
        int i;
        int sum = 0;
        for (i = start; i <= end; i++)
