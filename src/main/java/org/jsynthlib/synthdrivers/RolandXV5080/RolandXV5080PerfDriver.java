...
// @version $Id: RolandXV5080PerfDriver.java 662 2004-08-13 03:08:21Z hayashi $
...
public class RolandXV5080PerfDriver extends AbstractPatchDriver {
    final static int[] PATCH_SYSEX_START = new int[] {
            0, 65, 222, 286, 381, 405, 429, 453, 477, 501, 525, 549, 573, 597,
            621, 645, 669, 693, 717, 741, 765, 810, 855, 900, 945, 990, 1035,
            1080, 1125, 1170, 1215, 1260, 1305, 1350, 1395, 1440, 1485, 1530,
            1575, 1620, 1665, 1710, 1755, 1800, 1845, 1890, 1935, 1980, 2025,
            2070, 2115, 2160 };
    final static int[] PATCH_SYSEX_SIZE = new int[] {
            65, 157, 64, 95, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24,
            24, 24, 24, 24, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45,
            45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45, 45,
            45, 45 };

    final static int PATCH_SIZE = 2205;

    final static int PATCH_NUMBER_OFFSET = 7; // (F0 41 10 00 10 12 20 **).size
                                              // - 1

    final static int PATCH_NAME_START = 10; // (F0 41 10 00 10 12 20 ** 00
                                            // 00).size
    ...
    final static SysexHandler SYSEX_REQUEST_DUMP = new SysexHandler(
            "F0 41 10 00 10 11 20 *patchNum* 00 00 00 01 00 00 00 F7");
    ...
    public void updatePatchNum(Patch p, int patchNum) {
        for (int i = 0; i < PATCH_SYSEX_START.length; i++)
            p.sysex[PATCH_SYSEX_START[i] + PATCH_NUMBER_OFFSET] =
                    (byte) (patchNum);
    }
    ...
    public void calculateChecksum(Patch p) {
        for (int i = 0; i < PATCH_SYSEX_START.length; i++) {
            ...
            RolandXV5080PatchDriver.calculateXV5080Checksum(((Patch) p).sysex,
                    checksumStart, checksumEnd, checksumOffset);
        }
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        RolandXV5080PatchDriver.calculateXV5080Checksum(sysex, 6, sysex.length - 3,
                sysex.length - 2);
        send(sysex);
    }
}
