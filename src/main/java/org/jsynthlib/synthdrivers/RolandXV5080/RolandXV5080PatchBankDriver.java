...
// @version $Id: RolandXV5080PatchBankDriver.java 668 2004-08-16 03:14:10Z hayashi $
...
public class RolandXV5080PatchBankDriver extends AbstractBankDriver {
    final static SysexHandler SYSEX_REQUEST_DUMP = new SysexHandler(
            "F0 41 10 00 10 11 30 *patchNum* 00 00 01 00 00 00 00 F7");
    ...
    public RolandXV5080PatchBankDriver() {
        super("PatchBank", "Phil Shepherd",
                RolandXV5080PatchDriver.PATCH_NUMBERS.length, 4);
    ...
    public String getPatchName(Patch p, int patchNum) {
        try {
            return new String(((Patch) p).sysex,
                    RolandXV5080PatchDriver.PATCH_SIZE * patchNum
                            + RolandXV5080PatchDriver.PATCH_NAME_START,
                    RolandXV5080PatchDriver.PATCH_NAME_SIZE, "US-ASCII");
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        try {
            ...
            System.arraycopy(((Patch) bank).sysex,
                    RolandXV5080PatchDriver.PATCH_SIZE * patchNum, sysex, 0,
                    RolandXV5080PatchDriver.PATCH_SIZE);
            return getPatchFactory().createNewPatch(sysex, getDevice());
        } catch (Exception ex) {
            ErrorMsg.reportError("Error", "Error in XV5080 Patch Bank Driver",
                    ex);
            return null;
        }
    }
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        Patch pInsert = getPatchFactory().createNewPatch(((Patch) p).sysex);
        RolandXV5080PatchDriver singleDriver =
                (RolandXV5080PatchDriver) pInsert.getDriver();
        ...
        ((Patch) bank).sysex =
                Utility.byteArrayReplace(((Patch) bank).sysex,
                        RolandXV5080PatchDriver.PATCH_SIZE * patchNum,
                        RolandXV5080PatchDriver.PATCH_SIZE, pInsert.sysex, 0,
                        RolandXV5080PatchDriver.PATCH_SIZE);
    }
