...
public class YamahaUB99BankDumpConverter extends Converter {
    ...
    public YamahaUB99BankDumpConverter(YamahaUB99Driver singleDriver) {
        super("Yamaha UB99 Bankdump Converter",
                "Ton Holsink <a.j.m.holsink@chello.nl>");
    ...
    public boolean supportsPatch(String patchString, byte[] sysex) {
        ...
        boolean ok =
                compareString.toString().equalsIgnoreCase(
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        for (int i = 0; i < sm.length; i++) {
            ...
            if ((msg[9] == (byte) 0x00) && (msg[10] == (byte) 0x30)
                    && (msg[11] == (byte) 0x01) && (nr == -1)) {
                nr = msg[12];
            }

            // Part 1 of patch data
            if ((msg[9] == (byte) 0x20) && (msg[10] == (byte) 0x20)
                    && (nr > -1)) {
                System.arraycopy(msg, 13, sysex,
                        YamahaUB99Const.BANK_PATCH_OFFSET + nr
                                * YamahaUB99Const.SINGLE_SIZE, 32);
                System.arraycopy(msg, 29, sysex,
                        YamahaUB99Const.BANK_NAME_OFFSET + nr
                                * YamahaUB99Const.NAME_SIZE,
                        YamahaUB99Const.NAME_SIZE);
            }

            // Part 2 of patch data
            if ((msg[9] == (byte) 0x7F) && (msg[10] == (byte) 0x20)
                    && (nr > -1)) {
                System.arraycopy(msg, 13, sysex,
                        YamahaUB99Const.BANK_PATCH_OFFSET + nr
                                * YamahaUB99Const.SINGLE_SIZE + 32, 127);
            }

            // Signals end of patch data
            if ((msg[9] == (byte) 0x00) && (msg[10] == (byte) 0x30)
                    && (msg[11] == (byte) 0x11) && (msg[12] == nr)) {
                nr = -1;
            }
        ...
        newPatchArray[0] =
                getPatchFactory().createNewPatch(sysex, new YamahaUB99BankDriver(singleDriver));
