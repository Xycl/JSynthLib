...
/**
 * Formant sequence driver for FS1R. Size of sysex depends of frame format which
 * can be 128, 256, 384 or 512 bytes long. It gives sizes from 6443 to 25643.
 * @author Denis Queffeulou mailto:dqueffeulou@free.fr
 * @version $Id: YamahaFS1RFseqDriver.java 662 2004-08-13 03:08:21Z hayashi $
 */
public class YamahaFS1RFseqDriver extends AbstractPatchDriver {
    ...
    public void setBankNum(int bankNum) {
        ...
        sysexRequestDump =
                new SysexHandler("F0 43 20 5E 6" + bankNum
                        + " 00 *patchNum* F7");
    }
    ...
    public Patch createNewPatch() {/*
                                    * byte[] sysex = new
                                    * byte[PATCH_AND_HEADER_SIZE]; sysex[0] =
                                    * (byte) 0xF0; sysex[1] = (byte) 0x0F;
                                    * sysex[2] = (byte) 0x05; sysex[3] = (byte)
                                    * 0x00; sysex[4] = (byte) 0x00; sysex[5] =
                                    * (byte) 0x0B;
                                    * sysex[PATCH_AND_HEADER_SIZE-1] = (byte)
                                    * 0xF7; Patch p = getPatchFactory().createNewPatch(sysex);
                                    * p.ChooseDriver();
    ...
    static class DoubleModel extends Model {
        ...
        public int get() {
            int oVal = patch.sysex[offset + 1] + (patch.sysex[offset] << 7);
            return oVal;
        }
    }
