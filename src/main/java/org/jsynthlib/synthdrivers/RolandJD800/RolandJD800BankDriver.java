...
/**
 * Patch Memory Area (bank) driver for Roland JD800. JD800 operates on memory
 * areas instead of a patch concept. The entire area size is 24576 bytes. The
 * maximum amount of data in one sysex message in 256 bytes so it needs to
 * create 96 sysex messages to send a bank. To simplify its operation the driver
 * needs to concatenate the sysex messages into a one single message.
 */
public class RolandJD800BankDriver extends AbstractBankDriver {
    ...
    private static final SysexHandler SYS_REQ = new SysexHandler(
            "F0 41 @@ 3D 11 05 00 00 01 40 00 00 F7");

    public RolandJD800BankDriver() {
        super("Patch (Bank)Memory Area", "Robert Wirski",
                JD800.nrOfPatchesInABank, 4);
        ...
        System.arraycopy(generateNumbers(1, 8, "Patch I-10"), 0,
                patchNumbers, 0, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-20"), 0,
                patchNumbers, 8, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-30"), 0,
                patchNumbers, 16, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-40"), 0,
                patchNumbers, 24, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-50"), 0,
                patchNumbers, 32, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-60"), 0,
                patchNumbers, 40, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-70"), 0,
                patchNumbers, 48, 8);
        System.arraycopy(generateNumbers(1, 8, "Patch I-80"), 0,
                patchNumbers, 56, 8);
    ...
    /**
     * Gets a single patch at the given number <code>patchNum</code> from a
     * bank.
     */
    @Override
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(p.sysex, JD800.SizeOfSyxHeader, bank.sysex,
                getPatchStart(patchNum), JD800.SizeOfSinglePatch);
    }

    /**
     * Compares the header & size of a Patch to this driver to see if this
     * driver is the correct one to support the patch. It is overrided as it
     * needs to serve for both sysex message sizes: original 96 sysex messages
    ...
    public boolean supportsPatch(String patchString, byte[] sysex) {
        ...
        if (((JD800.SizeOfPatchSyx1 * JD800.nrOfSyxForABank) != sysex.length)
                & ((JD800.SizeOfSyxHeader + JD800.SizeOfSinglePatch
                        * JD800.nrOfPatchesInABank) != sysex.length)) {
            return false;
        }
    ...
    public IPatch createPatch(byte[] sysex) {
        ...
        for (int i = 0; i < JD800.nrOfSyxForABank; i++) {
            System.arraycopy(sysex, JD800.SizeOfSyxHeader + i
                    * JD800.SizeOfPatchSyx1, out, JD800.SizeOfSyxHeader + i
                    * JD800.MaxSyxDataBlock, JD800.MaxSyxDataBlock);
        }
        return getPatchFactory().createNewPatch(out, this);
    };
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        byte[] sysex =
                new byte[JD800.SizeOfSyxHeader + JD800.SizeOfSinglePatch];
        sysex[0] = (byte) 0xF0;
        sysex[1] = (byte) 0x41;
    ...
    public BankPatch createNewPatch() {
        byte[] sysex =
                new byte[JD800.SizeOfSyxHeader + JD800.SizeOfSinglePatch
                        * JD800.nrOfPatchesInABank];
        byte[] buffer = new byte[JD800.SizeOfPatchSyx1 + JD800.SizeOfPatchSyx2];
        try {
            InputStream fileIn =
                    getClass().getResourceAsStream("JD800_InitialPatch.syx");
            fileIn.read(buffer);
            fileIn.close();
        } catch (Exception e) {
            ErrorMsg.reportError("Error",
                    "Unable to find JD800_InitialPatch.syx", e);
            return null;
        }
        sysex[0] = (byte) 0xF0;
        sysex[1] = (byte) 0x41;
        ...
        for (int i = 0; i < JD800.nrOfPatchesInABank; i++) {
            System.arraycopy(buffer, JD800.SizeOfSyxHeader, sysex,
                    JD800.SizeOfSyxHeader + i * JD800.SizeOfSinglePatch,
                    JD800.MaxSyxDataBlock);
            System.arraycopy(buffer, JD800.SizeOfSyxHeader
                    + JD800.SizeOfPatchSyx1, sysex, JD800.SizeOfSyxHeader
                    + JD800.MaxSyxDataBlock + i * JD800.SizeOfSinglePatch,
    ...
    /**
     * Stores the bank to JD800. Ignores the parameters. It splits the data into
     * 96 sysex messages.
     */
    @Override
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        for (int i = 0; i < JD800.nrOfSyxForABank; i++) {
            ...
            s[6] = (byte) Addr;
            s[7] = (byte) 0x00;
            System.arraycopy(p.sysex, JD800.SizeOfSyxHeader + i
                    * JD800.MaxSyxDataBlock, s, JD800.SizeOfSyxHeader,
                    JD800.MaxSyxDataBlock);
            calculateChecksum(s, JD800.checksumStartSyx1,
                    JD800.checksumEndSyx1, JD800.checksumOffsetSyx1);
            sendPatchWorker(getPatchFactory().createNewPatch(s, this));
            try {
                Thread.sleep(25);
            } catch (Exception e) {
            }
