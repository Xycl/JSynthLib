 ...
 * of a patch concept. The maximum size of data send in one sysex message is 256
 * bytes. The single patch size is 384 bytes so it needs 2 sysex messages. The
 * author decided to join both sysex messages into one including the header of
 * the the first one at the beginning.
 */
public class RolandJD800SinglePatchDriver extends AbstractPatchDriver {
    ...
    private static final SysexHandler SYS_REQ = new SysexHandler(
            "F0 41 @@ 3D 11 *AddrMSB* *Addr* 00 00 03 00 00 F7");
    /** The MSB byte of the 3-byte patch address */
    final static int AddrMSB[] = {
            0x00, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05,
    ...
    final static int Addr[] = {
            0x00, 0x00, 0x03, 0x06, 0x09, 0x0C, 0x0F, 0x12, 0x15, 0x18, 0x1B,
            0x1E, 0x21, 0x24, 0x27, 0x2A, 0x2D, 0x30, 0x33, 0x36, 0x39, 0x3C,
            0x3F, 0x42, 0x45, 0x48, 0x4B, 0x4E, 0x51, 0x54, 0x57, 0x5A, 0x5D,
            0x60, 0x63, 0x66, 0x69, 0x6C, 0x6F, 0x72, 0x75, 0x78, 0x7B, 0x7E,
            0x01, 0x04, 0x07, 0x0A, 0x0D, 0x10, 0x13, 0x16, 0x19, 0x1C, 0x1F,
            0x22, 0x25, 0x28, 0x2B, 0x2E, 0x31, 0x34, 0x37, 0x3A, 0x3D };
    ...
    public RolandJD800SinglePatchDriver() {
        ...
        System.arraycopy(new String[] {
            "Patch Temporary" }, 0, patchNumbers, 0, 1);
     ...
     * needs to serve for both sysex message sizes: original two sysex messages
    ...
    /**
     * Sends a patch to a set location on a synth. The method creates two sysex
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        calculateChecksum(s1, JD800.checksumStartSyx1,
                JD800.checksumEndSyx1, JD800.checksumOffsetSyx1);
        s1[265] = (byte) 0xF7;
        sendPatchWorker(getPatchFactory().createNewPatch(s1, this));
        try {
            Thread.sleep(25);
        } catch (Exception e) {
        }
        ...
        System.arraycopy(p.sysex,
                ...
                JD800.SizeOfSyxHeader, JD800.SizeOfSinglePatch
                        - JD800.MaxSyxDataBlock);
        calculateChecksum(s2, JD800.checksumStartSyx2,
                JD800.checksumEndSyx2, JD800.checksumOffsetSyx2);
        s2[137] = (byte) 0xF7;
        sendPatchWorker(getPatchFactory().createNewPatch(s2, this));
    };
    ...
    /**
     * Creates a new initial patch. The patch is read from the
     * JD800_InitialPatch.syx file which is a dump received from the JD800 using
    ...
    public Patch createNewPatch() {
        ...
        try {
            fileIn.read(buffer);
            fileIn.close();
            System.arraycopy(buffer, 0, out, 0, JD800.MaxSyxDataBlock
                    + JD800.SizeOfSyxHeader);
            System.arraycopy(buffer, JD800.SizeOfPatchSyx1
                    + JD800.SizeOfSyxHeader, out, JD800.SizeOfSyxHeader
                    + JD800.MaxSyxDataBlock, JD800.SizeOfSinglePatch
                    - JD800.MaxSyxDataBlock);
            return getPatchFactory().createNewPatch(out, this);
        } catch (Exception e) {
