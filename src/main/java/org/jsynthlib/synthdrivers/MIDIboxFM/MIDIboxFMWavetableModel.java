 ...
 * @version $Id: MIDIboxFMWavetableModel.java 820 2005-01-13 21:35:08Z tklose $
 *
...
class MIDIboxFMWavetableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int col) {
        if (col == 0)
            return convertByteToString((byte) (0x80 + row));
        if (col == 1)
            return modeNames[dump[4 * row + 0]];
        if (col == 2)
            return convertByteToString(dump[4 * row + 1]);
        if (col == 3)
            return convertByteToString(dump[4 * row + 2]);
        if (col == 4)
            return convertByteToString(dump[4 * row + 3]);
        return "Invalid";
    }
