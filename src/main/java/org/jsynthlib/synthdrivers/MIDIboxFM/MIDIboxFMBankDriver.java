 ...
 * @version $Id: MIDIboxFMBankDriver.java 1035 2006-01-28 13:26:37Z tklose $
 *
...
public class MIDIboxFMBankDriver extends AbstractBankDriver {
    private final transient Logger log = Logger.getLogger(getClass());
    private byte sysex_type;
    private int num_patches;

    public MIDIboxFMBankDriver(String bank_name, int _num_patches,
            byte _sysex_type) {
        ...
        sysexRequestDump =
                new SysexHandler("F0 00 00 7E 49 @@ 03 " + type_digit1
                        + type_digit0 + " *bankNum* F7");

        bankNumbers = new String[] {
                "A", "B", "C", "D", "E", "F", "G", "H" };
        patchNumbers = new String[num_patches];
        System.arraycopy(generateNumbers(1, num_patches, "000"), 0,
    ...
    public String getPatchName(Patch p, int patchNum) {
        if (sysex_type < 0x10) {
            ...
            try {
                StringBuffer s =
                        new StringBuffer(new String(((Patch) p).sysex,
                                nameStart, 16, "US-ASCII"));
                return s.toString();
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(((Patch) p).sysex, 10, ((Patch) bank).sysex,
