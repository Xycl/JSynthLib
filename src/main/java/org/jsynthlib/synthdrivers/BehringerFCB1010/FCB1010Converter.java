 ...
 * patch. When responding to a sysex data dump request, the FCB1010 sometimes
 ...
 * valid sysex message message sent is always the one we are interested in but
 * it can be preceded or succeeded with other garbage data. This converter will
 * attempt to parse out the last FCB patch in the block and discard the rest of
 * the data.
 * @author Jeff Weber
 */
class FCB1010Converter extends Converter {
    ...
    public boolean supportsPatch(String patchString, byte[] sysex) {
        if (sysex.length == Constants.HDR_SIZE + Constants.FCB1010_NATIVE_SIZE
                + 1) {
     ...
     * sysex record against the defined header for the FCB1010 and returns true
     * if there is a match.
     */
    private boolean thisSupportsPatch(byte[] sysex) {
        if (isFCBPatch(sysex, 0)) {
            if (sysex.length == Constants.HDR_SIZE
                    + Constants.FCB1010_NATIVE_SIZE + 1) {
                return true;
            } else {
                return false;
            }
    ...
    /**
     * Extracts a patch from a patch dump response (block of data). Calls
     * parseSysex to do the extraction.
     */
    public Patch[] extractPatch(Patch p) {
        byte[] sysex = parseSysex(p.getByteArray());

        Patch[] newPatchArray = new Patch[1];
        newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new FCB1010Driver());
        return newPatchArray;
    }

    /**
     * Attempts to parse an FCB patch from an FCB response to a dump request. If
     * parseSysex cannot find a valid FCB patch within the block of data, it
    ...
    private byte[] parseSysex(byte[] sysex) {
            ...
            if (sysex[i] == Constants.FCB1010_DUMP_HDR_BYTES[0]) {
                if ((i + 4) < sysex.length) {
                    if (isFCBPatch(sysex, i)) {
                        if (sysex[i + Constants.HDR_SIZE
                                + Constants.FCB1010_NATIVE_SIZE] == (byte) 0xF7) {
                            patchLength =
                                    Constants.HDR_SIZE
                                            + Constants.FCB1010_NATIVE_SIZE + 1;
                        }
                        ...
                        if ((i + patchLength - 1) < sysex.length) {
                            if (sysex[i + patchLength - 1] == (byte) 0xF7) {
                                System.arraycopy(sysex, i, FCBSysex, 0,
                                        (patchLength));
                                break;
                            }
                        }
        ...
        if ((FCBSysex[0] == (byte) 0xF0)
                && (FCBSysex[patchLength - 1] == (byte) 0xF7)) {
