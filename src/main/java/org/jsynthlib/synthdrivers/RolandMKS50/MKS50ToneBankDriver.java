...
// @version $Id: MKS50ToneBankDriver.java 891 2005-02-06 19:28:41Z hayashi $
...
public class MKS50ToneBankDriver extends AbstractBankDriver {
     ...
     * "Store Tone Bank", JOptionPane.PLAIN_MESSAGE ); storePatch(p, 0, 0); }
     */
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        // DCO AFTER DEPTH (sysex docs say 0-127 value, but real range is 0-15
        // and it's stored shifted)
        ...
        // VCF KEY FOLLOW (sysex docs say 0-127 value, but real range is 0-15
        // and it's stored shifted)
        bankSysex[0] |= (byte) ((((Patch) p).sysex[27] & 0x78) >> 3);
        // VCF AFTER DEPTH (sysex docs say 0-127 value, but real range is 0-15
        // and it's stored shifted)
        bankSysex[1] |= (byte) ((((Patch) p).sysex[28] & 0x78) << 1);
        // VCA LEVEL
        bankSysex[11] |= ((Patch) p).sysex[29];
        // VCA AFTER DEPTH (sysex docs say 0-127 value, but real range is 0-15
        // and it's stored shifted)
        ...
        // ENV KEY FOLLOW (sysex docs say 0-127 value, but real range is 0-15
        // and it's stored shifted)
        ...
        System.arraycopy(bankSysexNibbles, 0, ((Patch) bank).sysex,
                patchOffset, 64);
    }
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        ...
        sysex[53] = (byte) 0xF7;
        int patchOffset = getPatchStart(patchNum);
        System.arraycopy(((Patch) bank).sysex, patchOffset, bankSysexNibbles,
                0, 64);

        // convert bank tone (31 bytes, lo/hi nibble) to single tone (46 bytes)
        for (int i = 0; i < 32; i++) {
            bankSysex[i] =
                    (byte) (bankSysexNibbles[i * 2] | bankSysexNibbles[i * 2 + 1] << 4);
        }
        // DCO ENV MODE
        sysex[7] =
                (byte) ((bankSysex[5] & 0x80) >> 6 | (bankSysex[6] & 0x80) >> 7);
        // VCF ENV MODE
        sysex[8] =
                (byte) ((bankSysex[7] & 0x80) >> 6 | (bankSysex[8] & 0x80) >> 7);
        // VCA ENV MODE
        sysex[9] =
                (byte) ((bankSysex[9] & 0x80) >> 6 | (bankSysex[10] & 0x80) >> 7);
        // DCO WAVEFORM PULSE
        sysex[10] =
                (byte) ((bankSysex[17] & 0x80) >> 6 | (bankSysex[18] & 0x80) >> 7);
        // DCO WAVEFORM SAWTOOTH
        sysex[11] =
                (byte) ((bankSysex[14] & 0x80) >> 5
                        | (bankSysex[15] & 0x80) >> 6 | (bankSysex[16] & 0x80) >> 7);
        // DCO WAVEFORM SUB
        sysex[12] =
                (byte) ((bankSysex[11] & 0x80) >> 5
                        | (bankSysex[12] & 0x80) >> 6 | (bankSysex[13] & 0x80) >> 7);
        // DCO RANGE
        sysex[13] =
                (byte) ((bankSysex[21] & 0x80) >> 6 | (bankSysex[22] & 0x80) >> 7);
        // DCO SUB LEVEL
        sysex[14] =
                (byte) ((bankSysex[23] & 0x80) >> 6 | (bankSysex[24] & 0x80) >> 7);
        // DCO NOISE LEVEL
        sysex[15] =
                (byte) ((bankSysex[25] & 0x80) >> 6 | (bankSysex[26] & 0x80) >> 7);
        // HPF CUTOFF FREQ
        sysex[16] =
                (byte) ((bankSysex[19] & 0x80) >> 6 | (bankSysex[20] & 0x80) >> 7);
        // CHORUS
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        int patchOffset = getPatchStart(patchNum);
        System.arraycopy(((Patch) p).sysex, patchOffset, bankSysexNibbles, 0,
                64);
        for (int i = 0; i < 32; i++) {
            bankSysex[i] =
                    (byte) (bankSysexNibbles[i * 2] | bankSysexNibbles[i * 2 + 1] << 4);
        ...
        for (int i = 0; i < 10; i++) {
            patchName[i] =
                    MKS50ToneSingleDriver.nameChars[(byte) (bankSysex[i + 21] & 0x3F)];
        }
    ...
    // {
    // byte []tmp = new byte[266]; // send in 16 messages containing 4 tones
    // each
    // try
    // {
    // for (int i = 0; i < 16; i++)
