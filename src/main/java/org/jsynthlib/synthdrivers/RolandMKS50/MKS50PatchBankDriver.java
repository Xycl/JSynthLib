...
// @version $Id: MKS50PatchBankDriver.java 891 2005-02-06 19:28:41Z hayashi $
...
public class MKS50PatchBankDriver extends AbstractBankDriver {
     ...
     * "Store Patch Bank", JOptionPane.PLAIN_MESSAGE ); storePatch(p, 0, 0); }
     */
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        // KEY RANGE LOW
        bankSysex[1] |= (byte) (((Patch) p).sysex[8] + 4); // sysex docs didn't
                                                           // show, but needed
                                                           // to get correct
                                                           // value
        // KEY RANGE HIGH
        bankSysex[2] |= (byte) (((Patch) p).sysex[9] + 4); // sysex docs didn't
                                                           // show, but needed
                                                           // to get correct
                                                           // value
        // PORTAMENTO TIME
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        ...
        // convert bank patch (31 bytes, lo/hi nibble) to single patch (46
        // bytes)
        for (int i = 0; i < 32; i++) {
            bankSysex[i] =
        ...
        sysex[7] = bankSysex[0];
        // KEY RANGE LOW
        sysex[8] = (byte) (bankSysex[1] - 4); // sysex docs didn't show, but
                                              // needed to get correct value
        // KEY RANGE HIGH
        sysex[9] = (byte) (bankSysex[2] - 4); // sysex docs didn't show, but
                                              // needed to get correct value
        // PORTAMENTO TIME
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        for (int i = 0; i < 10; i++) {
            patchName[i] =
                    MKS50ToneSingleDriver.nameChars[(byte) (bankSysex[i + 11] & 0x3F)];
        }
