...
/**
 * Convert a bulk patch into an array of single bank, multi bank, drumset, and
 * effect bank patches.
 * @version $Id: KawaiK4BulkConverter.java 678 2004-08-21 17:08:55Z hayashi $
 */
public class KawaiK4BulkConverter extends Converter {
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        System.arraycopy(sysex, HSIZE + SSIZE, mx, HSIZE, MSIZE);
        // Copy the data into the drumset
        System.arraycopy(sysex, 0, dx, 0, HSIZE);
        System.arraycopy(sysex, HSIZE + SSIZE + MSIZE, dx, HSIZE, DSIZE);
        // Copy the data into the Effect Bank
        System.arraycopy(sysex, 0, ex, 0, HSIZE);
        System.arraycopy(sysex, HSIZE + SSIZE + MSIZE + DSIZE, ex, HSIZE, ESIZE);
        ...
        pf[0] = getPatchFactory().createNewPatch(sx);
        pf[1] = getPatchFactory().createNewPatch(mx);
        pf[2] = getPatchFactory().createNewPatch(ex);
        pf[3] = getPatchFactory().createNewPatch(dx);
