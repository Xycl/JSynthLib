 ...
 * @version $Id: QuasimidiQuasarBankDriver.java 878 2005-02-02 06:38:06Z
 *          jbackhaus $
 */
public class QuasimidiQuasarBankDriver extends AbstractBankDriver {
    private final QuasimidiQuasarSingleDriver singleDriver;

    public QuasimidiQuasarBankDriver(QuasimidiQuasarSingleDriver singleDriver) {
        super("Performance Bank", "Joachim Backhaus",
                QuasarConstants.PATCH_NUMBERS.length, 5);
    ...
    private final void doSendPatch(Patch p) {
        if (deviceIDoffset > 0) {
            ...
            for (int patchNo = 0; patchNo < patchNumbers.length; patchNo++) {
                ...
                p.sysex[temporaryOffset + QuasarConstants.ARRAY_PART_1_OFFSET] =
                        (byte) deviceID;
                p.sysex[temporaryOffset + QuasarConstants.ARRAY_PART_2_OFFSET] =
                        (byte) deviceID;
                p.sysex[temporaryOffset + QuasarConstants.ARRAY_PART_3_OFFSET] =
                        (byte) deviceID;
                p.sysex[temporaryOffset + QuasarConstants.ARRAY_PART_4_OFFSET] =
                        (byte) deviceID;
                p.sysex[temporaryOffset + QuasarConstants.ARRAY_NAME_OFFSET] =
                        (byte) deviceID;
            }
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(p.sysex, QuasarConstants.SYSEX_HEADER_OFFSET,
                bank.sysex, getPatchStart(patchNum)
                        + QuasarConstants.SYSEX_HEADER_OFFSET, this.singleSize
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        nameStart += QuasarConstants.PATCH_NAME_START; // offset of name in
                                                       // patch data
        try {
            StringBuffer s =
                    new StringBuffer(new String(p.sysex, nameStart,
                            QuasarConstants.PATCH_NAME_SIZE, "US-ASCII"));
            return s.toString();
    ...
    public void setPatchName(Patch p, int patchNum, String name) {
        int tempPatchNameStart =
                QuasarConstants.PATCH_NAME_START + this.getPatchStart(patchNum);
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        if (sysexRequestDump == null) {
            ...
            // Request dumps for all 100 RAM peformances (that are 5 * 100
            // requests!!!)
            for (int patchNo = 0; patchNo < patchNumbers.length; patchNo++) {
                for (int count = 0; count < QuasarConstants.SYSEX_PERFORMANCE_REQUEST.length; count++) {
                    ...
                    try {
                        // Wait a little bit so that everything is in the
                        // correct sequence
                        Thread.sleep(50);
                    } catch (Exception ex) {
                        ErrorMsg.reportError(
                                "Error",
                                "Error requesting all Quasar RAM performances.",
                                ex);
                    }
                }
            }
    ...
    public BankPatch createNewPatch() {
        ...
        for (int patchNo = 0; patchNo < patchNumbers.length; patchNo++) {
            tempPatch =
                    singleDriver.createNewPatch(patchNo,
                            QuasarConstants.SYSEX_PERFORMANCE_OFFSET);

            System.arraycopy(tempPatch.sysex, 0, sysex, (patchNo * singleSize),
                    singleSize);
        }

        return getPatchFactory().newBankPatch(sysex);
    }
}
