...
public class MultiWidget extends SysexWidget {
    ...
    /**
     * @param params
     *            The parameters for the MultiWidget
     * @param patch
     *            The patch data
     * @param pmodel
     *            Model
    ...
    protected void eventListener(ItemEvent e) {
        if (e.getStateChange() == ItemEvent.ITEM_STATE_CHANGED) {
            ...
            for (int count = 0; count < multiWidgetParams.length; count++) {
                iTemp =
                        (int) Math.pow(2,
                                multiWidgetParams[count].getStartBit());
                ...
                if (multiWidgetParams[count].isCheckBox()) {
                    ...
                    iValue +=
                            iTemp
                                    * comboBoxes[multiWidgetParams[count]
    ...
    public void setValue(int iValue) {
        ...
        for (int count = 0; count < checkBoxes.length; count++) {
            checkBoxes[count].setSelected(this.multiWidgetParams[count]
                    .getRealValue(getValue()) != 0);
            checkBoxes[count].doClick();
        }
        for (int count = 0; count < comboBoxes.length; count++) {
            comboBoxes[count].setSelectedIndex(this.multiWidgetParams[count]
                    .getRealValue(getValue()));
        }
    }
}