 ...
 * @version $Id: QuasimidiQuasarSingleDriver.java 844 2005-01-24 07:29:48Z
 *          jbackhaus $
 */
public class QuasimidiQuasarSingleDriver extends AbstractPatchDriver {

    /*
     * The Quasar sends only all 100 RAM performances (22300 Bytes) or all
     * temporary parameters and the system parameters (644 Bytes) The pure
     * performance needs only 223 Bytes
     */
    public QuasimidiQuasarSingleDriver() {
        ...
        // This one is never really used, just a dummy to prevent the
        // "this synth doesn't support patch request" dialog
        this.sysexRequestDump =
                new SysexHandler(QuasarConstants.SYSEX_PERFORMANCE_REQUEST[0]);
        ...
        // "Temporary" is a fake bank! Otherwise the selection of patch numbers
        // doesn't work
        ...
        // Patch size is variable (644 Bytes for manual dump from device, 223
        // Bytes for trimmed dump).
    ...
    /**
     * Replacement for sendPatchWorker which doesn't work here as the Device ID
     * has to be set in each one of the 6 SysEx sub parts
     * @param p
    ...
    private final void doSendPatch(Patch p) {
        ...
        if (deviceIDoffset > 0) {
            ...
            p.sysex[deviceIDoffset + QuasarConstants.ARRAY_PART_1_OFFSET] =
                    (byte) deviceID;
            p.sysex[deviceIDoffset + QuasarConstants.ARRAY_PART_2_OFFSET] =
                    (byte) deviceID;
            p.sysex[deviceIDoffset + QuasarConstants.ARRAY_PART_3_OFFSET] =
                    (byte) deviceID;
            p.sysex[deviceIDoffset + QuasarConstants.ARRAY_PART_4_OFFSET] =
                    (byte) deviceID;
            p.sysex[deviceIDoffset + QuasarConstants.ARRAY_NAME_OFFSET] =
                    (byte) deviceID;
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        int performanceOffset =
                patchNum + QuasarConstants.SYSEX_PERFORMANCE_OFFSET;

        // Set the performance number
        p.sysex[QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;

        // Part 1
        p.sysex[QuasarConstants.ARRAY_PART_1_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_1_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x01;
        // Part 2
        p.sysex[QuasarConstants.ARRAY_PART_2_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_2_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x02;
        // Part 3
        p.sysex[QuasarConstants.ARRAY_PART_3_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_3_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x03;
        // Part 4
        p.sysex[QuasarConstants.ARRAY_PART_4_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_4_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x04;
        // The performance name
        p.sysex[QuasarConstants.ARRAY_NAME_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_NAME_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x05;
    ...
    public void sendPatch(Patch p) {
        ...
        p.sysex[QuasarConstants.ARRAY_PART_1_OFFSET
                ...
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_1_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x0D;
        // Part 14
        p.sysex[QuasarConstants.ARRAY_PART_2_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_2_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x0E;
        // Part 15
        p.sysex[QuasarConstants.ARRAY_PART_3_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_3_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x0F;
        // Part 16
        p.sysex[QuasarConstants.ARRAY_PART_4_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_PART_4_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x10;
        // The temporary name
        p.sysex[QuasarConstants.ARRAY_NAME_OFFSET
                + QuasarConstants.ARRAY_PERFORMANCE_OFFSET] =
                (byte) performanceOffset;
        p.sysex[QuasarConstants.ARRAY_NAME_OFFSET
                + QuasarConstants.ARRAY_PERF_PART_OFFSET] = (byte) 0x11;
     ...
     *            Ignored
     * @param patchNum
     *            The number of the Performance which is requested
     */
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        if (sysexRequestDump == null) {
            ...
            for (int count = 0; count < QuasarConstants.SYSEX_PERFORMANCE_REQUEST.length; count++) {
                this.sysexRequestDump =
                        new SysexHandler(
                                QuasarConstants.SYSEX_PERFORMANCE_REQUEST[count]);

                send(sysexRequestDump.toSysexMessage(getDeviceID(),
                        new SysexHandler.NameValue("perfNumber", patchNum
                                + QuasarConstants.SYSEX_PERFORMANCE_OFFSET)));

                try {
                    // Wait a little bit so that everything is in the correct
                    // sequence
                    Thread.sleep(50);
                } catch (Exception ex) {
    ...
    /*
     * Trims 644 Byte sysex files to the 223 Bytes this driver uses public int
     * trimSysex(Patch p) { if (p.sysex.length == 644) { byte [] sysex = new
     * byte[QuasarConstants.PATCH_SIZE]; // starting at 19h (=25): Temporary
     * name: 17 Bytes System.arraycopy(p.sysex, 25, sysex,
     * QuasarConstants.ARRAY_NAME_OFFSET, 17); // starting at 2Ah (= 42):
     * Temporary common parameters: 74 Bytes System.arraycopy(p.sysex, 42,
     * sysex, 0, 74); // starting at 200h (=512): Part 13: 33 Bytes
     * System.arraycopy(p.sysex, 512, sysex,
     * QuasarConstants.ARRAY_PART_1_OFFSET, 33); // starting at 221h (=545):
     * Part 14: 33 Bytes System.arraycopy(p.sysex, 545, sysex,
     * QuasarConstants.ARRAY_PART_2_OFFSET, 33); // starting at 242h (=578):
     * Part 15: 33 Bytes System.arraycopy(p.sysex, 578, sysex,
     * QuasarConstants.ARRAY_PART_3_OFFSET, 33); // starting at 263h (=611):
     * Part 16: 33 Bytes System.arraycopy(p.sysex, 611, sysex,
     * QuasarConstants.ARRAY_PART_4_OFFSET, 33); p.sysex = sysex; } return
     * p.sysex.length; }
     */
    ...
    /**
     * Same as createNewPatch(performanceNumber,
     * QuasarConstants.SYSEX_TEMPORARY_OFFSET); <strong>For internal use
     * only!!!</strong>
    ...
    private final Patch createNewPatch(int performanceNumber) {
        return createNewPatch(performanceNumber,
                QuasarConstants.SYSEX_TEMPORARY_OFFSET);
    }
     ...
     *            The number where the Performance should be stored later
     * @param sysexOffset
     *            The offset where the Performance should be stored:<br>
     *            QuasarConstants.SYSEX_TEMPORARY_OFFSET<br>
     *            or<br>
     *            QuasarConstants.SYSEX_PERFORMANCE_OFFSET<br>
     */
    public final Patch createNewPatch(int performanceNumber,
            int sysexOffset) {
        ...
        sysex[offset + 8] = (byte) 0x00; // Foot control toggle mode (00h = off,
                                         // 01h = on)
        // Modulation matrix
        ...
        /*
         * arp pak 1 bit 2 arp_on (0 = off, 1 = on) bits 0-1 arp resolution (00
         * = 4, 01 = 8, 10 = 16, 11 = 32)
         */
        ...
        /*
         * arp pak 2 bits 5-6 arp_sync (00 = int, 01 = ext1, 10 = ext2) bits 3-4
         * arp_dir (00 = up, 01 = down, 10 = up/down) bit 2 arp_sort (0 = off, 1
         * = on) bit 1 arp_hold (0 = off, 1 = on) bit 0 arp_velo (0 = off, 1 =
         * on)
         */
        sysex[offset + 22] = (byte) 0x00;
        /*
         * arp pak 3 bits 3-6 arp_track (0000 = 1, 0001 = 2, 0010 = 3, ... ,
         * 1111 = 16) bit 2 arp_thru (0 = off, 1 = on) bit 1 arp_out (0 = off, 1
         * = on) bit 0 arp_freeze (0 = off, 1 = on)
         */
        ...
        for (int partNumber = 1; partNumber <= 4; partNumber++) {
            ...
            sysex[offset + 2] = (byte) 0x01; // Trackmode (00h = muted, 01h =
                                             // poly, 02h = mono)
            ...
            sysex[offset + 7] = (byte) 0x18; // Transpose (18h = no transpose,
                                             // 00h = -24, 30h = +24)
            sysex[offset + 8] = (byte) 0x40; // Tune (Offset value, -64 ... +63,
                                             // 40h = 0)
            sysex[offset + 9] = (byte) 0x40; // Cutoff frequency (Offset value,
                                             // -64 ... +63, 40h = 0)
            sysex[offset + 10] = (byte) 0x40; // Resonance (Offset value, -64
                                              // ... +63, 40h = 0)
            sysex[offset + 11] = (byte) 0x40; // EG Attack (Offset value, -64
                                              // ... +63, 40h = 0)
            sysex[offset + 12] = (byte) 0x40; // EG Decay (Offset value, -64 ...
                                              // +63, 40h = 0)
            sysex[offset + 13] = (byte) 0x40; // EG Release (Offset value, -64
                                              // ... +63, 40h = 0)
            sysex[offset + 14] = (byte) 0x40; // Vibrato rate (Offset value, -64
                                              // ... +63, 40h = 0)
            sysex[offset + 15] = (byte) 0x40; // Vibrato depth (Offset value,
                                              // -64 ... +63, 40h = 0)
            sysex[offset + 16] = (byte) 0x40; // Vibrato delay (Offset value,
                                              // -64 ... +63, 40h = 0)
            ...
            sysex[offset + 20] = (byte) 0x0C; // Pitch sensivity (00h = -12, 0Ch
                                              // = 0, 18h = +12)
            sysex[offset + 21] = (byte) 0x40; // Volume mod. sens.(Offset value,
                                              // -64 ... +63, 40h = 0)
            sysex[offset + 22] = (byte) 0x40; // Tone mod. sens.(Offset value,
                                              // -64 ... +63, 40h = 0)
        ...
        Patch p = getPatchFactory().createNewPatch(sysex);
        return p;
    }
