...
public class D10SysexMessage {
    ...
    /*
     * byte manufacturer; byte deviceId; byte model; byte command; int address;
     * int size;
     */
    public D10SysexMessage(byte command, int bodySize) {
        message =
                new byte[D10Constants.SIZE_HEADER + bodySize
                        + D10Constants.SIZE_TRAILER];
    ...
    private static void calculateChecksum(byte[] buf, int start, int end,
            int ofs) {
        int sum = 0;
    ...
    private static void calculateChecksum(byte[] buf) {
        int size = buf.length;
        D10SysexMessage.calculateChecksum(buf, D10Constants.OFS_ADDRESS,
                size - 3, size - 2);
    }
}
