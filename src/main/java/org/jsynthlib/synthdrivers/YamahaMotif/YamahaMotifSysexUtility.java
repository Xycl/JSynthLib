...
public abstract class YamahaMotifSysexUtility {
    ...
    public static void checksum(byte[] sysex) {
        for (int size = 0, offset = 0; offset <= sysex.length - SYSEX_OVERHEAD; offset +=
            ...
            checksum(sysex, offset + SIZE_OFFSET, offset + size
                    + SYSEX_OVERHEAD + CHECKSUM_OFFSET);
        }
    }

    public static void checksum(byte[] sysex, int offset) {
        int size = getSize(sysex, offset);
        checksum(sysex, offset + SIZE_OFFSET, offset + size + SYSEX_OVERHEAD
                + CHECKSUM_OFFSET);
    }
    ...
    public static byte getParameter(byte[] sysex, int address) {
        return getParameter(sysex, address,
    ...
    public static byte getParameter(byte[] sysex, int address, int offset) {
        if (offset < 0 || offset + ADDRESS_OFFSET + 2 >= sysex.length)
            return -1;
        int index =
                offset + DATA_OFFSET + (address & 127)
                        - sysex[offset + ADDRESS_OFFSET + 2];
    ...
    public static boolean setShortParameter(byte[] sysex, int address,
            int value, int offset) {
        return setParameter(sysex, address, (value >> 7) & 127, offset)
                && setParameter(sysex, address + 1, value & 127, offset);
    }

    public static void splitAndSendBulk(byte[] sysex, AbstractPatchDriver driver,
            int deviceid) {
        byte[] msg;
        for (int size = 0, offset = 0; offset <= sysex.length - SYSEX_OVERHEAD; offset +=
                size + SYSEX_OVERHEAD) {
