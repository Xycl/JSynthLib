...
public abstract class YamahaMotifSingleDriver extends AbstractPatchDriver {
    ...
    protected void yamaha_init() {
        ...
        sysexRequestDump =
                new SysexHandler("F0 43 @@ 6B 0E " + base_address + " ** F7");
    ...
    public String getPatchName(Patch ip) {
        ...
        int offset =
                YamahaMotifSysexUtility.findBaseAddressOffset(
                        ((Patch) ip).sysex, address);
        try {
            return new String(((Patch) ip).sysex, offset
                    + YamahaMotifSysexUtility.DATA_OFFSET, 10, "US-ASCII");
    ...
    public void setPatchName(Patch p, String name) {
        ...
        int offset =
                YamahaMotifSysexUtility.findBaseAddressOffset(
                        ((Patch) p).sysex, address);
        try {
            namebytes = name.getBytes("US-ASCII");
            for (int i = 0; i < 10; i++) {
                if (i >= namebytes.length)
                    ((Patch) p).sysex[offset + i
                            + YamahaMotifSysexUtility.DATA_OFFSET] = (byte) ' ';
                else
                    ((Patch) p).sysex[offset + i
                            + YamahaMotifSysexUtility.DATA_OFFSET] =
                            namebytes[i];
            }
    ...
    protected void sendPatchWorker(Patch p, int patchnum) {
        ...
        for (int offset = 0; offset <= p.sysex.length
                - YamahaMotifSysexUtility.SYSEX_OVERHEAD; offset +=
                p.sysex.length - YamahaMotifSysexUtility.SYSEX_OVERHEAD) {
            p.sysex[offset + YamahaMotifSysexUtility.ADDRESS_OFFSET + 1] =
                    (patchnum == -1) ? Byte.parseByte(edit_buffer_base_address,
                            16) : Byte.parseByte(base_address, 16);
    ...
    public Patch createNewPatch() {
        try {
            InputStream fileIn =
                    getClass().getResourceAsStream(defaults_filename);
            ...
            ErrorMsg.reportError("Error",
                    "Unable to find " + defaults_filename, e);
            return null;
        }
    }
