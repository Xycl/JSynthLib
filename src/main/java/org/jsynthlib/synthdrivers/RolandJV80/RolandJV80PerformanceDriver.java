 ...
 * @version $Id: RolandJV80PerformanceDriver.java 851 2005-01-27 07:36:36Z
 *          jbackhaus $
 */
public class RolandJV80PerformanceDriver extends AbstractRolandJV80Driver {

    final static SysexHandler SYSEX_SETPERFORMANCECOMMONJV880 =
            new SysexHandler(
    ...
    final static SysexHandler SYSEX_SETPERFORMANCECOMMONJV80 =
            SYSEX_SETPERFORMANCECOMMONJV880;
    ...
    final String PERFORMANCENUMBERS[] = generateNumbers(1,
            PERFORMANCES_PER_BANK, "Performance ##");
    ...
    void setup(String model) {
        ...
        if (isJV80) {
            ...
            patchSize =
                    performanceCommonLength + PERFORMANCE_PATCHES
                            * performancePartLength;
        } else {
        ...
        for (int i = 0; i < PERFORMANCE_PATCHES; i++)
            performancePartOffsets[i] =
                    performanceCommonLength + i * performancePartLength;
    }
    ...
    public void calculateChecksum(Patch p, int offset) {
        calculateChecksum(p.sysex, offset, performanceCommonLength
                - SYSEX_OVERHEAD);
        for (int i = 0; i < 8; i++) {
            calculateChecksum(p.sysex, offset + performancePartOffsets[i],
                    performancePartLength - SYSEX_OVERHEAD);
        }
    }
    ...
    public void setPerformanceNum(byte[] sysex, int offset, int bankNum,
            int performanceNum) {
        ...
        for (int i = 0; i < 8; i++) {
            sysex[offset + performancePartOffsets[i] + JV80Constants.ADDR1_IDX] =
                    (byte) (0x01 + bankNum);
            sysex[offset + performancePartOffsets[i] + JV80Constants.ADDR2_IDX] =
                    (byte) (performanceNum);
            sysex[offset + performancePartOffsets[i] + JV80Constants.ADDR3_IDX] =
                    (byte) (0x18 + i);
            sysex[offset + performancePartOffsets[i] + JV80Constants.ADDR4_IDX] =
                    0x00;
        }
    }
    ...
    public Patch createNewPatch() {
        ...
        byte sysexcommon[] =
                setPerformanceCommon.toByteArray(getDeviceID(), 00);
        ...
        for (int i = 0; i < 8; i++) {
            ...
            System.arraycopy(sysexpart, 0, sysex, performancePartOffsets[i],
                    performancePartLength);
        }
    ...
    public JSLFrame editPatch(Patch p) {
        return new RolandJV80PerformanceEditor(p, this);
    }

    public void requestPatchDump(int bankNum, int patchNum) {
        Device dev = getDevice();
        byte[] sysex =
                JV80Constants.sysexRequestDump.toByteArray(
                        dev.getDeviceID() - 1, 0);
        ...
        setRequestLength(sysex, performanceCommonLength - SYSEX_OVERHEAD);
        sendRequestSysex(dev, sysex);

        for (int i = 0; i < 8; i++) {
            sysex[JV80Constants.ADDR3_IDX] = (byte) (0x18 + i);
            setRequestLength(sysex, performancePartLength - SYSEX_OVERHEAD);
            sendRequestSysex(dev, sysex);
        }
    }
}
