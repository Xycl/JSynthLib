 ...
 * @version $Id: RolandJV80Converter.java 851 2005-01-27 07:36:36Z jbackhaus $
 */
public class RolandJV80Converter extends Converter {
    ...
    public boolean supportsPatch(String header, byte[] sysex) {
        ...
        return sysex[0] == (byte) 0xF0 && sysex[1] == 0x41 && sysex[3] == 0x46
                && sysex[4] == 0x12;
    }
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        for (int i = 0; i < msgs.length; i++) {
            ...
            byte dump[] = sm.getData(); // <-- MISSING THE FIRST F0 AND ENDING
                                        // F7!!
            ...
            if (supportsData(dump)) {
                ...
                if (addr != -1) {
                    ...
                    // A JV880 patch tone in a JV80 bank will overwrite the
                    // checksum
                    // byte, which is revalidated anyway.
                    System.arraycopy(dump, 0, bpatch.sysex, addr + 1,
                            dump.length - 1);
                }
            }
        }
