 ...
 * @version $Id: RolandJV80PatchDriver.java 851 2005-01-27 07:36:36Z jbackhaus $
 */
public class RolandJV80PatchDriver extends AbstractRolandJV80Driver {
    ...
    public RolandJV80PatchDriver() {
        ...
        patchNumbers =
                generateNumbers(1, RolandJV80BankDriver.PATCHES_PER_BANK,
                        "Patch ##");
    ...
    public void calculateChecksum(Patch p, int offset) {
        calculateChecksum(p.sysex, offset, patchCommonLength
                - SYSEX_OVERHEAD);
        for (int i = 0; i < 4; i++) {
            calculateChecksum(p.sysex, offset
                    + patchToneOffsets[i], patchToneLength - SYSEX_OVERHEAD);
        }
    }
    ...
    public void setPatchNum(byte[] sysex, int offset, int bankNum, int patchNum) {
        setPatchNum(sysex, offset, bankNum, patchNum, -1);
        for (int i = 0; i < 4; i++) {
            setPatchNum(sysex, offset + patchToneOffsets[i],
                    bankNum, patchNum, i);
        }
    }
    ...
    public Patch createNewPatch() {
        ...
        for (int i = 0; i < 4; i++) {
            System.arraycopy(sysextone, 0, sysex, patchToneOffsets[i],
                    sysextone.length);
        }
    ...
    public JSLFrame editPatch(Patch p) {
        return new RolandJV80PatchEditor(p, this);
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        byte[] se =
                JV80Constants.sysexRequestDump.toByteArray(
                        dev.getDeviceID() - 1, 0);
        setPatchNum(se, 0, bankNum, patchNum, -1);
        setRequestLength(se, patchCommonLength - SYSEX_OVERHEAD);
        sendRequestSysex(dev, se);

        for (int i = 0; i < 4; i++) {
            setPatchNum(se, 0, bankNum, patchNum, i);
            setRequestLength(se, patchToneLength - SYSEX_OVERHEAD);
            sendRequestSysex(dev, se);
        }
    }
}
