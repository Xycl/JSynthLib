 ...
 * @author Jeff Weber
 */
public class Line6BassPodSingleDriver extends AbstractPatchDriver {

    /**
     * Single Program Dump Request
     */
    private static final SysexHandler SYS_REQ = new SysexHandler(
            Constants.SIGL_DUMP_REQ_ID); // Program Patch Dump Request
    ...
    public Line6BassPodSingleDriver() {
        ...
        patchNameStart = Constants.PDMP_HDR_SIZE + Constants.PATCH_NAME_START; // DOES
                                                                               // include
                                                                               // sysex
                                                                               // header
        ...
        for (int i = 0; i < patchNameSize; i++) {
            c[i] =
                    (char) PatchBytes.getSysexByte(p.sysex,
                            Constants.PDMP_HDR_SIZE, Constants.PDMP_HDR_SIZE
                                    + i + nameStart);
        }
        return new String(c);
    }
    ...
    /**
     * Sets the name of the program patch. Patch p is the target program patch.
    ...
    public void setPatchName(Patch p, String name) {
        ...
        try {
            ...
            for (int i = 0; i < patchNameSize; i++) {
                PatchBytes.setSysexByte(p, Constants.PDMP_HDR_SIZE,
                        Constants.PDMP_HDR_SIZE + i + nameStart, nameBytes[i]);
            }
    ...
    public void sendPatch(Patch p) {
        ...
        System.arraycopy(Constants.EDIT_DUMP_HDR_BYTES, 0, newSysex, 0,
                Constants.EDMP_HDR_SIZE);
        System.arraycopy(p.sysex, Constants.PDMP_HDR_SIZE, newSysex,
                Constants.EDMP_HDR_SIZE, newSysexLength
                        - Constants.EDMP_HDR_SIZE);
        p.sysex = newSysex;
        sendPatchWorker(p);
     ...
     * bankNum is a user bank number in the range 0 to 9. patchNum is a patch
     * number within the bank, in the range 0 to 3.
     */
    @Override
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        sendPatchWorker(p);
        try {
            Thread.sleep(Constants.PATCH_SEND_INTERVAL); // Delay so POD can
                                                         // keep up (pauses
                                                         // between each patch
    ...
    /**
     * Presents a dialog instructing the user to play his instrument. Line6 Pod
     * devices do not "Play" patches, so a dialog is presented instead.
     */
    @Override
    public void playPatch(Patch p) {
        log.info(getPatchName(p) + "  Header -- " + "  "
                + MidiMessageFormatter.hexDump(p.sysex, 0, Constants.PDMP_HDR_SIZE, 16)
                + "  Data -- " + "  "
    ...
    /**
     * Requests a dump of a single program patch. Even though, from an
     * operational standpoint, the POD has nine banks (numbered 1 through 9) of
     * four patches each (numbered A, B, C, and D), internally there is only a
     * single bank of 36 patch locations, referenced by program change numbers
     * 0-35. By assigning the numbers 0 through 8 for the banks and 0 through 3
     * for the patches, the conversion is as follows: program number = (bank
     * number * 4) + patch number
     */
    @Override
    ...
    /*
     * public void setBankNum (int bankNum) { // Not used for POD } public void
     * setPatchNum (int patchNum) { // Not used for POD }
     */
    ...
    public JSLFrame editPatch(Patch p) {
        return new Line6BassPodSingleEditor(p);
    }
}
