...
/**
 * Line6 Bank Driver. Used for Line6 bank patch. Line6 devices appear to the
 * user to have nine banks consisting of four patch locations apiece. The banks
 * are numbered 1 through 9 and the patches are designated by the letters A, B,
 * C, and D. The actual physical layout is a single bank consisting of 36 patch
 * locations numbered 0 to 35.
 * @author Jeff Weber
 */
public class Line6BassPodBankDriver extends AbstractBankDriver {
    /**
     * Program Bank Dump Request
     */
    private static final SysexHandler SYS_REQ = new SysexHandler(
            Constants.BANK_DUMP_REQ_ID); // Program Bank Dump Request
    ...
    /**
     * Offset of the patch name in the sysex record, not including the sysex
     * header.
     */
    private static int nameStart = Constants.PATCH_NAME_START;

    /**
     * Constructs a Line6BassPodBankDriver
     */
    public Line6BassPodBankDriver() {
        super(Constants.BANK_PATCH_TYP_STR, Constants.AUTHOR,
        ...
        patchSize =
                Constants.PATCHES_PER_BANK * Constants.SIGL_SIZE
                        + Constants.BDMP_HDR_SIZE + 1;
        patchNameStart = Constants.BDMP_HDR_SIZE + Constants.PATCH_NAME_START; // DOES
                                                                               // include
                                                                               // sysex
                                                                               // header
     ...
     * designated by a number between 0 and 35.
     */
    @Override
    public String getPatchName(Patch p, int patchNum) {
        ...
        for (int i = 0; i < patchNameSize; i++) {
            c[i] =
                    (char) PatchBytes.getSysexByte(p.sysex,
                            Constants.BDMP_HDR_SIZE, i + nameOfs);
        }
     ...
     * patchNum represents the location of the single patch within the bank,
     * designated by a number between 0 and 35. String name contains the name to
    ...
    public void setPatchName(Patch p, int patchNum, String name) {
        ...
        try {
            ...
            for (int i = 0; i < patchNameSize; i++) {
                PatchBytes.setSysexByte(p, Constants.BDMP_HDR_SIZE,
                        i + nameOfs, namebytes[i]);
            }
     ...
     * where patchNum is in the range 0 through 35. The header and trailer bytes
     * are stripped from the sysex data and the target location within the bank
     * is overwritten.
     */
    @Override
    public void putPatch(Patch bank, Patch p, int patchNum) // Tested?? //
                                                               // Retest with
                                                               // new version of
                                                               // Core.*
    {
        ...
        System.arraycopy(p.sysex, Constants.PDMP_HDR_SIZE, bank.sysex,
                getNibblizedSysexStart(patchNum), Constants.SIGL_SIZE);
    }
     ...
     * patchNum, where patchNum is in the range 0 through 35. The patch is
     * extracted from the bank and a valid Line6 program patch header is
     * appended at the beginning and 0xF7 is appended at the end.
     */
    @Override
    public Patch getPatch(Patch bank, int patchNum) {
        byte[] sysex =
                new byte[Constants.SIGL_SIZE + Constants.PDMP_HDR_SIZE + 1];
        System.arraycopy(Constants.SIGL_DUMP_HDR_BYTES, 0, sysex, 0,
                Constants.PDMP_HDR_SIZE);
        sysex[7] = (byte) patchNum;
        sysex[Constants.SIGL_SIZE + Constants.BDMP_HDR_SIZE + 1] = (byte) 0xF7;
        System.arraycopy(bank.sysex, getNibblizedSysexStart(patchNum), sysex,
                Constants.PDMP_HDR_SIZE, Constants.SIGL_SIZE);
        try {
     ...
     * patch given by patchNum. The value returned includes the header bytes and
     * represents the offset in nibblized (Line6-native) bytes. Called by
    ...
    protected BankPatch createNewPatch() {
        byte[] sysex =
                new byte[Constants.BDMP_HDR_SIZE
                        + (Constants.SIGL_SIZE * Constants.PATCHES_PER_BANK)
                        + 1];
        System.arraycopy(Constants.BANK_DUMP_HDR_BYTES, 0, sysex, 0,
                Constants.BDMP_HDR_SIZE);
        sysex[Constants.BDMP_HDR_SIZE
                + (Constants.SIGL_SIZE * Constants.PATCHES_PER_BANK)] =
                (byte) 0xF7;
        BankPatch p = getPatchFactory().newBankPatch(sysex, this);
        for (int i = 0; i < Constants.PATCHES_PER_BANK; i++) {
            System.arraycopy(Constants.NEW_SYSEX, Constants.PDMP_HDR_SIZE,
                    p.sysex, getNibblizedSysexStart(i), Constants.SIGL_SIZE);
    ...
    /**
     * Requests a dump of a Line6 bank consisting of 36 patches. The bankNum and
     * patchNum parameters are ignored.
     */
    @Override
    ...
    /**
     * Sends a bank patch to the device. Patch p represents the bank patch to be
     * stored. For the Pods, the bankNum and patchNum parameters are ignored
     * (since the Pod only has one bank).
     */
    @Override
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        for (int progNbr = 0; progNbr < Constants.PATCHES_PER_BANK; progNbr++) {
            ...
            ((Line6BassPodSingleDriver) thisPatch[progNbr].getDriver())
                    .storePatch(thisPatch[progNbr], bankNbr, ptchNbr);
        }
    }
}