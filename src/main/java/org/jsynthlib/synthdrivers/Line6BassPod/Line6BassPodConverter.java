...
public class Line6BassPodConverter extends Converter {
     ...
     */
    //
    // if block of data contains (as the last valid line6 patch) a program patch
    // or a bank patch--
    // (Not testing for edit buffer patch here because if it is, it still needs
    // to be converted.)
    public boolean supportsPatch(String patchString, byte[] sysex) {
        if (super.supportsPatch(patchString, parseSysex(sysex))) { // Parse out
                                                                   // the last
                                                                   // patch in
                                                                   // the block
                                                                   // and test
                                                                   // it.
            if ((sysex.length == Constants.PDMP_HDR_SIZE + Constants.SIGL_SIZE
                    + 1)
                    || (sysex.length == Constants.BDMP_HDR_SIZE
                ...
                return true; // If sysex contains a block'o'data (or edit buffer
                             // sysex), it still needs to be converted
            }
        } else {
    ...
    public Patch[] extractPatch(Patch p) {
        ...
        if (sysex[6] == (byte) 0x00) { // Is this a program patch?
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new Line6BassPodSingleDriver());
        } else {
            newPatchArray[0] = getPatchFactory().createNewPatch(sysex, new Line6BassPodBankDriver());
        }
        return newPatchArray;
    }

    /**
     * Attempts to parse a Line6 patch (program, edit buffer, or bank) from a
     * Line6 response to a dump request. If parseSysex cannot find a valid Line6
     * patch within the block of data, it returns the original block of data,
     * unchanged.
     */
    ...
    /**
     * Converts a Line6 Edit Buffer patch to a Line6 program patch. Any time an
     * edit buffer patch is received from a Line6 device it is converted to a
     * program patch. From that point on it is handled like any other program
     * patch.
     */
    private byte[] convertToProgramPatch(byte[] sysex) {
        int newSysexLength = sysex.length + 1;
        byte newSysex[] = new byte[newSysexLength];
        System.arraycopy(Constants.SIGL_DUMP_HDR_BYTES, 0, newSysex, 0,
                Constants.PDMP_HDR_SIZE);
        System.arraycopy(sysex, Constants.EDMP_HDR_SIZE, newSysex,
                Constants.PDMP_HDR_SIZE, newSysexLength
                        - Constants.PDMP_HDR_SIZE);
        return newSysex;
    }
}
