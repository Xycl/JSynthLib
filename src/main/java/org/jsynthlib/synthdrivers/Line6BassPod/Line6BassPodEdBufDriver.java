...
/**
 * Line6 Edit Buffer Driver. Used for Line6 Edit Buffer patch. Note that on the
 * Pod, the edit buffer patch has an 8 byte header and the program patch has a 9
 * byte header. The only reason for having this driver is to be able to request
 * an edit buffer patch. As soon as the edit buffer patch is received,
 * Line6BassPodConverter converts it to a regular program patch. From that point
 * on it is handled like any other program patch.
 * @author Jeff Weber
 */
public class Line6BassPodEdBufDriver extends Line6BassPodSingleDriver {
    ...
    private static final SysexHandler SYS_REQ = new SysexHandler(
            Constants.EDIT_DUMP_REQ_ID); // Edit Buffer Dump Request
    ...
    public Line6BassPodEdBufDriver() {
        ...
        // Edit buffer patch will be converted to regular program patch right
        // away
        ...
        patchNameStart = Constants.PATCH_NAME_START; // does NOT include sysex
                                                     // header
    ...
    /**
     * Requests a dump of the Line6 edit buffer. The bankNum and patchNum
