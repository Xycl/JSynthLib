 ...
 *		(used by DX7-II, DX7s)
 * ===============================================================
 * @version $Id: DX7FamilyPerformanceIIBankDriver.java 744 2004-10-03 17:44:17Z hayashi $
...
public class DX7FamilyPerformanceIIBankDriver extends AbstractBankDriver {
    byte[] initSysex;
    ...
    private static final int dxPatchNameSize = 20; // size of patchname of
                                                   // single patch
    private static final int dxPatchNameOffset = 31; // offset in packed bank
                                                     // format
    private static final int dxSinglePackedSize = 51; // size of single patch in
                                                      // packed bank format
    private static final int dxSysexHeaderSize = 16; // length of sysex header

    public DX7FamilyPerformanceIIBankDriver(byte[] initSysex,
        ...
        sysexRequestDump =
                new SysexHandler("f0 43 @@ 7e 4c 4d 20 20 38 39 37 33 50 4d f7");
        deviceIDoffset = 2;
        patchNameStart = 0;
        patchNameSize = 0;
                                                            ...
                                                            // needed
    {
        ...
        for (int i = 0; i < 51; i++) {
            bank.sysex[getPatchStart(patchNum) + i] =
                    (p.sysex[16 + i]);
        }
    ...
    @Override
    public BankPatch createNewPatch() // create a bank with 32 performance patches
    {
