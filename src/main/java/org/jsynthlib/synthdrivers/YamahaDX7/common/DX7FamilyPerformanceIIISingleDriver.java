 ...
 *		(used by TX802)
 * ==========================================================================
 * @version $Id: DX7FamilyPerformanceIIISingleDriver.java 662 2004-08-13 03:08:21Z hayashi $
...
public class DX7FamilyPerformanceIIISingleDriver extends AbstractPatchDriver {
    byte[] initSysex;
    String[] dxPatchNumbers;
    String[] dxBankNumbers;

    public DX7FamilyPerformanceIIISingleDriver(byte[] initSysex,
        ...
        numSysexMsgs = 1;
        sysexRequestDump =
                new SysexHandler("F0 43 @@ 7E 4C 4D 20 20 38 39 35 32 50 45 F7");
    }
    ...
    public JSLFrame editPatch(Patch p) {
        return new DX7FamilyPerformanceIIIEditor(getManufacturerName() + " "
    ...
    public String getPatchName(Patch p) {
        Patch ip = (Patch) p;
        try {
            byte[] b = new byte[patchNameSize / 2]; // 1 character encoded in 2
                                                    // bytes!

            for (int i = 0; i < b.length; i++) {
                b[i] =
                        (byte) (DX7FamilyByteEncoding
                                .AsciiHex2Value(ip.sysex[16 + 2 * (96 + i)]) * 16 + DX7FamilyByteEncoding
                                .AsciiHex2Value(ip.sysex[16 + 2 * (96 + i) + 1]));
            }

            StringBuffer s =
                    new StringBuffer(new String(b, 0, patchNameSize / 2,
                            "US-ASCII"));
    ...
    public void setPatchName(Patch p, String name) {
        byte[] namebytes = new byte[patchNameSize / 2]; // 1 character encoded
                                                        // in 2 bytes!
        ...
        try {
            ...
            for (int i = 0; i < namebytes.length; i++) {
                ((Patch) p).sysex[16 + 2 * (96 + i)] =
                        (byte) (DX7FamilyByteEncoding
                                .Value2AsciiHexHigh(namebytes[i]));
                ((Patch) p).sysex[16 + 2 * (96 + i) + 1] =
                        (byte) (DX7FamilyByteEncoding
