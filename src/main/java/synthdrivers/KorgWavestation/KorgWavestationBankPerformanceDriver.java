 ...
 * @version $Id: KorgWavestationBankPerformanceDriver.java 939 2005-03-03
 *          04:05:40Z hayashi $
...
public class KorgWavestationBankPerformanceDriver extends BankDriver {
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        try {
            ...
            for (int i = 0; i < 16; i++) {
                byteBuffer[i] =
                        (byte) (((Patch) p).sysex[nameStart + i * 2] + ((0x10) * ((Patch) p).sysex[nameStart
                                + i * 2 + 1]));
                if (byteBuffer[i] == 0)
                    byteBuffer[i] = 0x20;
            }
    ...
    public void setPatchName(Patch p, int patchNum, String name) {
        ...
        try {
            ...
            for (int i = 0; i < patchNameSize; i++) {
                ((Patch) p).sysex[patchNameStart + i * 2] =
                        (byte) (namebytes[i] & 0x0f);
                ((Patch) p).sysex[patchNameStart + i * 2 + 1] =
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(((Patch) p).sysex, 8, ((Patch) bank).sysex,
