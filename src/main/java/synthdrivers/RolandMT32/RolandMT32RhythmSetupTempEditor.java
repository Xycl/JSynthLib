 ...
 *
 * @version $Id: RolandMT32RhythmSetupTempEditor.java 1031 2005-10-10 17:44:56Z billzwicky $
 */
...
class RolandMT32RhythmSetupTempEditor extends PatchEditorFrame {
    ...
    public RolandMT32RhythmSetupTempEditor(Patch patch) {
        ...
        int gy = 0; // row count
        int k = 0x10 + 0x80;
        int basad = 3; // point to base address range 03 (00 00) of Patch temp
                       // Area
        int lwc = getLabelWidth("Reverb Switch  "); // Longest label length
        JPanel cmnPane = new JPanel();
        cmnPane.setLayout(new GridBagLayout());
        gbc.weightx = 0;
        addWidget(cmnPane, new ComboBoxWidget("Timbre", patch, new MT32Model(
                patch, 0x00), new MT32Sender(k + 0x00, basad), TimbreName), 0,
                gy, 1, 1, 11);
        gy++;
        addWidget(cmnPane, new ScrollBarWidget("Output Level", patch, 0, 100,
                0, lwc, new MT32Model(patch, 0x01), new MT32Sender(k + 0x01,
                        basad)), 0, gy, 5, 1, 1);
        gy++;
        addWidget(cmnPane, new ScrollBarWidget("Pan Pot (R-L)", patch, 0, 14,
                -7, lwc, new MT32Model(patch, 0x02), new MT32Sender(k + 0x02,
                        basad)), 0, gy, 5, 1, 2);
        gy++;
        addWidget(cmnPane, new CheckBoxWidget("Reverb", patch, new MT32Model(
                patch, 0x03), new MT32Sender(k + 0x03, basad)), 0, gy, 5, 1, 4);
        // addWidget(cmnPane, new ComboBoxWidget("Reverb Switch", patch,
        // new MT32Model(patch,0x03), new MT32Sender(k+0x03,basad),
        // RevSwitchName), 0, gy, 1, 1, 11);
        ...
        cmnPane.setBorder(new TitledBorder(
                new EtchedBorder(EtchedBorder.RAISED), "Rhythm Setup",
