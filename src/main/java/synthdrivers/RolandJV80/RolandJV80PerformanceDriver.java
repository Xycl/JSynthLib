...
public class RolandJV80PerformanceDriver extends Driver {
    ...
    final static String PERFORMANCENUMBERS[] = DriverUtil.generateNumbers(1,
    ...
    public void calculateChecksum(Patch p, int offset) {
        JV80Constants.calculateChecksum(p.sysex, offset,
                performanceCommonLength - SYSEX_OVERHEAD);
        for (int i = 0; i < 8; i++) {
            JV80Constants.calculateChecksum(p.sysex, offset
                    + performancePartOffsets[i], performancePartLength
                    - SYSEX_OVERHEAD);
        }
    }
    ...
    public void requestPatchDump(int bankNum, int patchNum) {
        ...
        JV80Constants.setRequestLength(sysex, performanceCommonLength
                - SYSEX_OVERHEAD);
        JV80Constants.sendRequestSysex(dev, sysex);

        for (int i = 0; i < 8; i++) {
            sysex[JV80Constants.ADDR3_IDX] = (byte) (0x18 + i);
            JV80Constants.setRequestLength(sysex, performancePartLength
                    - SYSEX_OVERHEAD);
            JV80Constants.sendRequestSysex(dev, sysex);
        }
    }
