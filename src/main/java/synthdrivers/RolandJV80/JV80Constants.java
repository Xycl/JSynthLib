...
public class JV80Constants {
    ...
    final static String BANKS[] = new String[] {
            "Internal", "Card" };
    final static String PATCHNUMBERS[] = DriverUtil.generateNumbers(1,
            RolandJV80BankDriver.PATCHES_PER_BANK, "Patch ##");
    ...
    static void calculateChecksum(byte sysex[], int offset, int datalen) {
        DriverUtil.calculateChecksum(sysex, offset + CHECKSUM_START, offset
                + CHECKSUM_START + datalen + 3, offset + CHECKSUM_START
                + datalen + 4);
    }
    ...
    static void setPatchNum(byte[] sysex, int offset, int bankNum,
