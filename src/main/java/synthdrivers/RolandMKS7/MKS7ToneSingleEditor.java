...
// @version $Id: MKS7ToneSingleEditor.java 1057 2006-03-07 20:47:31Z jbackhaus $
...
class MKS7ToneSingleEditor extends PatchEditorFrame {
    public MKS7ToneSingleEditor(Patch patch) {
        ...
        addWidget(vcfPane, new ScrollBarWidget("Resonance", patch, 0, 127, 0,
                new ParamModel(patch, 11), new MKSToneSender(6)), 0, 1, 3, 1,
                10);
        addWidget(vcfPane, new ScrollBarWidget("Env Amount", patch, 0, 127, 0,
                new ParamModel(patch, 12), new MKSToneSender(7)), 0, 2, 3, 1,
                11);
        addWidget(vcfPane, new ScrollBarWidget("Key Follow", patch, 0, 127, 0,
                new ParamModel(patch, 14), new MKSToneSender(9)), 0, 3, 3, 1,
                12);
        ...
        addWidget(lfoPane, new ScrollBarWidget("DCO Mod Depth", patch, 0, 127,
                0, new ParamModel(patch, 7), new MKSToneSender(2)), 0, 2, 7, 1,
                16);
        addWidget(lfoPane, new ScrollBarWidget("PWM Mod Depth", patch, 0, 127,
                0, new ParamModel(patch, 8), new MKSToneSender(3)), 0, 3, 7, 1,
                17);
        addWidget(lfoPane, new ScrollBarWidget("VCF Mod Depth", patch, 0, 127,
                0, new ParamModel(patch, 13), new MKSToneSender(8)), 0, 4, 7,
                1, 18);
        gbc.gridx = 0;
        gbc.gridy = 3;
        gbc.gridwidth = 1;
        ...
        addWidget(envPane, new EnvelopeWidget(" ", patch,
                new EnvelopeWidget.Node[] {
                        ...
                        new EnvelopeWidget.Node(0, 127, new ParamModel(patch,
                        ...
                        new EnvelopeWidget.Node(127, 127, null,
                                EnvelopeWidget.Node.SAME, 0, null, 0, false,
                                null, null, null, null),
                        new EnvelopeWidget.Node(0, 127, new ParamModel(patch,
                                19), 0, 0, null, 0, false,
        ...
        addWidget(cmnPane, new ScrollBarWidget("VCA Level", patch, 0, 127, 0,
                new ParamModel(patch, 15), new MKSToneSender(10)), 0, 3, 3, 1,
                22);
        gbc.gridx = 0;
        gbc.gridy = 1;
        gbc.gridwidth = 1;
...
class MKSToneSender extends SysexSender {
    byte b[] = {
            (byte) 0xF0, (byte) 0x41, (byte) 0x32, (byte) 0x00, (byte) 0x00,
            (byte) 0x00, (byte) 0xF7 };
...
class ParamModelBit extends ParamModel {
    ...
    public void set(int i) {
        patch.sysex[ofs] =
                (byte) ((patch.sysex[ofs] & (~mask)) | ((i << power) & mask));
    }
...
class ParamModelBitExp extends ParamModel {
    ...
    public void set(int i) {
        patch.sysex[ofs] =
                (byte) ((patch.sysex[ofs] & (~mask)) | (((1 << i) << power) & mask));
    }

    public int get() {
        int r = ((patch.sysex[ofs] & mask) >> power);
        return ((r <= 1) ? 0 : (r <= 2) ? 1 : (r <= 4) ? 2 : (r <= 8) ? 3
                : (r <= 16) ? 4 : (r <= 32) ? 5 : (r <= 64) ? 6 : 7);
    }
}
