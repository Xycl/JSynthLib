/*
 * @version $Id: OberheimMatrixBankDriver.java 939 2005-03-03 04:05:40Z hayashi $
 */
...
public class OberheimMatrixBankDriver extends BankDriver {
    ...
    public void calculateChecksum(Patch p) {
        for (int i = 0; i < 100; i++)
            calculateChecksum(p, 5 + getPatchStart(i), 272 + getPatchStart(i),
                    273 + getPatchStart(i));
    ...
    public String getPatchName(Patch p, int patchNum) {
        try {
            ...
            byte[] b = new byte[8];
            b[0] =
                    ((byte) (((Patch) p).sysex[start + 5] + ((Patch) p).sysex[start + 6] * 16));
            b[1] =
                    ((byte) (((Patch) p).sysex[start + 7] + ((Patch) p).sysex[start + 8] * 16));
            b[2] =
                    ((byte) (((Patch) p).sysex[start + 9] + ((Patch) p).sysex[start + 10] * 16));
            b[3] =
                    ((byte) (((Patch) p).sysex[start + 11] + ((Patch) p).sysex[start + 12] * 16));
            b[4] =
                    ((byte) (((Patch) p).sysex[start + 13] + ((Patch) p).sysex[start + 14] * 16));
            b[5] =
                    ((byte) (((Patch) p).sysex[start + 15] + ((Patch) p).sysex[start + 16] * 16));
            b[6] =
                    ((byte) (((Patch) p).sysex[start + 17] + ((Patch) p).sysex[start + 18] * 16));
            b[7] =
                    ((byte) (((Patch) p).sysex[start + 19] + ((Patch) p).sysex[start + 20] * 16));
    ...
    // if (deviceIDoffset>0)
    // ((Patch)p).sysex[deviceIDoffset]=(byte)(getChannel()-1);
    // try {
    // for (int i=0;i<100;i++)
    // {
