...
public class RolandD10Support {
     ...
     * @param entity
     *            The 7 bit hex entity to convert.
     ...
     * entity.
     */
    public static final int getRecordAddress(int baseAddress, int recordSize,
            int recordNumber) {
        return to7Bits(from7Bits(baseAddress) + recordNumber
                * from7Bits(recordSize));
    }
        ...
        for (int ab = 0; ab < 2; ab++) {
            for (int bank = 0; bank < 8; bank++) {
                for (int number = 0; number < 8; number++) {
                    patchNumbers[ab * 64 + bank * 8 + number] =
                            "AB".substring(ab, ab + 1)
    ...
    public static void copyPatchFromBank(int patchNum, byte[] bankData,
            byte[] patchData) {
        copyPatchData(patchNum, bankData, patchData, true);
    }

    public static void copyPatchToBank(int patchNum, byte[] bankData,
            byte[] patchData) {
        copyPatchData(patchNum, bankData, patchData, false);
    }

    public static void copyPatchData(int patchNum, byte[] bankData,
        ...
        final int startOffsetInMessage =
                startLinearOffset - (startsInMessage * MAX_BYTES_PER_MESSAGE);
        final int startOffset =
                (SYSEX_FRAME_SIZE + MAX_BYTES_PER_MESSAGE) * startsInMessage
                        + SIZE_HEADER_DT1 + startOffsetInMessage;
        if (startsInMessage == endsInMessage) {
            // Message in one piece.
            if (fromBank) {
                System.arraycopy(bankData, startOffset, patchData,
                        SIZE_HEADER_DT1, sizeOfPatch);
            } else {
                System.arraycopy(patchData, SIZE_HEADER_DT1, bankData,
                        startOffset, sizeOfPatch);
            }
        } else {
            final int firstPartLength =
                    MAX_BYTES_PER_MESSAGE - startOffsetInMessage;
            final int secondPartLength = sizeOfPatch - firstPartLength;
            if (fromBank) {
                System.arraycopy(bankData, startOffset, patchData,
                        SIZE_HEADER_DT1, firstPartLength);
                System.arraycopy(bankData, startOffset + firstPartLength
                        + SYSEX_FRAME_SIZE, patchData, SIZE_HEADER_DT1
                        + firstPartLength, secondPartLength);
            } else {
                System.arraycopy(patchData, SIZE_HEADER_DT1, bankData,
                        startOffset, firstPartLength);
                System.arraycopy(patchData, SIZE_HEADER_DT1 + firstPartLength,
                        bankData, startOffset + firstPartLength
                                + SYSEX_FRAME_SIZE, secondPartLength);
            }
        }
    }
