...
public class YamahaMotifNormalVoiceEditor extends PatchEditorFrame {
    ...
    protected ScrollBarWidget addSlider(String name, int address,
    ...
    /*
     * protected ScrollBarWidget addLabeledSlider(String name, int address, int
     * mid, boolean _short, int min, int max, int offset) { Box box =
     * Box.createVerticalBox(); panel.add(box); int sendaddress = address; if (
     * (byte)(address >> 8) == (byte) -1) sendaddress = (address & 0x7f007f) |
     * (mid << 8); JLabel label = new JLabel(name);
     * label.setMaximumSize(label.getPreferredSize()); box.add(label);
     * ScrollBarWidget bar = new ScrollBarWidget("", p, min, max, offset, new
     * MotifParamModel(p, address, mid, _short), new ParamSender(sendaddress,
     * _short)); bar.jlabel = label; box.add(bar); addWidget(null, bar,
     * slidercount++, false); return bar; }
     */
    ...
    protected void addGlue(Container c) {
        c.add(Box.createGlue());
    }

    protected void addHGlue() {
        panel.add(Box.createHorizontalGlue());
    }

    protected void addHGlue(Container c) {
        c.add(Box.createHorizontalGlue());
    }

    protected void addVGlue() {
        panel.add(Box.createVerticalGlue());
    }

    protected void addVGlue(Container c) {
    ...
    public void addWidget(Container parent, SysexWidget widget, int slidernum,
        ...
        try {
             ...
             * sliderList.add(((ScrollBarWidget)widget).slider); if (widget
             * instanceof VertScrollBarWidget)
             * sliderList.add(((VertScrollBarWidget)widget).slider); if (widget
    ...
    class GeneralPanel extends JPanel {
        public GeneralPanel() {
            ...
            cat =
                    addCombo("Category", 0x40700C, new String[] {
            ...
            panel = Box.createHorizontalBox();
            c.add(panel);
            addCombo("", 0x400003, new String[] {
                    "Mono", "Poly" });
            addHGlue();
            addCombo("Note Mode", 0x400002, new String[] {
                    "Single", "Multi" });

            panel = Box.createHorizontalBox();
            c.add(panel);
            addCombo("Tuning", 0x400004, new String[] {
                    "Equal Temp", "Pure Major C", "Pure Major C#",
            ...
            panel = Box.createHorizontalBox();
            c.add(panel);
            addCombo("Mode", 0x400007, new String[] {
                    "Legato", "Always" });
            addHGlue();
            addCombo("Time Mode", 0x40000A, new String[] {
                    "Rate", "Time" });
            addHGlue(row);
            ...
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Filter Cutoff Offset", 0x407037, 0, false, 0,
                    0x7f, -64);
            addLabeledSlider("Resonance Offset", 0x407038, 0, false, 0, 0x7f,
                    -64);
            // addHGlue();
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Chorus Send Offset", 0x407026, 0, false, 0, 0x7f,
                    -64);
            addLabeledSlider("Pitch Bend Upper Range", 0x400005, 0, false,
                    0x10, 0x58, -64);
            // addHGlue();
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Pitch Bend Lower Range", 0x400006, 0, false,
                    0x10, 0x58, -64);
            addCombo("Default Knob Mode", 0x407024, new String[] {
                    "Pan & Send", "Tone", "Assign", "M EQ Offset" });
            // addHGlue();
    ...
    class ArpPanel extends JPanel {
        public ArpPanel() {
            ...
            arp.add(panel);
            // addFiller(arp, 0, 5);
            addCombo("", 0x407015, new String[] {
                    "Off", "On" });
            addCombo("Hold", 0x407016, new String[] {
                    "sync-off", "off", "on" });
            ...
            panel = Box.createHorizontalBox();
            c.add(panel);
            addCombo("Key Mode", 0x40701C, new String[] {
                    "Sort", "Thru", "Direct" });
            addCombo("Vel. Mode", 0x40701D, new String[] {
                    "Original", "Thru" });
            addHGlue();
            panel = Box.createHorizontalBox();
            c.add(panel);
            addSlider("Arp Tempo", 0x407022, true, 1, 300, 0);
            addCombo("Unit Multiply", 0x407017, new String[] {
    ...
    /*
     * class EffectsPanel extends JPanel { public EffectsPanel () { super();
     * setLayout( new BoxLayout(this, BoxLayout.Y_AXIS) ); } }
     */
    class ElementPanel extends JPanel {
        ...
        public ElementPanel(int i) {
            ...
            panel = Box.createHorizontalBox();
            c.add(panel);
            addCombo("", 0x41ff00, element, new String[] {
                    "Off", "On" });
            ...
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Random Pan Depth", 0x41ff09, element, false, 0,
                    0x7f, 0);
            addLabeledSlider("Alternate Pan Depth", 0x41ff0A, element, false,
                    0, 0x7f, -64);
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Scaling Pan Depth", 0x41ff0B, element, false, 0,
                    0x7f, -64);
            addLabeledSlider("Velocity Cross Fade", 0x41ff10, element, false,
                    0, 0x7f, 0);
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Note Limit Low", 0x41ff0C, element, false, 0,
                    0x7f, 0);
            addLabeledSlider("Note Limit High", 0x41ff0D, element, false, 0,
                    0x7f, 0);
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Vel. Limit Low", 0x41ff0E, element, false, 0,
                    0x7f, 0);
            addLabeledSlider("Vel. Limit High", 0x41ff0F, element, false, 0,
                    0x7f, 0);
            panel = Box.createHorizontalBox();
            c.add(panel);
            addLabeledSlider("Level Velocity Sensitivity", 0x41ff21, element,
                    false, 0, 0x7f, -64);
            addLabeledSlider("Level Sensitivity Curve", 0x41ff22, element,
                    false, 0, 4, 0);
            panel = Box.createHorizontalBox();
            c.add(panel);
            addWidget(panel, new EnvelopeWidget("Amp Envelope", p,
                    new EnvelopeWidget.Node[] {
                            new EnvelopeWidget.Node(0, 127,
                                    ...
                                    "Delay", null),
                            new EnvelopeWidget.Node(10, 10, null, 0, 127,
    ...
    class MotifParamModel extends ParamModel {
        ...
        public void set(int value) {
            if (twobytes)
                YamahaMotifSysexUtility.setShortParameter(patch.sysex, ofs,
                        value);
            else
