 ...
 * reference guide. Known issues: Midi channel is partly fixed to 0. This is set
 * in the third byte to be send, actually it's four lower bits is exacty the
 * channel to be used
 * @author Juha Tukkinen
 * @version $Id: KorgX3SingleDriver.java 741 2004-10-02 05:18:14Z hayashi $
 */
public class KorgX3SingleDriver extends Driver {
    ...
    private byte[] programMode = {
            (byte) 0xF0, (byte) 0x42, 0, (byte) 0x35, (byte) 0x4E, (byte) 0x02,
            (byte) 0x00, (byte) 0xF7 };
    private byte[] programEditMode = {
            (byte) 0xF0, (byte) 0x42, 0, (byte) 0x35, (byte) 0x4E, (byte) 0x03,
            (byte) 0x00, (byte) 0xF7 };
    private byte[] programWriteRequest = {
            (byte) 0xF0, (byte) 0x42, 0, (byte) 0x35, (byte) 0x11, 0, 0,
            (byte) 0xF7 };
    ...
    /**
     * Overrided storePatch. Sends a patch to the current edit buffer on the
     * synthesizer and saves the patch.
     * @param p
     *            Patch to be sent
     * @param bankNum
     *            Bank number
     ...
     */
    /*
     * public void sendPatch (Patch p) { sendPatchWorker(p); }
     */
    /**
    ...
    protected void sendPatch(Patch p) {
        // ##TODO## first go to program edit mode so that the edit buffer will
        // be active.
        ...
        for (int i = 0; i < 164;) {
            ...
            for (int k = 0; k < 7; k++) {
                if (i + k < 164) {
                    b7 +=
                            (((Patch) p).sysex[i + k + EXTRA_HEADER] & 128) >> (7 - k);
                    pd[j + k + 1 + 5] =
                            (byte) (((Patch) p).sysex[i + k + EXTRA_HEADER] & (byte) 0x7F);
                }
            }
            pd[j + 5] = b7;
            j += 8;
