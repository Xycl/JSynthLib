/* Made by Yves Lefebvre
   ...
   @version $Id: NovationNova1SinglePerformanceDriver.java 832 2005-01-16 18:05:50Z hayashi $
 */
...
public class NovationNova1SinglePerformanceDriver extends BankDriver {
    ...
    public String getPatchName(Patch ip) {
        ...
        try {
            StringBuffer s =
                    new StringBuffer(new String(((Patch) ip).sysex,
                            (296 * 8) + 8, 16, "US-ASCII"));
            return s.toString();
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        System.arraycopy(((Patch) p).sysex, 9, ((Patch) bank).sysex,
                getPatchStart(patchNum), 296 - 9);
        calculateChecksum(bank);
    }

    public Patch getPatch(Patch bank, int patchNum) {
        // this method is call when you have a single perf opened and want to
        // send or play individual patches
        ...
        try {
            ...
            sysex[8] = (byte) 0x09;
            sysex[295] = (byte) 0xF7;
            System.arraycopy(((Patch) bank).sysex, getPatchStart(patchNum),
                    sysex, 9, 296 - 9);
    ...
    public Patch createNewPatch() {
        byte[] sysex = new byte[((296 * 8) + 406)]; // 406 is the size of the
                                                    // actual Performance data
                                                    // Note that there is 8 part
                                                    // even if only 6 are usable
        ...
        sysexHeader[8] = (byte) (0x00); // this is the part number in the
                                        // performance
        ...
        System.arraycopy(NovationNova1InitPatch.initperf, 0, p.sysex,
                (8 * 296), 406);
    ...
    /*
     * commented out because this is not used for bank driver public void
     * sendPatch (Patch bank) { byte [] newsysex = new byte[296]; Patch p = new
     * Patch(newsysex); try { for (int i=0;i<8;i++) {
     * System.arraycopy(((Patch)bank).sysex,296*i,p.sysex,0,296);
     * sendPatchWorker(p); Thread.sleep(5); // Nova have problem receiving too
     * fast, The loop itself introduce more delay so the sleep may not be
     * necessary. // NOTE : Do not modify this to send all patch in one shot! It
     * will be faster but some patch may not be received correctly on the Nova!
     * } // Now, send the rest which is the performance data itself byte []
     * newsysex2 = new byte[406]; Patch perf = new Patch(newsysex2);
     * System.arraycopy(((Patch)bank).sysex,296*8,perf.sysex,0,406);
     * perf.sysex[387] = (byte)(getChannel()-1); // there is a small sysex msg
     * at the end that // need to have the channel byte set
     * sendPatchWorker(perf); }catch (Exception e)
