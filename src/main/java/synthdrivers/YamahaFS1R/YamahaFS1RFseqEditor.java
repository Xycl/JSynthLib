 ...
 * @version $Id: YamahaFS1RFseqEditor.java 937 2005-03-02 13:28:11Z hayashi $
 */
class YamahaFS1RFseqEditor extends PatchEditorFrame {
    static final Color mColors[] = {
            Color.blue, Color.red, Color.cyan, Color.gray, Color.magenta,
    ...
    private Container buildHeaderWindow() {
        ...
        oPanel1.add(new ComboBoxWidget("Loop mode", p,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x14),
                new YamahaFS1RFseqDriver.Sender(0x14), new String[] {
                        "One way", "Round" }));
        oPanel1.add(new ComboBoxWidget("Pitch mode", p,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x17),
                new YamahaFS1RFseqDriver.Sender(0x17), new String[] {
                        "Pitch", "Non-pitch" }));
        YamahaFS1RFseqDriver.Model oFrameModel =
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x1B);
        oPanel1.add(new ComboBoxWidget("Frame format", p, oFrameModel,
                new YamahaFS1RFseqDriver.Sender(0x1B), new String[] {
                        "128", "256", "384", "512" }));
        oPanel.add(oPanel1);
        ...
        oPanel2.add(new KnobWidget("Data end", p, 0, 512, 0,
                new YamahaFS1RFseqDriver.DoubleModel((Patch) p, 0x1E),
                new YamahaFS1RFseqDriver.Sender(0x1E)));
        oPanel2.add(new KnobWidget("Loop start", p, 0, 512, 0,
                new YamahaFS1RFseqDriver.DoubleModel((Patch) p, 0x10),
                new YamahaFS1RFseqDriver.Sender(0x10)));
        oPanel2.add(new KnobWidget("Loop end", p, 0, 512, 0,
                new YamahaFS1RFseqDriver.DoubleModel((Patch) p, 0x12),
                new YamahaFS1RFseqDriver.Sender(0x12)));
        oPanel2.add(new KnobWidget("Speed adjust", p, 0, 127, -64,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x15),
                new YamahaFS1RFseqDriver.Sender(0x15)));
        oPanel2.add(new KnobWidget("Tempo vel sens", p, 0, 127, 0,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x16),
                new YamahaFS1RFseqDriver.Sender(0x16)));
        oPanel.add(oPanel2);

        JPanel oPanel3 = new JPanel(new FlowLayout(FlowLayout.LEFT));
        oPanel3.add(new ComboBoxWidget("Formant note", p,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x18),
                new YamahaFS1RFseqDriver.Sender(0x18),
                YamahaFS1RPerformanceEditor.mNotes));
        oPanel3.add(new SpinnerWidget("Pitch tuning", p, 0, 126, -63,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x19),
                new YamahaFS1RFseqDriver.Sender(0x19)));
        oPanel3.add(new KnobWidget("Delay", p, 0, 0x63, 0,
                new YamahaFS1RFseqDriver.Model((Patch) p, 0x1A),
    ...
    class FSeqGraphWindow extends Actions.MenuFrame {
        FSeqGraphWindow() {
            ...
            oRBPane.setBorder(new TitledBorder(LineBorder
                    .createGrayLineBorder(), "Sequence", TitledBorder.LEFT,
                    TitledBorder.CENTER));
            ...
            if (aSeq.equals(SEQ_PITCH)) {
                ...
                for (int op = 0; op < mNbOp; op++) {
                    if (aSeq.equals(SEQ_VOICEDFREQ)) {
                        mY[op] =
                                YamahaFS1RFseqDriver.getFrequencies((Patch) p,
                                        op, true);
                    } else if (aSeq.equals(SEQ_VOICEDLEVEL)) {
                        mY[op] =
                                YamahaFS1RFseqDriver.getLevels((Patch) p, op,
                                        true);
