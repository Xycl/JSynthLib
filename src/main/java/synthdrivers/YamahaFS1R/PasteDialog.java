...
class PasteDialog extends JDialog {
     ...
     */
    PasteDialog(Frame aOwner, Patch aSource, Patch aDest, int aPart,
            ArrayList aWidgets[]) {
         ...
         * oCopyUnvoices.addActionListener (new ActionListener () { public void
    ...
    private void pasteVoices() {
        ...
        int oIndexSource =
                YamahaFS1RVoiceDriver.VOICE_VOICE_OFFSET
                        + YamahaFS1RVoiceDriver.VOICE_SIZE
                        * Integer.valueOf(oSel.getActionCommand()).intValue();

        for (int i = 0; i < 8; i++) {
            int oIndexDest =
                    YamahaFS1RVoiceDriver.VOICE_VOICE_OFFSET
                            + YamahaFS1RVoiceDriver.VOICE_SIZE * i;
            if (mDest[i].isSelected()) {
                System.arraycopy(mSourcePatch.getByteArray(), oIndexSource,
                        mDestPatch.getByteArray(), oIndexDest,
                        YamahaFS1RVoiceDriver.VOICE_VOICE_SIZE);
                // update widgets
