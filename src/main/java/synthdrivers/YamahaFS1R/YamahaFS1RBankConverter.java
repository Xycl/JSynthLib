 ...
 * @version $Id: YamahaFS1RBankConverter.java 939 2005-03-03 04:05:40Z hayashi $
 */
public class YamahaFS1RBankConverter extends Converter {
    public YamahaFS1RBankConverter() {
        ...
        // sauf le premier caractere (F0) qui est mis en dur dans le code
        // core...
    ...
    Patch importFSEditor(byte[] aBuffer) {
        Patch oBank =
                (Patch) YamahaFS1RBankDriver.getInstance().createNewPatch();
        int oIndex = 26;
        ...
        for (int p = 0; p < 128; p++) {
            ...
            DriverUtil.calculateChecksum(oBank.sysex, oCSStart, oCSEnd,
                    oCSEnd + 1);
        }
        for (int v = 0; v < 128; v++) {
            oBank.sysex[oIDest++] = (byte) 0xF0;
