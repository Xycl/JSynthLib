...
public class YamahaCS2xBank2Driver extends BankDriver {
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        if (oSize == ((Patch) p).sysex.length) {
            System.arraycopy(((Patch) p).sysex, 0, ((Patch) bank).sysex,
                    oStart, oSize);
        } else if (csize == ((Patch) p).sysex.length
                && ((Patch) p).sysex[6] == (byte) 0x60) {
            System.arraycopy(((Patch) p).sysex, 0, ((Patch) bank).sysex,
                    oStart, 95);
            System.arraycopy(((Patch) p).sysex, 97, ((Patch) bank).sysex, 95,
                    20);
        }
    }
    ...
    protected boolean canHoldPatch(Patch p) {
        ...
        return (((Patch) p).sysex.length == 339 || ((Patch) p).sysex.length == 341);// check
                                                                                    // size
        // } else {
        // return false;
        // }
