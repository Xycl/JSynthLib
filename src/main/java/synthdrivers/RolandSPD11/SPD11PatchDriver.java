...
public class SPD11PatchDriver extends BankDriver {
     ...
     *           padarray[0] = new Patch(msgs[0].getData(),settingsDriver); for
     *           (int i = 1; i < 33; i++) { // 32 times do this: (with each
     *           patch) padarray[i] =
     *           padDriver.createNewPatch(msgs[i].getMessage()); //padarray[i]=
     *           new Patch(msgs[i].getData(),padDriver); // copy the sysex of
     *           each message in a new patch with this /* //check if byte nr8 is
