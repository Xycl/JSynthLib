 ...
 * parameters represented by a single set sysex record.
 * @author Jeff Weber
 */
class NRPNSender implements SysexWidget.ISender {
    ...
    /**
     * Constructs an NRPNSender given the parameter, which represents the mask
     ...
     * exclusively by the DM5ComboBoxWidget for the Drum Sound parameter in the
     * single set editor. When the user selects a new family using the Family
     * ComboBox, the Drum Sound ComboBox list has to be updated to show the list
     * of voices for the selected family. Since each family has a different
     * number of voices, the maximum number of voices for the Drum Sound
     * comboBox has to be changed and the lookup table has to be re- calculated.
     */
    void setMax(int max) {
        this.max = max;
        this.ccMap = getCCMap(max);
     ...
     * we have to mimic as close as possible what the DM5 is doing. In some
     * cases this doesn't even work, which is why we furnish another constructor
     * that lets the user specify the conversion table explicitly.
     */
     ...
     * NRPNs associated with any of the parameters of the single drumset record.
     * A single invocation of the send method will cause three NRPN messages to
     * be sent: the MSB, the LSB representing the selected parameter, followed
     * by the Data Entry value for that parameter.
     */
    public void send(IPatchDriver driver, int value) {
        try {
            driver.send(newControlChange(driver, NRPN_MSB, 0)); // Send NRPN MSB
            driver.send(newControlChange(driver, NRPN_LSB, param)); // Command
                                                                    // to select
                                                                    // the
                                                                    // parameter
            driver.send(newControlChange(driver, DATA_ENTRY_MSB, ccMap[value])); // Set
                                                                                 // the
                                                                                 // NRPN
                                                                                 // value
                                                                                 // using
     ...
     */
    protected ShortMessage newControlChange(IPatchDriver driver,
            int controlNumber, int value) throws InvalidMidiDataException {
        ShortMessage ccMessage = new ShortMessage();
        ccMessage.setMessage(ShortMessage.CONTROL_CHANGE, driver.getDevice()
                .getChannel() - 1, controlNumber, value);
        return ccMessage;
    }
}
