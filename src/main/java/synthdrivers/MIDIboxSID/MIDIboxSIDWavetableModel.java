...
class MIDIboxSIDWavetableModel extends AbstractTableModel {
    ...
    final String[] NoteName = new String[] {
            "---", "+++", "_C-2", "_D#2", "_E-2", "_F-2", "_F#2", "_G-2",
            "_G#2", "_A-2", "_A#2", "_B-2", "_C-1", "_C#1", "_D-1", "_D#1",
            "_E-1", "_F-1", "_F#1", "_G-1", "_G#1", "_A-1", "_A#1", "_B-1",
            ...
            "F#7", "G-7", "G#7", "A-7", "A#7", "B-7", "C-8", "C#8", "D-8",
            "D#8", "KEY1", "KEY2", "KEY3", "KEY4" };
    ...
    private byte convertStringToByte(String str) {
        ...
        try {
            value =
                    (int) java.lang.Integer.parseInt(str, getHexView() ? 16
                            : 10);
        } catch (Exception e) {
        }
    ...
    private String convertByteToString(byte value, int cc) {
        ...
        if (value >= 0) {
            if (value_i < 0x40) {
                sign = "+";
                value_i &= 0x3f;
            } else if (value_i >= 0x40) {
                sign = "-";
                value_i = 0x80 - value_i;
            }
        } else {
            if (cc >= 8 && cc <= 11) {
        ...
        if (getHexView()) {
            String hex_str =
                    "00"
                            + java.lang.Integer.toString(value_i, 16)
                                    .toUpperCase();
    ...
    public Object getValueAt(int row, int col) {
        if (col == 0)
            return convertByteToString((byte) (0x80 + row), 0);
        if (col == 1)
            return modeNames[dump[4 * row + 0]];
        if (col == 2)
            return convertByteToString(dump[4 * row + 1], columnCC[0]);
        if (col == 3)
            return convertByteToString(dump[4 * row + 2], columnCC[1]);
        if (col == 4)
            return convertByteToString(dump[4 * row + 3], columnCC[2]);
        return "Invalid";
    }
    ...
    public void setCookedDump(byte[] cooked_dump) {
        for (int i = 0; i < 32; ++i) {
            ...
            dump[i * 4 + 1] =
                    (byte) ((int) cooked_dump[i * 4 + 1] | ((mode_entry & 0x10) > 0 ? 0x80
                            : 0x00));
            dump[i * 4 + 2] =
                    (byte) ((int) cooked_dump[i * 4 + 2] | ((mode_entry & 0x20) > 0 ? 0x80
                            : 0x00));
            dump[i * 4 + 3] =
                    (byte) ((int) cooked_dump[i * 4 + 3] | ((mode_entry & 0x40) > 0 ? 0x80
                            : 0x00));
        }
    }
