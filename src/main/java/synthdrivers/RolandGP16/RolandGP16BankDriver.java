 ...
 * @version $Id: RolandGP16BankDriver.java 797 2005-01-03 23:45:04Z hayashi $
 */
public class RolandGP16BankDriver extends BankDriver {
    ...
    public RolandGP16BankDriver(RolandGP16SingleDriver singleDriver) {
        ...
        System.arraycopy(
                DriverUtil.generateNumbers(1, NS, "Group A - Bank ##"), 0,
                bankNumbers, 0, NS);
        System.arraycopy(
                DriverUtil.generateNumbers(1, NS, "Group B - Bank ##"), 0,
                bankNumbers, NS, NS);
        patchNumbers = new String[NS * 1];
        System.arraycopy(DriverUtil.generateNumbers(1, NS, "Patch ##"), 0,
                patchNumbers, 0, NS);
    ...
    public void calculateChecksum(Patch p) {
        for (int i = 0; i < NS; i++)
            calculateChecksum(p, i * singleSize + 5, i * singleSize + 124, i
                    * singleSize + 125);
    }
    ...
    public void requestSinglePatchDump(int bankNum, int patchNum) {
        ...
        nVs[0] =
                new SysexHandler.NameValue("patchnumber", bankNum * 8
                        + patchNum);
        nVs[1] = new SysexHandler.NameValue("checksum", 0);
        Patch p = new Patch(SYS_REQ.toByteArray(getChannel(), nVs));
        calculateChecksum(p, 5, 10, 11); // the gp-16 requires correct checksum
    ...
    public Patch createNewPatch() {
        ...
        for (int i = 0; i < NS; i++)
            System.arraycopy(((Patch) blankPatch).sysex, 0, sysex,
                    getPatchStart(i), singleSize);
