 ...
 * ====================================================================
 * @version $Id: DX7FamilyFractionalScalingBankDriver.java 891 2005-02-06 19:28:41Z hayashi $
...
public class DX7FamilyFractionalScalingBankDriver extends BankDriver {
    ...
    private static final int dxSinglePackedSize = 505; // size of single patch
                                                       // in packed bank format
    private static final int dxSysexHeaderSize = 4; // length of sysex header

    public DX7FamilyFractionalScalingBankDriver(byte[] initSysex,
            String[] dxPatchNumbers, String[] dxBankNumbers) {
        super("Fractional Scaling Bank", "Torsten Tittmann",
        ...
        sysexRequestDump =
                new SysexHandler(
                        "f0 43 @@ 7e 4c 4d 20 20 46 4b 53 59 43 20 f7 ");
        deviceIDoffset = 2;
        patchNameStart = 0;
        patchNameSize = 0;
        ...
        singleSize = 510;
        // checksumOffset=16163; // This patch doesn't uses an over-all checksum
        // for bank bulk data
        // checksumStart=6;
        // checksumEnd=16162;
        numSysexMsgs = 1;
    ...
    public void calculateChecksum(Patch p) {
        // This patch doesn't uses an over-all checksum for bank bulk data. Do
        // nothing!
    }
                                                            ...
                                                            // needed
    {
        ...
        ((Patch) bank).sysex[getPatchStart(patchNum) + 0] = (byte) (0x03); // Byte
                                                                           // Count
                                                                           // MSB
        ((Patch) bank).sysex[getPatchStart(patchNum) + 1] = (byte) (0x76); // Byte
                                                                           // Count
                                                                           // LSB
        ...
        for (int i = 0; i < 492; i++) {
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + i] =
                    (byte) ((Patch) p).sysex[16 + i];
        }

        // Calculate checkSum of single bulk data
        calculateChecksum(bank, getPatchStart(patchNum) + 2,
                getPatchStart(patchNum) + 12 + 492 - 1,
                                                    ...
                                                    // needed
    {
        try {
            ...
            for (int i = 0; i < 492; i++) {
                sysex[16 + i] =
                        (byte) (((Patch) bank).sysex[getPatchStart(patchNum)
                                + 12 + i]);
            }
    ...
    public Patch createNewPatch() // create a bank with 32 fractional scaling
                                  // patches
    {
