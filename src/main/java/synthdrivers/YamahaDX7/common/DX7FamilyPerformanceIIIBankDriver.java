 ...
 *		(used by TX802)
 * ================================================================
 * @version $Id: DX7FamilyPerformanceIIIBankDriver.java 744 2004-10-03 17:44:17Z hayashi $
...
public class DX7FamilyPerformanceIIIBankDriver extends BankDriver {
    ...
    private static final int dxPatchNameSize = 20; // size of patchname of
                                                   // single patch ==
                                                   // patchNameSize/2
    private static final int dxPatchNameOffset = 12 + (2 * 64); // offset in
                                                                // packed bank
                                                                // format
    private static final int dxSinglePackedSize = 3 + 10 + (2 * 84); // size of
                                                                     // single
                                                                     // patch in
                                                                     // packed
                                                                     // bank
    ...
    public DX7FamilyPerformanceIIIBankDriver(byte[] initSysex,
        ...
        sysexRequestDump =
                new SysexHandler("F0 43 @@ 7E 4C 4D 20 20 38 39 35 32 50 4D F7");
        deviceIDoffset = 2;
        patchNameStart = 0;
        patchNameSize = 0;
        ...
        singleSize = 250;
        // checksumOffset=11587; // This patch doesn't uses an over-all checksum
        // for bank bulk data
        // checksumStart=6;
        // checksumEnd=11586;
        numSysexMsgs = 1;
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        try {
            ...
            for (int i = 0; i < b.length; i++) {
                b[i] =
                        (byte) (DX7FamilyByteEncoding
                                .AsciiHex2Value(((Patch) p).sysex[patchNameStart
                                        + (2 * i)]) * 16 + DX7FamilyByteEncoding
                                .AsciiHex2Value(((Patch) p).sysex[patchNameStart
                                        + (2 * i) + 1]));
            }

            StringBuffer s =
                    new StringBuffer(new String(b, 0, dxPatchNameSize,
                            "US-ASCII"));
    ...
    public void setPatchName(Patch p, int patchNum, String name) {
        ...
        try {
            ...
            for (int i = 0; i < dxPatchNameSize; i++) {
                ((Patch) p).sysex[patchNameStart + (2 * i)] =
                        (byte) (DX7FamilyByteEncoding
                                .Value2AsciiHexHigh(namebytes[i]));
                ((Patch) p).sysex[patchNameStart + (2 * i) + 1] =
                        (byte) (DX7FamilyByteEncoding
    ...
    public int getByte(Patch p, int index) {
        return (DX7FamilyByteEncoding.AsciiHex2Value(p.sysex[index]) * 16 + DX7FamilyByteEncoding
                .AsciiHex2Value(p.sysex[index + 1]));
    }

    public void putPatch(Patch bank, Patch ip, int patchNum) // puts a patch
                                                             // into the bank,
                                                             // converting it as
                                                             // needed
    {
        ...
        ((Patch) bank).sysex[getPatchStart(patchNum) + 0] = (byte) (0x01); // Byte
                                                                           // Count
                                                                           // MSB
        ((Patch) bank).sysex[getPatchStart(patchNum) + 1] = (byte) (0x28); // Byte
                                                                           // Count
                                                                           // LSB
        ...
        ((Patch) bank).sysex[getPatchStart(patchNum) + 6] = (byte) (0x38); // "8"
        ((Patch) bank).sysex[getPatchStart(patchNum) + 7] = (byte) (0x39); // "9"
        ((Patch) bank).sysex[getPatchStart(patchNum) + 8] = (byte) (0x35); // "5"
        ((Patch) bank).sysex[getPatchStart(patchNum) + 9] = (byte) (0x32); // "2"
        ((Patch) bank).sysex[getPatchStart(patchNum) + 10] = (byte) (0x50); // "P"
        ((Patch) bank).sysex[getPatchStart(patchNum) + 11] = (byte) (0x4D); // "M"

        for (int i = 0; i < 8; i++) { // TG1-8 - Voice Channel Offset (0-7) |
                                      // MIDI Receive Channel (0-16)
            value =
                    getByte(p, 16 + (2 * 0) + (2 * i)) * 32
                            + getByte(p, 16 + (2 * 8) + (2 * i));

            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 0)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 0)
                    + (2 * i) + 1] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexLow(value));
        }

        for (int i = 0; i < 8; i++) { // TG1-8 - Voice Number (0-255)
            value = getByte(p, 16 + (2 * 16) + (2 * i));

            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 8)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 8)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 8; i++) { // TG1-8 - Micro Tuning Table # (0-254)
            ...
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 16)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 16)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 8; i++) { // TG1-8 - Output Volume (0-99)
            ...
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 24)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 24)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 8; i++) { // TG1-8 - Detune (0-14) | Key Assign
                                      // Group (0-1) | Output Assign (0-3)
            value =
                    getByte(p, 16 + (2 * 24) + (2 * i)) * 16
                            + getByte(p, 16 + (2 * 80) + (2 * i)) * 8
                            + getByte(p, 16 + (2 * 40) + (2 * i));

            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 32)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 32)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 8; i++) { // TG1-8 - Note Limit Low (0-127)
            ...
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 40)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 40)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 8; i++) { // TG1-8 - Note Limit High (0-127)
            ...
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 48)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 48)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 8; i++) { // TG1-8 - EG forced Damp (0-1) | Note
                                      // Shift (0-48)
            value =
                    getByte(p, 16 + (2 * 72) + (2 * i)) * 64
                            + getByte(p, 16 + (2 * 64) + (2 * i));

            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 56)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 56)
                    + (2 * i) + 1] =
        ...
        for (int i = 0; i < 20; i++) { // Performance Name (ASCII)
            ...
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 64)
                    + (2 * i)] =
                    (byte) (DX7FamilyByteEncoding.Value2AsciiHexHigh(value));
            ((Patch) bank).sysex[getPatchStart(patchNum) + 12 + (2 * 64)
                    + (2 * i) + 1] =
        ...
        calculateChecksum(bank, getPatchStart(patchNum) + 2,
                getPatchStart(patchNum) + 179, getPatchStart(patchNum) + 180);
    }

    public Patch getPatch(Patch b, int patchNum) // Gets a patch from the bank,
                                                 // converting it as needed
    {
        try {
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Voice Channel Offset (0-7)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 0)
                                + (2 * i)) & 224) / 32;
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Voice Number (0-255)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 8)
                                + (2 * i)) & 255);
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Detune (0-14)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 32)
                                + (2 * i)) & 112) / 16;
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Output Volume (0-99)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 24)
                                + (2 * i)) & 127);
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Note Limit low (0-127)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 40)
                                + (2 * i)) & 127);
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Note Limit high (0-127)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 48)
                                + (2 * i)) & 127);
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Note Shift (0-48)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 56)
                                + (2 * i)) & 63);
            ...
            for (int i = 0; i < 8; i++) { // TG1-8 - Micro Tuning Table #
                                          // (0-254)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 16)
                                + (2 * i)) & 255);
            ...
            for (int i = 0; i < 20; i++) { // Performance Name (ASCII)
                value =
                        (getByte(bank, getPatchStart(patchNum) + 12 + (2 * 64)
                                + (2 * i)) & 255);
