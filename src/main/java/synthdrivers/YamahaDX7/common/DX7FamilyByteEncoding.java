 ...
 * ================================================================
 * @version $Id: DX7FamilyByteEncoding.java 505 2004-05-03 04:41:00Z ribrdb $
...
public class DX7FamilyByteEncoding {
    // ################################# ASCII Hex format of TX802 Performance
    // patch <-> Parameter Value ########################
    // 'Value' is split into high and low nibble. Each nibble is encoded as a
    // Hex number, written in ASCII
    // E.g.: value = 0x7F --> ASCII String = 7F --> high nibble = 0x37, low
    // nibble = 0x46
    //
    // Attention! The values 0 - 9 correlate with ASCII value 0x30 - 0x39
    // and A - F corralate with ASCII value 0x41 - 0x46
    // The correlation is made by the arrays ASCII_HEX_2_PARAMETER_VALUE and
    ...
    protected final static int AsciiHex2Value(int value) {
        return (ASCII_HEX_2_PARAMETER_VALUE[Byte.parseByte(Integer
                .toHexString(value - 0x30))]);
    }

    // Convertion of Parameter value to ASCII HEX - High Nibble
    protected final static int Value2AsciiHexHigh(int value) {
        return (Integer.valueOf(
                Integer.toString(PARAMETER_VALUE_2_ASCII_HEX[value / 16]), 16)
                .intValue() + 0x30);
    }
    ...
    protected final static int Value2AsciiHexLow(int value) {
        return (Integer
                .valueOf(
                        Integer.toString(PARAMETER_VALUE_2_ASCII_HEX[value & 0x0F]),
                        16).intValue() + 0x30);
    }
}
