...
public class TCElectronicGMajorBankDriver extends BankDriver {
    ...
    public TCElectronicGMajorBankDriver(
        ...
        System.arraycopy(DriverUtil.generateNumbers(1,
    ...
    public String getPatchName(Patch p, int patchNum) {
        ...
        try {
            return new String(((Patch) p).sysex, nameOfst, patchNameSize,
                    "US-ASCII");
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        System.arraycopy(((Patch) p).sysex, 0, ((Patch) bank).sysex, singleSize
                * patchNum, singleSize);
        singleDriver.calculateChecksum(bank, (singleSize * patchNum)
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        ...
        System.arraycopy(((Patch) bank).sysex, singleSize * patchNum, sysex, 0,
                singleSize);
    ...
    public void storePatch(Patch p, int bankNum, int patchNum) {
        ...
        for (int i = 0; i < TCElectronicGMajorConst.NUM_PATCH; i++) {
            System.arraycopy(((Patch) p).sysex, singleSize * i, tmpPatch.sysex,
                    0, singleSize);

            // TODO: CREATE FACTORYBANK=0 AND USERBANK=1 CONSTANTS
            ((Patch) tmpPatch).sysex[7] =
                    (byte) TCElectronicGMajorUtil.calcBankNum(1, i);
            ((Patch) tmpPatch).sysex[8] =
