...
public class TCElectronicGMajorModDialog extends JDialog implements
        ItemListener {
    ...
    private final String[][] blockString =
            {
                    {
                            "[AutoRes] Mix", "[Res] Hi Cut", "[VinPhas] Speed",
                            "[SmthPhas] Speed", "[Tremolo] Speed",
                            "[Pan] Speed" },
                    {
                            "[AutoRes] OutLev", "[Res] Res", "[VinPhas] Depth",
                            "[SmthPhas] Depth", "[Tremolo] Depth",
                            "[Pan] Width" },
                    {
                            "", "[Res] Mix", "[VinPhas] FB", "[SmthPhas] FB",
                            "[Tremolo] Hi Cut", "[Pan] OutLev" },
                    {
                            "", "[Res] OutLev", "[VinPhas] Mix",
                            "[SmthPhas] Mix", "[Tremolo] OutLev", "" },
                    {
                            "", "", "[VinPhas] OutLev", "[SmthPhas] OutLev",
                            "", "" },
                    {
                            "[Detune] Mix", "[Whammy] Pitch", "[Octaver] Mix",
                            "[Shifter] Voice1" },
                    {
                            "[Detune] OutLev", "[Whammy] OutLev",
                            "[Octaver] OutLev", "[Shifter] Voice2" },
                    {
                            "", "", "", "[Shifter] Pan1" },
                    {
                            "", "", "", "[Shifter] Pan2" },
                            ...
                            "", "", "", "[Shifter] Mix" },
                    {
                            "", "", "", "[Shifter] OutLev" },
                    {
                            "[ClasCho] Speed", "[AdvCho] Speed",
                            "[ClasFla] Speed", "[AdvFla] Speed",
                            "[Vibrator] Speed" },
                    {
                            "[ClasCho] Depth", "[AdvCho] Depth",
                            "[ClasFla] Depth", "[AdvFla] Depth",
                            "[Vibrator] Depth" },
                    {
                            "[ClasCho] Hi Cut", "[AdvCho] Hi Cut",
                            "[ClasFla] Hi Cut", "[AdvFla] Hi Cut",
                            "[Vibrator] Hi Cut" },
                    {
                            "[ClasCho] Mix", "[AdvCho] Mix", "[ClasFla] FB",
                            "[AdvFla] FB", "[Vibrator] OutLev" },
                    {
                            "[ClasCho] OutLev", "[AdvCho] OutLev",
                            "[ClasFla] FB Cut", "[AdvFla] FB Cut", "" },
                    {
                    ...
                    {
                            "[PingPong] FB Hi Cut", "[Dynamic] FB Hi Cut",
                            "[Dual] FB1" },
                    {
                            "[PingPong] FB Lo Cut", "[Dynamic] FB Lo Cut",
                            "[Dual] FB2" },
                    {
                            "[PingPong] Mix", "[Dynamic] Mix",
                            "[Dual] FB Hi Cut" },
                    {
                            "[PingPong] OutLev", "[Dynamic] OutLev",
                            "[Dual] FB Lo Cut" },
                    {
                            "", "", "[Dual] Pan1" },
                    {
                            ...
                            "", "", "[Dual] Mix" },
                    {
                            "", "", "[Dual] OutLev" },
                    {
                            "[Spring] Mix", "[Hall] Mix", "[Room] Mix",
                            "[Plate] Mix" },
                    {
                            "[Spring] OutLev", "[Hall] OutLev",
                            "[Room] OutLev", "[Plate] OutLev" } };
    ...
    private TCElectronicGMajorModDialog(Frame frame, Component locationComp,
            Patch p) {
        ...
        JLabel label =
                new JLabel(
                        "Parameter                 Modifier   Min Out      Mid Out      Max Out");
        ...
        rightButtonPane.setLayout(new BoxLayout(rightButtonPane,
                BoxLayout.Y_AXIS));
        rightButtonPane.setBorder(BorderFactory
        ...
        bottomButtonPane.setLayout(new BoxLayout(bottomButtonPane,
                BoxLayout.LINE_AXIS));
        bottomButtonPane.setBorder(BorderFactory.createEmptyBorder(0, 10, 10,
                10));
    ...
    private void getModItems(Vector v, int from, int to, int ofs) {
        ...
        for (int i = from; i < to; i++) {
            param =
                    blockString[i][TCElectronicGMajorUtil.getValue(patch.sysex,
                            ofs)];

            if (param.length() > 0) {
                bit.setMaskOffset(TCElectronicGMajorConst.MOD_MASK, 29 + i * 4);
                mod = bit.get();
                bit.setMaskOffset(TCElectronicGMajorConst.MINOUT_MASK,
                        29 + i * 4);
                min = bit.get();
                bit.setMaskOffset(TCElectronicGMajorConst.MIDOUT_MASK,
                        30 + i * 4);
                mid = bit.get();
                bit.setMaskOffset(TCElectronicGMajorConst.MAXOUT_MASK,
                        31 + i * 4);
                max = bit.get();
    ...
    public void itemStateChanged(ItemEvent e) {
        if (e.getStateChange() == ItemEvent.SELECTED) {
            ...
            listVector.set(
                    modList.getSelectedIndex(),
                    new TCModListItem(tc.getOfs(), tc.getParam(), modCombo
                            .getSelectedIndex(), minCombo.getSelectedIndex(),
                            midCombo.getSelectedIndex(), maxCombo
                                    .getSelectedIndex()));
            modList.repaint();
