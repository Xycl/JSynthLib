/*
 * @version $Id: KawaiK5000BankDriver.java 744 2004-10-03 17:44:17Z hayashi $
 */
...
public class KawaiK5000BankDriver extends BankDriver {
    ...
    final static SysexHandler SYSEX_REQUEST_A_DUMP = new SysexHandler(
            "F0 40 @@ 01 00 0A 00 00 00 F7");
    // N.B. Following message specified incorrectly in K5000W/2 Midi
    // Implementation
    final static SysexHandler SYSEX_REQUEST_D_DUMP = new SysexHandler(
            "F0 40 @@ 01 00 0A 00 02 00 F7");
    ...
    public void generateIndex(Patch p) {
        ...
        for (int i = 0; i < 128; i++) {
            if (patchExists(p, i)) {
                ...
                int newPatchStart =
                        currentPatchStart
                                + (82 + 86 * p.sysex[currentPatchStart + 51]);
                int numWaveData = 0;
                for (int j = 0; j < p.sysex[currentPatchStart + 51]; j++)
                    if (((p.sysex[81 + currentPatchStart + 29 + (86 * j)] & 7) * 128 + p.sysex[81
                            + currentPatchStart + 30 + (86 * j)]) == 512)
                        numWaveData++;
    ...
    public String getPatchName(Patch ip, int patchNum) {
        ...
        try {
            StringBuffer s =
                    new StringBuffer(new String(p.sysex, nameStart, 8,
                            "US-ASCII"));
            return s.toString();
    ...
    public void putPatch(Patch b, Patch p, int patchNum) {
        ...
        // Find next existing patch in bank, this gives us the index at which
        // the data needs to be inserted
        ...
        ((Patch) bank).sysex =
                Utility.byteArrayReplace(((Patch) bank).sysex,
                        patchIndex[nextPatchNum], patchSize(bank, patchNum),
                        ((Patch) p).sysex, 9, ((Patch) p).sysex.length - 10);
    ...
    public void deletePatch(Patch b, int patchNum) {
        ...
        ((Patch) bank).sysex =
                Utility.byteArrayDelete(((Patch) bank).sysex,
                        patchIndex[patchNum], patchSize(bank, patchNum));
