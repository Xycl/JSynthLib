...
public class GenericDevice extends Device {
    ...
    private class IdentityDriver extends Driver {
        ...
        protected JSLFrame editPatch(Patch p) {
            ...
            f.append("Manuf ID             : "
                    + Utility.hexDump(p.sysex, 5, lengthOfID, -1, true) + " ("
                    + manuf + ")\n");
            f.append("Family (LSB First)   : "
                    + Utility.hexDump(p.sysex, 5 + lengthOfID, 2, -1, true)
                    + "\n");
            f.append("Product (LSB First)  : "
                    + Utility.hexDump(p.sysex, 7 + lengthOfID, 2, -1, true)
                    + "\n");
            f.append("Software (LSB First) : "
                    + Utility.hexDump(p.sysex, 9 + lengthOfID, 4, -1, true)
                    + "\n");
            return (f);
