 ...
 * files)
 * @version $Id: Storage.java 646 2004-07-30 16:57:44Z ribrdb $
...
public class Storage {
     ...
     * @param props
     *            the properties object to store it in
     */
    ...
    /**
     * Add the given object to the properties file, prefixing the names of its
     ...
     *            the properties object to store it in
     * @param prefix
     *            the prefix to prepend to all variable names
     */
        ...
        for (Iterator i = propNames.iterator(); i.hasNext();) {
            ...
            try {
                ...
                ErrorMsg.reportError("Storage Error",
                        "Error storing property '" + propName + "' for class '"
                                + storable.getClass() + "'", e);
            }
        }
    }
    ...
    /**
     * Restore the given object from the properties file, as a root-level object
     * @param storable
     *            the object to be restored
     * @param props
     *            the properties object from which to restore it
     */
    ...
    /**
     * Restore the given object from the properties file, prefixing the names of
    ...
    public static void restore(Storable storable, Properties props,
        ...
        for (Iterator i = propNames.iterator(); i.hasNext();) {
            ...
            try {
                ...
                ErrorMsg.reportError("Storage Error", "Cannot get property '"
                        + propName + "' for class '" + storable.getClass()
                        + "'", e);
            }
        }
     ...
     *            the object to store
     * @param props
     *            the properties file to store it into
     * @param propName
    ...
    private static void storeValue(Object value, Properties props,
        ...
        // Basic types
        if ((propType == Boolean.class) || (propType == Byte.class)
                || (propType == Character.class) || (propType == Short.class)
                || (propType == Integer.class) || (propType == Long.class)
                || (propType == Float.class) || (propType == Double.class)
        ...
        // Arrays
            ...
            for (int i = 0; i < length; i++) {
                // storeValue(value, props, propname, prefix)
                storeValue(Array.get(value, i), props, "[" + i + "]", prefix
                        + propName);
            }
        }
    ...
    private static Object restoreValue(Class propType, Properties props,
        ...
        // Arrays
            ...
            String lengthString =
                    props.getProperty(prefix + propName + ".length");
            ...
            Object aryValue =
                    Array.newInstance(propType.getComponentType(), length);

            for (int i = 0; i < length; i++) {
                // restoreValue(propType, props, propName, prefix)
                Object value =
                        restoreValue(propType.getComponentType(), props, "["
                                + i + "]", prefix + propName);
