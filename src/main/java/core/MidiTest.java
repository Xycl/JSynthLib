...
public class MidiTest implements Runnable {
    ...
    static boolean runLoopbackTest(int inport, int outport) {
        ...
        try {
            ...
            for (int i = 0; i < msgList.size(); i++) {
                // pbar.setValue(i);
                runLoopbackTest(inport, outport,
                        (MidiMessage) msgList.elementAt(i));
            }
     ...
     * the "in" port intact.
     */
    static void runLoopbackTest(int inport, int outport, MidiMessage msg)
            throws Exception {
        ...
        // Send it
        MidiUtil.send(rcvr, msg, AppConfig.getMidiOutBufSize(),
        ...
        try {
            ...
            throw e;
            // throw(new
            // Exception("Time expired without seeing any message come in"));
        }
    }
    ...
    /**
     * This is so that we can run this as a separate thread. I haven't actually
     * done this yet, because I'm worried about other threads being able to do
     * wierd things with the MidiWrapper while the test is running. - emenaker
     * 2003.03.26
     */
    public void run() {
