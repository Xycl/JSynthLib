...
public class MidiScan extends Thread {
    ...
    public void run() {
        ...
        byte[] idData =
                {
                        (byte) 0xf0, (byte) 0x7e, (byte) 0, (byte) 0x06,
                        (byte) 0x01, (byte) 0xf7 };
        ...
        for (int j = 0; j < maxout; j++) { // For all Outputs
            ...
            for (int devID = 0; devID <= MAX_DEVICE_ID; devID++) { // every
                ...
                for (int i = 0; i < maxin; i++) { // For all Inputs
                    ...
                    //
                    // The (answerData[2] == 0x06) and (answerData[3] == 0x02)
                    // parts
                    // are a hack that is needed e.g. for the Waldorf Microwave
                    // 2
        ...
        if (!found) {
            ...
            report +=
                    "Unknown Synthesizer found:\n"
                            ...
                            + MidiUtil.getOutputMidiDeviceInfo(midiout)
                                    .toString()
                            + " with Device ID: "
                            + devID
                            + 1
                            + "\n The Input Port was: "
                            + MidiUtil.getInputMidiDeviceInfo(midiin)
                                    .toString()
