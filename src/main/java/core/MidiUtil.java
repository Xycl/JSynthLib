...
public final class MidiUtil {
     ...
     * one hexdump line. (Complete Sysex Message)
     */
    ...
    static {
        ...
        sysexInputQueue =
                new MidiUtil.SysexInputQueue[inputMidiDeviceInfo.length];
    }
    ...
    private static MidiDevice.Info[] createInputMidiDeviceInfo()
        ...
        for (int i = 0; i < infos.length; i++) {
            ...
            if (device.getMaxTransmitters() != 0
                    ...
                    && !(device instanceof Sequencer) && !EMULATE_NO_MIDI_IN) {
                if (isMac()) {
    ...
    private static MidiDevice.Info[] createOutputMidiDeviceInfo()
        ...
        for (int i = 0; i < infos.length; i++) {
            ...
            if (device.getMaxReceivers() != 0
                    && !(device instanceof Synthesizer)
                    && !(device instanceof Sequencer) && !EMULATE_NO_MIDI_OUT)
                if (isMac()) {
                    if ("com.github.osxmidi4j.CoreMidiDeviceInfo"
                            .equals(infos[i].getClass().getName())) {
                        list.add(infos[i]);
     ...
     * com.sun.media.sound.FastShortMessage object instead of ShortMessage.
     */
    ...
    static class SysexInputQueue extends InputQueue {
            ...
            do {
                ...
                if (firstMsg) {
                    ...
                    if (len == 1) { // I think this is javax.sound.midi bug.
                        ...
                        System.arraycopy(buf, 1, combineBuffer, totalLen,
                                len - 1);
                        totalLen += len - 1;
                    }
        ...
        MidiMessage getMessage() throws TimeoutException,
                InvalidMidiDataException {
    ...
    /**
     * Dump input MIDI <code>sysex</code> byte array one the MIDI Monitor Window
     * with port number information.
     * @param port
     *            port number
     * @param sysex
     *            byte array
    ...
    /**
     * Dump output MIDI <code>sysex</code> byte array one the MIDI Monitor
    ...
    /**
     * Get the state of displaying Midi Messages in the MIDI Monitor. (Complete
     * Sysex Message)
     */
    static boolean getCSM() {
        return CSMstate;
    }
