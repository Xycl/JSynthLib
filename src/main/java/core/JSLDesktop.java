...
public class JSLDesktop implements JSLFrameListener {
    ...
    protected JSLDesktop(String title, JMenuBar mb, JToolBar tb,
            Action exitAction) {
        ...
        if (useMDI) {
            ...
            if (isMac
                ...
                ((JSLFakeDesktop) proxy).createToolBarWindow(title
                        + " Tool Bar", mb, tb);
        }
    }
    ...
    public void JSLFrameClosed(JSLFrameEvent e) {
        ...
        if (windows.isEmpty()
                || (!useMDI && ((JSLFakeDesktop) proxy).toolbar != null && windows
                        .size() == 1))
    ...
    private class JSLJDesktop extends JDesktopPane implements JSLDesktopProxy {
        ...
        public JSLFrame getSelectedJSLFrame() {
            try {
                return ((JSLFrame.JSLFrameProxy) this.getSelectedFrame())
                        .getJSLFrame();
            ...
            if (f == toolbar) {
                synchronized (in_fake_activation) {
                    ...
                    if (last_selected != null
                        ...
                        ((JSLFrame.JSLJFrame) last_selected.getJFrame())
                                .fakeActivate();
