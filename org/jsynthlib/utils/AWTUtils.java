...
public class AWTUtils {
	...
	public static Rectangle getAllScreenBounds() {
		...
		for(int i =1; i<screens.length; i++)
			allScreenBounds = union(allScreenBounds, screens[i].getDefaultConfiguration().getBounds());
	...
	public static void moveOnScreen(JFrame frame) {
		...
		// ensure left of rightmost edge
		if(x > b.x+b.width)
			x = b.x+b.width-frame.getWidth();
		if(y > b.y+b.height)
			y = b.y+b.height-frame.getHeight();

		// now ensure right of leftmost edge
		if(x < b.x)
			x = b.x;
		if(y < b.y)
			y = b.y;
