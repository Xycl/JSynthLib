...
public class RocktronIntellifexBankDriver extends BankDriver {
    ...
    public RocktronIntellifexBankDriver(RocktronIntellifexPresetDriver pDriver, RocktronIntellifexS81Driver s81Driver, RocktronIntellifexS82Driver s82Driver) {
        ...
        patchNameStart = 116;
        bankNumbers = new String[] {"User Data"};
    ...
    protected String getPatchName(Patch p, int patchNum) {
        ...
        try {
            ...
            for (int i = 0; i < patchNameSize; i++) {    
                String s = new String(p.sysex, patchStart + patchNameStart + 2*i, 1, "US-ASCII");
                name = name.concat(s);
            }
            return name;
    ...
    public void calculateChecksum (Patch p) {
        for (int i = 0; i < NUM_PRESETS; i++)
            presetDriver.calculateChecksum(p, PRESET_SIZE * i + 6, PRESET_SIZE*i + 6 + PRESET_DATA, PRESET_SIZE*i + 6 + PRESET_DATA);

        int pos = PRESET_SIZE * NUM_PRESETS;
        s81Driver.calculateChecksum(p, pos+6, pos+6+S81_DATA, pos+6+S81_DATA);

        pos += S81_SIZE;
        s82Driver.calculateChecksum(p, pos+6, pos+6+S82_DATA, pos+6+S82_DATA);
    }
    ...
    public void putPatch(Patch bank, Patch p, int patchNum) {
        ...
        switch (patchNum) {
            case S81_PATCHNUM:
                System.arraycopy(p.sysex, 0, bank.sysex, getPatchStart(patchNum), S81_SIZE);
                break;
            case S82_PATCHNUM:
                System.arraycopy(p.sysex, 0, bank.sysex, getPatchStart(patchNum), S82_SIZE);
                break;
            default:
                System.arraycopy(p.sysex, 0, bank.sysex, getPatchStart(patchNum), PRESET_SIZE);
        }
        bank.sysex[getPatchStart(patchNum)+5] = (byte)(patchNum==0 ? 0x2a : 0x2b);
    ...
    public Patch getPatch(Patch bank, int patchNum) {
        ...
        switch (patchNum) {
            case S81_PATCHNUM:
                ...
                System.arraycopy(bank.sysex, getPatchStart(patchNum), sysex, 0, S81_SIZE);
                driver = s81Driver;
                break;
            case S82_PATCHNUM:
                sysex = new byte[S82_SIZE];
                System.arraycopy(bank.sysex, getPatchStart(patchNum), sysex, 0, S82_SIZE);
                driver = s82Driver;
                break;
            default:
                sysex = new byte[PRESET_SIZE];
                System.arraycopy(bank.sysex, getPatchStart(patchNum), sysex, 0, PRESET_SIZE);
        ...
        try {
            ...
            ErrorMsg.reportError("Error", "Error in RocktronIntellifex Bank Driver", e);
            return null;
        }
    }
    ...
    protected void storePatch(Patch bank, int bankNum, int patchNum) {
        ...
        for (int i=0; i<NUM_PRESETS;i++) {
            ...
            patch.send();
            try { sleepAtLeast(1000); } catch(InterruptedException e) { }
        }
